# Aquifer Open Study Notes

**Aquifer Open Study Notes**

This work is an adaptation of *Tyndale Open Study Notes* © 2023 Tyndale House Publishers, licensed under the CC BY\-SA 4\.0 license. The adaptation, *Aquifer Open Study Notes*, was created by Mission Mutual and is also licensed under CC BY\-SA 4\.0\.

This resource has been adapted into multiple languages, including English, Tok Pisin, Arabic (عربي), French (Français), Hindi (हिंदी), Indonesian (Bahasa Indonesia), Portuguese (Português), Russian (Русский), Spanish (Español), Swahili (Kiswahili), and Simplified Chinese (简体中文).



--------------------------------


## Acts 1:1–3 (id: 9275)

Here, Luke summarizes his first book, the Gospel of Luke, which he wrote for Theophilus. Luke also calls him “most excellent” (Luke 1:3). This title indicates that he was a person of very important social status (compare Acts 23:26; 24:2; 26:25). He was probably a financial donor or patron who helped Luke with the costs of publishing or distributing his work (see study note on Luke 1:3).

* **Associated Passages:** Acts 1:1-3
* **Associated Articles:** Luke 1:3 (ID: `20435@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Theophilus (ID: `person:Theophilus`); Holy Spirit (ID: `deity:HolySpirit`); Kingdom (ID: `keyterm:Kingdom`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Life (ID: `keyterm:Life`); To Command (ID: `keyterm:Command`); Chosen (ID: `keyterm:Chosen`)

## Acts 1:3 (id: 9287)

* *He presented Himself to them with many convincing proofs that He was alive*: The Gospels provide evidence for Christ’s resurrection:

    + Jesus’s tomb was empty (Luke 24:3–4).
        + No one disturbed his grave clothes (John 20:3–9).
        + Many people saw him (see Matthew 28; Mark 16:1–8; Luke 24:1–53; John 20:11–21:23; 1 Corinthians 15:3–8).
        + Jesus appeared to the apostles to help them overcome their doubt (Matthew 28:17; Luke 24:17–24, 38, 41; John 20:27; see Mark 16:14).
* *the kingdom of God*: This was the main theme of Jesus’s teaching (seeActs 1:6–8; Matthew 4:17; Mark 1:14–15; Luke 4:43).

* **Associated Passages:** Acts 1:3

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Kingdom (ID: `keyterm:Kingdom`); Life (ID: `keyterm:Life`)

## Acts 1:4 (id: 9288)

*wait for the gift the Father promised*: See 2:1–13; compare with John 14:15–17.

* **Associated Passages:** Acts 1:4

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`); Jerusalem (ID: `place:Jerusalem`)

## Acts 1:5 (id: 9289)

*baptized*: See “Baptism” Theme Note.

* **Associated Passages:** Acts 1:5
* **Associated Articles:** Baptism (ID: `500380@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** John (ID: `person:John`); Holy Spirit (ID: `deity:HolySpirit`); Baptism (ID: `keyterm:Baptism`)

## Acts 1:6–11 (id: 9290)

*He was taken up:* Jesusascended into heaven (see also Luke 24:50–53). This happened on the Mount of Olives (Acts 1:12; Luke 24:50). This was the last physical appearance of Jesus. He ascended into heaven, where he will stay "until the time comes for the restoration of all things" (Acts 3:21).

* **Associated Passages:** Acts 1:6-11

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Ability (ID: `keyterm:Ability`); Jesus (ID: `person:Jesus.2`); Judea (ID: `place:Judea`); Witness (ID: `keyterm:Witness`); Galileans (ID: `group:Galilee`); Holy Spirit (ID: `deity:HolySpirit`); Kingdom (ID: `keyterm:Kingdom`); Galilee (ID: `place:Galilee`); LORD (ID: `deity:Lord`); Samaria (ID: `place:Samaria`); Authority (ID: `keyterm:Authority.2`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Ask for (ID: `keyterm:AskFor`); Clothing (ID: `realia:Clothing`)

## Acts 1:8 (id: 9291)

* This key verse shows how the good news about Jesus extended from place to place:

    + It began in Jerusalem and Judea (1:6–8:1).
        + It then came to Samaria (8:4–25).
        + Next, it went to Antioch in Syria (11:19–30).
        + Later, it extended across the Mediterranean Sea to Rome (13:1–28:31).
* Christ gave a similar mission to Saul of Tarsus (9:15). At first, Saul was well\-known for persecuting Jewish Christians. After his encounter with Christ, he became the main person spreading the gospel to non\-Jews (Gentiles).
* *when the Holy Spirit comes: The Holy Spirit* provided the power for all these missionary efforts for the good news about Jesus.

* **Associated Passages:** Acts 1:8

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Witness (ID: `keyterm:Witness`); Holy Spirit (ID: `deity:HolySpirit`); Ability (ID: `keyterm:Ability`); Samaria (ID: `place:Samaria`); Jerusalem (ID: `place:Jerusalem`)

## Acts 1:10 (id: 9276)

*as He was* *going:* The ascension of Jesus into heaven shows his promotion to a position of ultimate authority (see Ephesians 1:19–23; Philippians 2:9–11).

* **Associated Passages:** Acts 1:10

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Clothing (ID: `realia:Clothing`)

## Acts 1:11 (id: 9277)

*in the same way*: Jesus promised to return from heaven in the same way (see Matthew 24:30; Mark 13:26; John 14:3). Jesus has gone to heaven to prepare a place for his followers (John 12:26; 14:1–4; 2 Corinthians 5:4; Philippians 3:21; Hebrews 6:20).

* **Associated Passages:** Acts 1:11

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Galileans (ID: `group:Galilee`); Jesus (ID: `person:Jesus.2`); Galilee (ID: `place:Galilee`)

## Acts 1:12 (id: 9278)

Those who witnessed Christ going into heaven returned to Jerusalem with joy, worship, excitement, and praise to God (Luke 24:52). The angels' message eased their doubts (see study note on Acts 1:3).

* **Associated Passages:** Acts 1:12
* **Associated Articles:** Acts 1:3 (ID: `9287@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Mount of Olives (ID: `place:OlivesMount`); Half a Mile (ID: `realia:HalfAMile`); Sabbath Journey (ID: `keyterm:HalfAMile`); Jerusalem (ID: `place:Jerusalem`); Olive Tree (ID: `flora:OliveTree`)

## Acts 1:13 (id: 9280)

All eleven of the remaining apostles were present (compare Matthew 10:2–4; Mark 3:16–19; Luke 6:13–16). Judas Iscariot is not on this list because he had already betrayed Jesus and killed himself (Acts 1:18–19).

* **Associated Passages:** Acts 1:13

* **Associated ACAI Concepts:** Bartholomew (ID: `person:Bartholomew`); Alphaeus (ID: `person:Alphaeus`); James (ID: `person:James.2`); Andrew (ID: `person:Andrew`); Simon (ID: `person:Simon.2`); Thomas (ID: `person:Thomas`); Upstairs Room (ID: `realia:UpstairsRoom`); Peter (ID: `person:Peter`); Philip (ID: `person:Philip`); John (ID: `person:John.2`); James (ID: `person:James`); Matthew (ID: `person:Matthew`); Judas (ID: `person:Judas`)

## Acts 1:13–26 (id: 9279)

The apostles waited for the promised gift of the Holy Spirit (1:4–5). All 120 disciples in Jerusalem were persisting in prayer (1:13–14). Then they chose a new apostle to replace Judas Iscariot (1:15–26).

* **Associated Passages:** Acts 1:13-26

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Matthias (ID: `person:Matthias`); Pray (ID: `keyterm:Pray`); Peter (ID: `person:Peter`); Judas (ID: `person:Judas.2`); Serve (ID: `keyterm:Serve`); Andrew (ID: `person:Andrew`); Akeldama (ID: `place:Akeldama`); Simon (ID: `person:Simon.2`); Mary (ID: `person:Mary`); Thomas (ID: `person:Thomas`); Joseph (ID: `person:Joseph.9`); Alphaeus (ID: `person:Alphaeus`); Bartholomew (ID: `person:Bartholomew`); James (ID: `person:James.2`); Position of Responsibility (ID: `keyterm:PositionOfResponsibility`); Resurrection (ID: `keyterm:Resurrection`); John (ID: `person:John.2`); Matthew (ID: `person:Matthew`); Witness (ID: `keyterm:Witness`); Judas (ID: `person:Judas`); Philip (ID: `person:Philip`); John (ID: `person:John`); David (ID: `person:David`); Knower of Hearts (ID: `keyterm:KnowerOfHearts`); Written (ID: `keyterm:Scripture`); To Cause to Happen (ID: `keyterm:Happen`); LORD (ID: `deity:Lord`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Baptism (ID: `keyterm:Baptism`); James (ID: `person:James`); Holy Spirit (ID: `deity:HolySpirit`); Blood (ID: `keyterm:Blood`); Unjust Deed (ID: `keyterm:UnjustDeed`); Chosen (ID: `keyterm:Chosen`); Lot (ID: `realia:Lot`); Upstairs Room (ID: `realia:UpstairsRoom`)

## Acts 1:14 (id: 9281)

* The first disciples came together for worship, fellowship, and prayer (see also 2:42, 46–47; 4:24–31; 5:12; 12:5; 13:1–3). The faithful women who witnessed Jesus's crucifixion and resurrection were there (Luke 23:49, 54–56; 24:1–10, 22).
* The brothers of Jesus did not believe in him during his earthly ministry (John 7:3–5; compare Matthew 12:46–49). But they became his disciples after his resurrection.

* **Associated Passages:** Acts 1:14

* **Associated ACAI Concepts:** Pray (ID: `keyterm:Pray`); Mary (ID: `person:Mary`); Jesus (ID: `person:Jesus.2`)

## Acts 1:16–17 (id: 9282)

*the Scripture had to be fulfilled*: See 1:20.

* **Associated Passages:** Acts 1:16-17

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Serve (ID: `keyterm:Serve`); Written (ID: `keyterm:Scripture`); To Cause to Happen (ID: `keyterm:Happen`); Jesus (ID: `person:Jesus.2`); Judas (ID: `person:Judas.2`); David (ID: `person:David`)

## Acts 1:18–19 (id: 9283)

The account in the Gospel of Matthew is slightly different (Matthew 27:3–10). We can match the two accounts by understanding the priests' purchase of the “Field of Blood” as if they acted on behalf of Judas. Also, Judas may have killed himself by hanging, but he later fell headfirst. Then his body split open, possibly when the rope broke.

* **Associated Passages:** Acts 1:18-19

* **Associated ACAI Concepts:** Akeldama (ID: `place:Akeldama`); Blood (ID: `keyterm:Blood`); Unjust Deed (ID: `keyterm:UnjustDeed`); Jerusalem (ID: `place:Jerusalem`)

## Acts 1:20–21 (id: 9284)

*May another take his position*: It was extremely important to find someone to replace Judas so there would still be twelve apostles. The church was the new Israel so it needed twelve leaders chosen and appointed by God (see Matthew 19:28; Luke 22:29–30).

* **Associated Passages:** Acts 1:20-21

* **Associated ACAI Concepts:** Position of Responsibility (ID: `keyterm:PositionOfResponsibility`); Jesus (ID: `person:Jesus.2`)

## Acts 1:21–22 (id: 9285)

To qualify to be an apostle, a man needed to have been with the other apostles throughout the public ministry of Jesus. This period started from the time John baptized Jesus and continued until Jesus was taken up into heaven.

* **Associated Passages:** Acts 1:21-22

* **Associated ACAI Concepts:** John (ID: `person:John`); Resurrection (ID: `keyterm:Resurrection`); Jesus (ID: `person:Jesus.2`); Witness (ID: `keyterm:Witness`); Baptism (ID: `keyterm:Baptism`)

## Acts 1:23–26 (id: 9286)

* With prayer and humble dependence on God that he would reveal his will, the apostles cast lots (1:14, 24). This was a traditional way to discover God’s will (see Leviticus 16:8; Numbers 27:21; Deuteronomy 33:8; Joshua 14:2; 18:3–10; Proverbs 16:33). God chose Matthias by lot to replace Judas. After Pentecost, the Holy Spirit guided the followers of Jesus through dreams, visions, and prophecies instead of through lots (compare Acts 2:17–18; 13:2; 16:7–10).
* *You know everyone's heart*: See also 1 Samuel 16:6–7; 1 Kings 8:39; 1 Chronicles 28:9; 2 Chronicles 6:30; Psalms 7:9; 44:21; Jeremiah 11:20; John 2:24–25; Revelation 2:23.

* **Associated Passages:** Acts 1:23-26

* **Associated ACAI Concepts:** Matthias (ID: `person:Matthias`); Joseph (ID: `person:Joseph.9`); Knower of Hearts (ID: `keyterm:KnowerOfHearts`); Pray (ID: `keyterm:Pray`); Serve (ID: `keyterm:Serve`); LORD (ID: `deity:Lord`); Judas (ID: `person:Judas.2`); Chosen (ID: `keyterm:Chosen`); Lot (ID: `realia:Lot`)

## Acts 2:1–4 (id: 9449)

Jews observed three traditional annual festivals. Many people traveled to the temple in Jerusalem for these feasts (see Leviticus 23:1–44): 

* The Feast of Unleavened Bread (which includes Passover)
* The Feast of Weeks (also called Pentecost or the Feast of Harvest)
* The Feast of Tabernacles

On Pentecost, Jewish people gathered in Jerusalem to thank God for the harvest and his blessings. At the same time, the disciples of Jesus gathered together in prayer with shared hope. Then the Holy Spirit came and filled the believers who were present.

* **Associated Passages:** Acts 2:1-4

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy Spirit (ID: `deity:HolySpirit`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 2:4 (id: 9460)

* *And they were all filled with the Holy Spirit*: This event marks when the Holy Spirit came down from heaven. He filled the disciples, as Jesus had promised, to empower them as witnesses of Christ's resurrection (1:4–5, 8). The Spirit's wisdom, energy, and power enabled the church's work and witness (for example, 2:14–41, 43; 4:31; 9:17, 20; 13:9–12; see also Ephesians 5:18).
* *began to speak in other tongues*: The Holy Spirit gave them the ability to communicate so people from other countries could hear about God's works in their own languages (Acts 2:6–11). This passage is one of several references to speaking in "other tongues" (see also 10:44–48; 19:6; 1 Corinthians 14:2–28, 39). This divine gift shows the energy of the presence of God's Spirit. This event marks the beginning of the gospel (the good news about Jesus) being announced to people from every nation.

* **Associated Passages:** Acts 2:4

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`)

## Acts 2:5–11 (id: 9461)

Since Pentecost was a feast people had to travel to attend, Jews from every nation were in Jerusalem (see study note on 2:1–4). They could hear the gospel (the good news about Jesus) in their own languages. The gospel is for all people (2:38–39).

* **Associated Passages:** Acts 2:5-11
* **Associated Articles:** Acts 2:1-4 (ID: `9449@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Elamites (ID: `group:Elam`); Medan (ID: `person:Medan`); Phrygia (ID: `place:Phrygia`); Cretans (ID: `group:Cretan`); Pontus (ID: `place:Pontus`); Jerusalem (ID: `place:Jerusalem`); Libya (ID: `place:Libya`); Cyrene (ID: `place:Cyrene`); Elam (ID: `place:Elam`); Galileans (ID: `group:Galilee`); Parthians (ID: `group:Parthians`); Cappadocia (ID: `place:Cappadocia`); Crete (ID: `place:Crete`); Egypt (ID: `place:Egypt`); Mesopotamia (ID: `place:Mesopotamia`); Pamphylia (ID: `place:Pamphylia`); Arabians (ID: `group:Arabia`); Sky (ID: `keyterm:Heaven`); Asia (ID: `place:Asia`); Medes (ID: `group:Mede`); LORD (ID: `deity:Lord`); Mighty Act (ID: `keyterm:MightyAct`); Judea (ID: `place:Judea`); Rome (ID: `place:Rome`); Galilee (ID: `place:Galilee`); Arabia (ID: `place:Arabia`)

## Acts 2:9–11 (id: 9462)

* The Parthians came from the region between the Tigris River and India.

    The Medes, now known as Kurds, were from Media, located east of Mesopotamia and north of the Persian Gulf.

    The Elamites were from Elam. This area is now in Iran, north of the Persian Gulf and just east of the Tigris River.

    Mesopotamia was between the Tigris and Euphrates rivers.

    Judea was well known as the Jewish homeland.

    Cappadocia, Pontus, and Asia were Roman provinces in Asia Minor, now part of Turkey (see 1 Peter 1:1). The apostle Paul later taught and preached in some of these areas (see Acts 16:6; 19:10, 26).

    Phrygia and Pamphylia were districts in Asia Minor that Paul later visited (13:13; 18:23).

    Egypt had a large Jewish population, especially in Alexandria (see 18:24).

    North Africa included Libya and Cyrene.

    Rome was the capital of the Roman Empire and home to thousands of *Jews*.

    The Cretans were from the island of Crete, located south\-southeast of Greece.

    The Arabs were from the area south and east of Jerusalem.

* Including all of these nations demonstrates how God intended the gospel to reach the ends of the earth (1:8).

* **Associated Passages:** Acts 2:9-11

* **Associated ACAI Concepts:** Elamites (ID: `group:Elam`); Medan (ID: `person:Medan`); Phrygia (ID: `place:Phrygia`); Cretans (ID: `group:Cretan`); Pontus (ID: `place:Pontus`); Libya (ID: `place:Libya`); Cyrene (ID: `place:Cyrene`); Elam (ID: `place:Elam`); Parthians (ID: `group:Parthians`); Cappadocia (ID: `place:Cappadocia`); Crete (ID: `place:Crete`); Jews (ID: `group:Jews`); Egypt (ID: `place:Egypt`); Mesopotamia (ID: `place:Mesopotamia`); Pamphylia (ID: `place:Pamphylia`); Arabians (ID: `group:Arabia`); Asia (ID: `place:Asia`); Medes (ID: `group:Mede`); LORD (ID: `deity:Lord`); Mighty Act (ID: `keyterm:MightyAct`); Judea (ID: `place:Judea`); Rome (ID: `place:Rome`); Arabia (ID: `place:Arabia`)

## Acts 2:10–11 (id: 9450)

*converts to Judaism*: See study note on 13:43.

* **Associated Passages:** Acts 2:10-11
* **Associated Articles:** Acts 13:43 (ID: `9343@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Cretans (ID: `group:Cretan`); Phrygia (ID: `place:Phrygia`); Cyrene (ID: `place:Cyrene`); Libya (ID: `place:Libya`); Egypt (ID: `place:Egypt`); LORD (ID: `deity:Lord`); Mighty Act (ID: `keyterm:MightyAct`); Pamphylia (ID: `place:Pamphylia`); Arabia (ID: `place:Arabia`); Rome (ID: `place:Rome`); Crete (ID: `place:Crete`); Jews (ID: `group:Jews`); Arabians (ID: `group:Arabia`)

## Acts 2:14–36 (id: 9451)

This is the first of about 30 speeches in Acts and one of the most important. It marks the beginning of the Christian church. It is a typical example of the preaching of the apostles. 

The apostles proclaimed: 

1. that the Old Testament promises came true in the life, death, and resurrection of Jesus, the promised Messiah;
2. that the apostles were eyewitnesses of the public ministry of Jesus and were his chosen representatives;
3. that people must turn away (repent) from their sins and trust in God through Christ; and
4. that salvation and the presence of the Holy Spirit are promised to those who have a positive response to this message of the gospel.

This basic message repeats in the sermons of chapters 3, 4, 5, 8, 10, and 13. The same themes characterized the apostle Paul's preaching (see 1 Corinthians 15:3–9). The apostles all repeated this message when preaching to both Jews and non\-Jews (Gentiles) throughout the world around the Mediterranean Sea. All people must turn away (repent) from their sins and turn to God by trusting in Jesus Christ.

* **Associated Passages:** Acts 2:14-36

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Body (ID: `keyterm:Body.3`); David (ID: `person:David`); Holy Spirit (ID: `deity:HolySpirit`); Portent (ID: `keyterm:Portent`); World of the Dead (ID: `keyterm:WorldOfTheDead`); Hell (ID: `place:Hell`); Israel (ID: `person:Israel`); Symbol (ID: `keyterm:Example`); Speak as a Prophet (ID: `keyterm:Speak`); Sky (ID: `keyterm:Heaven`); Blood (ID: `keyterm:Blood`); Cross (ID: `realia:Cross`); Peter (ID: `person:Peter`); Resurrection (ID: `keyterm:Resurrection`); Joel (ID: `person:Joel.14`); Judeans (ID: `group:Judea`); Word (ID: `keyterm:Word`); Call Upon (ID: `keyterm:CallUpon`); Oath (ID: `keyterm:Oath`); Heart (ID: `keyterm:Heart`); Hope (ID: `keyterm:Hope`); Witness (ID: `keyterm:Witness`); Something Definite (ID: `keyterm:SomethingDefinite`); Lawless (ID: `keyterm:Lawless`); Salvation (ID: `keyterm:Salvation`); Life (ID: `keyterm:Life`); Holy (ID: `keyterm:Holy`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Judea (ID: `place:Judea`); Israelites (ID: `group:Israel`); Nazareth (ID: `place:Nazareth`); Ability (ID: `keyterm:Ability`); Promise (ID: `keyterm:Promise`); Be Lawful (ID: `keyterm:Lawful`); Footstool (ID: `realia:Footstool`); Grave (ID: `realia:Grave`); Throne (ID: `realia:Throne`)

## Acts 2:17–21 (id: 9452)

This passage quotes Joel 2:28–32. The prophet Joel prophesied that in the last days, many people would have the gifts of prophecy, including visions and dreams. This would include both young and old, and both men and women (Joel 2:28–29). In Acts, on the day of Pentecost, Peter announced that Joel’s prophecy was coming true, indicating that "the last days" had begun.

* **Associated Passages:** Acts 2:17-21

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`); Speak as a Prophet (ID: `keyterm:Speak`); Blood (ID: `keyterm:Blood`); Call Upon (ID: `keyterm:CallUpon`); Portent (ID: `keyterm:Portent`); Body (ID: `keyterm:Body.3`); Name (ID: `keyterm:Name`); Salvation (ID: `keyterm:Salvation`); Symbol (ID: `keyterm:Example`); Sky (ID: `keyterm:Heaven`)

## Acts 2:23–28 (id: 9453)

Although cruel men put Jesus to death on a cross, they did not defeat God’s plan for salvation. In fact, God raised Jesus from the dead, just as David had foretold (2:25–28).

* **Associated Passages:** Acts 2:23-28

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); World of the Dead (ID: `keyterm:WorldOfTheDead`); Lawless (ID: `keyterm:Lawless`); Body (ID: `keyterm:Body.3`); Heart (ID: `keyterm:Heart`); Hell (ID: `place:Hell`); Hope (ID: `keyterm:Hope`); Life (ID: `keyterm:Life`); Holy (ID: `keyterm:Holy`); David (ID: `person:David`); Cross (ID: `realia:Cross`)

## Acts 2:27 (id: 9454)

*abandon my soul to Hades*: See the study note on Matthew 11:23.

* **Associated Passages:** Acts 2:27
* **Associated Articles:** Matthew 11:23 (ID: `21692@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** World of the Dead (ID: `keyterm:WorldOfTheDead`); Hell (ID: `place:Hell`); Holy (ID: `keyterm:Holy`)

## Acts 2:32–36 (id: 9455)

*God has raised this Jesus to life, to which we are all witnesses*: The theme of witnessing repeats often in Acts (for example, 3:15; 4:33; 5:32; 10:39–43). The main point of this message is that God the Father, as he promised, is now pouring out his Holy Spirit on the church through the resurrected Jesus. All the people of Israel were called to recognize that God had acted with power. He made Jesus, whom they put to death on a cross, both Lord and Messiah!

* **Associated Passages:** Acts 2:32-36

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Holy Spirit (ID: `deity:HolySpirit`); Something Definite (ID: `keyterm:SomethingDefinite`); Israel (ID: `person:Israel`); Promise (ID: `keyterm:Promise`); Witness (ID: `keyterm:Witness`); Sky (ID: `keyterm:Heaven`); David (ID: `person:David`); Cross (ID: `realia:Cross`); Footstool (ID: `realia:Footstool`)

## Acts 2:34–35 (id: 9456)

This passage quotes Psalm 110:1, which Jesus fulfilled when he ascended (went up) into heaven (Acts 1:9).

* **Associated Passages:** Acts 2:34-35

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Sky (ID: `keyterm:Heaven`); David (ID: `person:David`); Footstool (ID: `realia:Footstool`)

## Acts 2:37–38 (id: 9457)

*Brothers, what shall we do?*: Peter urged his listeners to repent of their sins, trust in God, and be baptized. See the “Baptism” Theme Note.

* **Associated Passages:** Acts 2:37-38
* **Associated Articles:** Baptism (ID: `500380@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Repent (ID: `keyterm:Repent`); Baptism (ID: `keyterm:Baptism`); Holy Spirit (ID: `deity:HolySpirit`); Heart (ID: `keyterm:Heart`); Forgive (ID: `keyterm:Forgive`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); To Sin (ID: `keyterm:Sin`)

## Acts 2:42 (id: 9459)

* *fellowship*: The Greek word *koinōnia* means a close relationship marked by sharing and participating in life together.
* *and to prayer:* At this stage, prayer probably included taking part in the formal prayers of the temple (see 3:1).

* **Associated Passages:** Acts 2:42

* **Associated ACAI Concepts:** Fellowship (ID: `keyterm:Fellowship`); Pray (ID: `keyterm:Pray`)

## Acts 2:42–47 (id: 9458)

Luke makes a clear connection between personal faith and belonging to the Christian community. Life in this new community included:

* devotion to the teaching of the apostles from God’s word,
* fellowship,
* sharing,
* joy, and
* praise.

As a result, the Lord continued to add to their number those who were being saved.

* **Associated Passages:** Acts 2:42-47

* **Associated ACAI Concepts:** Fellowship (ID: `keyterm:Fellowship`); Pray (ID: `keyterm:Pray`); Favor (ID: `keyterm:Favor`); Fear (ID: `keyterm:Fear`); Heart (ID: `keyterm:Heart`); LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Symbol (ID: `keyterm:Example`); Salvation (ID: `keyterm:Salvation`); In Common (ID: `keyterm:InCommon`); Portent (ID: `keyterm:Portent`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 3:1–11 (id: 9590)

Jesus promised that his disciples would do greater works than he did (John 14:12; see Mark 16:20). This promise came true through the signs, wonders, and mighty works of the apostles (Acts 2:43; 5:12; 8:4–8). 

The apostle Peter showed the power to heal in the name of Jesus Christ of Nazareth (3:6, 16). The healing was immediate and evident (3:8). It led the man to praise God (3:8–9; compare 2:47; 16:25; Luke 2:20; 17:15–18; 18:43; 19:37; 24:53). This event is the first of many displays of divine power given to the apostles in Acts (Acts 4:24–31; 5:12; 6:8; 8:6; 9:33–42; 28:8).

* **Associated Passages:** Acts 3:1-11

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Temple (ID: `realia:Temple`); John (ID: `person:John.2`); Beautiful Gate (ID: `place:BeautifulGate`); LORD (ID: `deity:Lord`); Solomon's Colonnade (ID: `place:SolomonsPortico`); Pray (ID: `keyterm:Pray`); Nazareth (ID: `place:Nazareth`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Ask for (ID: `keyterm:AskFor`); Porch (ID: `realia:Porch`); Door (ID: `realia:Door`)

## Acts 3:6 (id: 9600)

*In the name of Jesus*: Speaking the name of Jesus demonstrates who he is and his power to heal (see Mark 9:38–39).

* **Associated Passages:** Acts 3:6

* **Associated ACAI Concepts:** Nazareth (ID: `place:Nazareth`); Peter (ID: `person:Peter`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`)

## Acts 3:10–11 (id: 9591)

* The Beautiful Gate was probably the Nicanor Gate, made of bronze from the city of Corinth. It was on the east side of the temple (see Josephus, *War* 5\.5\.3\). It had rich decorations of silver and gold plates on its panels. The gold and silver were heavier and more valuable than the plates on other gates.
* Solomon’s Colonnade was a covered place to walk on the east side of the temple (see Josephus, *War* 5\.5\.1–3; *Antiquities* 20\.9\.7\).

* **Associated Passages:** Acts 3:10-11

* **Associated ACAI Concepts:** Solomon's Colonnade (ID: `place:SolomonsPortico`); Beautiful Gate (ID: `place:BeautifulGate`); Porch (ID: `realia:Porch`); Peter (ID: `person:Peter`); John (ID: `person:John.2`); Temple (ID: `realia:Temple`)

## Acts 3:12–26 (id: 9592)

The speeches in the book of Acts about telling others the good news focus on Jesus, who was put to death on a cross and raised to life. These speeches call people to repent and to place their faith in Jesus as the promised Messiah and the judge God has appointed (2:38; 3:19; 11:18; 17:30; 26:20). They also announce the same good news for the people of Israel and for Gentiles everywhere: there is "peace through Jesus Christ, who is Lord of all" (10:36).

* **Associated Passages:** Acts 3:12-26

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Abraham (ID: `person:Abraham`); Israel (ID: `person:Israel`); Holy (ID: `keyterm:Holy`); Samuel (ID: `person:Samuel`); To Cause to Happen (ID: `keyterm:Happen`); Peter (ID: `person:Peter`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Resurrection (ID: `keyterm:Resurrection`); Witness (ID: `keyterm:Witness`); To Sin (ID: `keyterm:Sin`); Covenant (ID: `keyterm:Covenant`); Isaac (ID: `person:Isaac`); Forgive (ID: `keyterm:Forgive`); Life (ID: `keyterm:Life`); Pilate (ID: `person:Pilate`); Sky (ID: `keyterm:Heaven`); Bless (ID: `keyterm:Bless`); Always (ID: `keyterm:Always`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Moses (ID: `person:Moses`); Repent (ID: `keyterm:Repent`); Israelites (ID: `group:Israel`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Decided (ID: `keyterm:Decided`)

## Acts 3:15 (id: 9593)

Many Jewish people could not imagine that God himself, the author of life, could be killed. They did not recognize Jesus as the Messiah (compare 13:27; Luke 23:34). They also did not understand that the Messiah would be divine, that is, sharing God’s nature (see John 1:1–18; compare Daniel 7:13–14).

* **Associated Passages:** Acts 3:15

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Resurrection (ID: `keyterm:Resurrection`); Witness (ID: `keyterm:Witness`); Life (ID: `keyterm:Life`)

## Acts 3:17 (id: 9594)

* *brothers*: This was a common way to address each other (13:15, 26, 38; Genesis 27:29; Leviticus 10:6; 25:46; Numbers 20:3; Deuteronomy 1:28; 24:7; Joshua 22:3–4; 2 Samuel 2:26; Ezekiel 11:15).
* *in ignorance*: See Acts 17:30; 1 Timothy 1:12–14. The Christian message challenges this lack of knowledge. The gospel calls everyone to respond with faith and to repent (Acts 2:38).

* **Associated Passages:** Acts 3:17


## Acts 3:19 (id: 9596)

Other sections of the book of Acts and the Gospel of Luke explain the good news of "forgiveness" in more detail (see Acts 5:30–32; 10:36–43; 13:26–38; Luke 24:25–27, 45–47).

* **Associated Passages:** Acts 3:19

* **Associated ACAI Concepts:** Repent (ID: `keyterm:Repent`); To Sin (ID: `keyterm:Sin`)

## Acts 3:19–21 (id: 9595)

The message of the good news about Jesus (the gospel) brings comfort (see Matthew 11:28–29). The second coming of Jesus will bring *times of refreshing* from God. He will *send Jesus* to his people again.

* **Associated Passages:** Acts 3:19-21

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Repent (ID: `keyterm:Repent`); Sky (ID: `keyterm:Heaven`); Always (ID: `keyterm:Always`); Jesus (ID: `person:Jesus.2`); Holy (ID: `keyterm:Holy`); To Sin (ID: `keyterm:Sin`)

## Acts 3:22 (id: 9597)

Jesus fulfills the ancient promise that God would raise up a prophet like Moses (see Deuteronomy 18:15, 18; compare John 6:14; 7:40).

* **Associated Passages:** Acts 3:22

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Moses (ID: `person:Moses`)

## Acts 3:23 (id: 9598)

Refusing to obey Jesus, who is God's ultimate prophet, will lead to severe consequences (John 3:16, 17, 36).

* **Associated Passages:** Acts 3:23


## Acts 3:25 (id: 9599)

*all the families of the earth will be blessed*: God always intended the covenant people to share his blessings with the world, not for the Jews to be selfish and keep God's blessing only for themselves.

* **Associated Passages:** Acts 3:25

* **Associated ACAI Concepts:** Abraham (ID: `person:Abraham`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Acts 4:1 (id: 9605)

*Sadducees*: See “The Sadducees” Profile.

* **Associated Passages:** Acts 4:1
* **Associated Articles:** The Sadducees (ID: `500538@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Sadducees (ID: `group:Sadducee`); Temple (ID: `realia:Temple`)

## Acts 4:1–22 (id: 9601)

Throughout the Bible, God's people often experienced persecution. God's servants often encountered hostility and opposition (Deuteronomy 30:7; 1 Kings 18:13; Nehemiah 4:1–3; Jeremiah 37–38; Matthew 23:34–37; Luke 11:49–51; 1 Thessalonians 2:14–15). Jesus was also persecuted (Luke 4:29; John 5:16). He warned his disciples to expect similar treatment (Matthew 10:23; 24:9; Mark 13:9; Luke 21:12; John 16:2). However, he promised that the Holy Spirit would provide them with strength (Acts 1:8; Luke 12:11–12; 21:15). 

The book of Acts records many examples of persecution (Acts 4:3; 5:17–41; 7:54–8:3; 9:1–2; 11:19; 12:2; 13:50; 14:19; 16:19–24). But it also emphasizes that the Holy Spirit empowers apostles to testify in such situations (4:8–13; 6:10; 7:55). The courage of Peter and John before the hostile Jewish high council shows how Christians should face persecution with courage and power (4:20).

* **Associated Passages:** Acts 4:1-22

* **Associated ACAI Concepts:** Name (ID: `keyterm:Name`); Jesus (ID: `person:Jesus.2`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Salvation (ID: `keyterm:Salvation`); Resurrection (ID: `keyterm:Resurrection`); John (ID: `person:John.2`); Symbol (ID: `keyterm:Example`); Jerusalem (ID: `place:Jerusalem`); Annas (ID: `person:Annas`); Sadducees (ID: `group:Sadducee`); Caiaphas (ID: `person:Caiaphas`); John (ID: `person:John.3`); Alexander (ID: `person:Alexander.2`); Discern (ID: `keyterm:Discern`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Israel (ID: `person:Israel`); Serve (ID: `keyterm:Serve`); Sky (ID: `keyterm:Heaven`); Believe (ID: `keyterm:Believe`); Holy Spirit (ID: `deity:HolySpirit`); Nazareth (ID: `place:Nazareth`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Decided (ID: `keyterm:Decided`); Cornerstone (ID: `realia:Cornerstone`); Temple (ID: `realia:Temple`); Cross (ID: `realia:Cross`)

## Acts 4:4 (id: 9612)

*and the number of men grew to about five thousand*: See study note on Matthew 14:21.

* **Associated Passages:** Acts 4:4
* **Associated Articles:** Matthew 14:21 (ID: `21771@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`)

## Acts 4:13 (id: 9602)

*they were unschooled, ordinary men*: This means they were not scholars or trained teachers of the Scriptures. Peter and John were working men without higher education. 

The members of the Jewish high council were educated, and people saw them as authorities on the Scriptures and religious matters. Because of this, the council members were amazed to hear untrained men speak so boldly about these topics.

* **Associated Passages:** Acts 4:13

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Jesus (ID: `person:Jesus.2`); John (ID: `person:John.2`)

## Acts 4:16–18 (id: 9603)

They acknowledged the miracle but still tried to prevent the message. Jesus had experienced similar resistance (Matthew 19:8; Mark 10:5; see also Deuteronomy 10:16; 30:6; Jeremiah 4:4).

* **Associated Passages:** Acts 4:16-18

* **Associated ACAI Concepts:** Name (ID: `keyterm:Name`); Jesus (ID: `person:Jesus.2`); Symbol (ID: `keyterm:Example`); Jerusalem (ID: `place:Jerusalem`)

## Acts 4:19–20 (id: 9604)

The apostles shared a principle that can guide God’s people in dealing with hostile authorities (see also 5:40; compare 1 Samuel 15:22; Jeremiah 7:23; Luke 20:20–26; Romans 13:1–7). Jesus had predicted that his disciples would encounter hostility and persecution (Luke 21:12–19).

* **Associated Passages:** Acts 4:19-20

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); John (ID: `person:John.2`); Decided (ID: `keyterm:Decided`)

## Acts 4:23–31 (id: 9606)

The believers heard the report that their leaders had been threatened and commanded never again to speak about Jesus (4:18). The Christian movement was clearly under attack from the religious authorities, so they united in prayer. They turned to God, sharing their troubles and asking for courage to speak the message. They also asked God to send signs and wonders to confirm that the message was true. God answered their request in a clear and powerful way.

* **Associated Passages:** Acts 4:23-31

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Gentiles (ID: `group:Gentile`); Holy Spirit (ID: `deity:HolySpirit`); Jesus (ID: `person:Jesus.2`); Holy (ID: `keyterm:Holy`); Pilate (ID: `person:Pilate`); Ask for (ID: `keyterm:AskFor`); Herod Antipas (ID: `person:Herod.2`); Truth (ID: `keyterm:Truth`); Empty (ID: `keyterm:Empty`); Israel (ID: `person:Israel`); Name (ID: `keyterm:Name`); Anointed (ID: `keyterm:Anointed`); Symbol (ID: `keyterm:Example`); Sky (ID: `keyterm:Heaven`); David (ID: `person:David`); Portent (ID: `keyterm:Portent`)

## Acts 4:25–26 (id: 9607)

This passage quotes Psalm 2:1–2, which describes how the Gentile (non\-Jewish) nations react to the Lord’s anointed king. Similarly, the Jews and Gentiles in Jerusalem had gathered together against Jesus and now against his followers.

* **Associated Passages:** Acts 4:25-26

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Holy Spirit (ID: `deity:HolySpirit`); LORD (ID: `deity:Lord`); Empty (ID: `keyterm:Empty`); David (ID: `person:David`)

## Acts 4:26 (id: 9608)

The Hebrew term *Messiah* (which means "anointed one") originally referred to Israel’s anointed king. Later, it referred to the Messiah, God's chosen future king of Israel who would establish God's kingdom forever (2 Samuel 7:14–16; Psalm 89:36–37; Isaiah 9:6–7; 11:1–9; 61:1; Jeremiah 23:5–6; 33:14–16; Luke 1:32–33).

* **Associated Passages:** Acts 4:26

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Acts 4:28 (id: 9609)

God had been guiding events to fulfill his purpose. The Old Testament also shows God's guidance in his people's lives (see Ezra 7:6, 9, 28; 8:18, 31; Nehemiah 2:8, 18; Ecclesiastes 9:1; Ezekiel 8:1; Romans 8:28; 1 Peter 5:6).

* **Associated Passages:** Acts 4:28


## Acts 4:32–35 (id: 9610)

The sharing among the early believers was remarkable (see also 2:44–45; compare 6:1). They were united in heart and mind (they thought alike and shared the same purpose). They freely shared what they owned to meet the needs of other believers. This sharing was voluntary and not forced. It responded to real needs in the community and was motivated by Christian love and care for one another.

* **Associated Passages:** Acts 4:32-35

* **Associated ACAI Concepts:** Bear Witness (ID: `keyterm:BearWitness`); Favor (ID: `keyterm:Favor`); Ability (ID: `keyterm:Ability`); Heart (ID: `keyterm:Heart`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Resurrection (ID: `keyterm:Resurrection`); In Common (ID: `keyterm:InCommon`); House (ID: `realia:House`)

## Acts 4:36–37 (id: 9611)

Barnabas is presented as an example of generous and unselfish giving in response to real needs in the Christian community.

* **Associated Passages:** Acts 4:36-37

* **Associated ACAI Concepts:** Barnabas (ID: `person:Barnabas`); Cypriots (ID: `group:Cyprus`); Levi (ID: `person:Levi.3`); Ask for (ID: `keyterm:AskFor`); Tribe of Levi (ID: `group:Levi`); Cyprus (ID: `place:Cyprus`); Money (ID: `realia:Money`)

## Acts 5:1–11 (id: 9613)

Luke likes to balance a positive example with a negative one. He just shared the remarkable and positive story of Barnabas (4:36–37). Now, Luke shows the opposite behavior of Ananias and Sapphira. This couple desired to earn status in the community and for others to admire them. Ananias did not have to sell his property or give away the money. Their sin was pretending and dishonesty.

* **Associated Passages:** Acts 5:1-11

* **Associated ACAI Concepts:** Ananias (ID: `person:Ananias`); Peter (ID: `person:Peter`); Holy Spirit (ID: `deity:HolySpirit`); Fear (ID: `keyterm:Fear`); LORD (ID: `deity:Lord`); Heart (ID: `keyterm:Heart`); Sapphira (ID: `person:Sapphira`); Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`); Satan (ID: `deity:Satan`); Name (ID: `keyterm:Name`); Church (ID: `keyterm:Church`); Door (ID: `realia:Door`)

## Acts 5:3–4 (id: 9620)

By lying about the sale of their land, Ananias was not just being dishonest towards others but also toward the Holy Spirit. This means he was lying to God. His actions harmed the honesty, unity, and integrity of the church at its foundation. As a result, he experienced God's direct judgment (compare Leviticus 10:1–5; Joshua 7:16–26).

* **Associated Passages:** Acts 5:3-4

* **Associated ACAI Concepts:** Heart (ID: `keyterm:Heart`); Ananias (ID: `person:Ananias`); Holy Spirit (ID: `deity:HolySpirit`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Satan (ID: `deity:Satan`); Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`)

## Acts 5:11 (id: 9614)

*great fear came over the whole church and all who heard about these events*: The sudden judgment on Ananias and Sapphira had a serious impact on both the Christian community and those who learned about it (see Hebrews 10:31). Christians were strongly reminded to remain pure. 

For non\-Christians, the integrity of Christians was restored. So once again "the people regarded them highly" (Acts 5:13). However, it also made those outside the church cautious about joining a group with such high standards (5:13). This divine "pruning" allowed for new growth to occur (see John 15:1–11; Acts 5:14; see also 6:5–7; 11:24; 21:20).

* **Associated Passages:** Acts 5:11

* **Associated ACAI Concepts:** Fear (ID: `keyterm:Fear`); Church (ID: `keyterm:Church`)

## Acts 5:12–16 (id: 9615)

The preaching of the apostles included many miraculous signs and wonders, similar to the ministry of Jesus. There were also powerful acts of healing and casting out demons (see also 6:8; compare Matthew 4:24; 9:35; Mark 1:32–34; Luke 4:40–41).

* **Associated Passages:** Acts 5:12-16

* **Associated ACAI Concepts:** Portent (ID: `keyterm:Portent`); Serve (ID: `keyterm:Serve`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Believe (ID: `keyterm:Believe`); Jerusalem (ID: `place:Jerusalem`); Symbol (ID: `keyterm:Example`); Porch (ID: `realia:Porch`); Bed (ID: `realia:Bed`)

## Acts 5:13 (id: 9616)

* *Although the people regarded them highly*: People respected the believers because their way of life was different.
* *no one else dared to join them*: The people may have been afraid because of the high level of commitment expected of believers. They may also have feared what happened to Ananias and Sapphira (5:11).

* **Associated Passages:** Acts 5:13


## Acts 5:17–40 (id: 9617)

The apostles' successful ministry caused strong opposition (compare 4:1–3). The wealthy and powerful Sadducees controlled the temple and had a temple police force. They opposed belief in the resurrection and were determined to challenge and, if possible, prevent the apostles from sharing their message about the resurrection of Jesus. They felt their influence over the Jewish people weakening. Jealousy and bitterness led them to attack the apostles.

* **Associated Passages:** Acts 5:17-40

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Prison (ID: `realia:Prison`); Temple (ID: `realia:Temple`); Persuade (ID: `keyterm:Persuade`); Heart (ID: `keyterm:Heart`); Israel (ID: `person:Israel`); Name (ID: `keyterm:Name`); Jesus (ID: `person:Jesus.2`); Word (ID: `keyterm:Word`); Door (ID: `realia:Door`); Gamaliel (ID: `person:Gamaliel`); Witness (ID: `keyterm:Witness`); Judas (ID: `person:Judas.3`); Theudas (ID: `person:Theudas`); Resentment (ID: `keyterm:Resentment`); Savior (ID: `keyterm:Savior`); Peter (ID: `person:Peter`); Resurrection (ID: `keyterm:Resurrection`); Pharisees (ID: `group:Pharisee`); A Group of Disciples (ID: `group:GenericDisciples`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); Sadducees (ID: `group:Sadducee`); Forgive (ID: `keyterm:Forgive`); Life (ID: `keyterm:Life`); Census (ID: `keyterm:Census`); Fear (ID: `keyterm:Fear`); Jerusalem (ID: `place:Jerusalem`); A Person (ID: `person:GenericPerson`); Repent (ID: `keyterm:Repent`); Holy Spirit (ID: `deity:HolySpirit`); Israelites (ID: `group:Israel`); Galilee (ID: `place:Galilee`); Blood (ID: `keyterm:Blood`); Opposing God (ID: `keyterm:OpposingGod`); Galileans (ID: `group:Galilee`); Cross (ID: `realia:Cross`)

## Acts 5:19–20 (id: 9618)

The irony is that the Sadducees denied the existence of angels (23:8). However, angels appear many times in the book of Acts (see also 10:3, 7, 22; 11:13; 12:6–11, 23; 27:23–24). Luke shows God guiding his people in their ministry, using angels to carry out his purpose.

* **Associated Passages:** Acts 5:19-20

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Word (ID: `keyterm:Word`); Life (ID: `keyterm:Life`); An Angel (ID: `deity:Angel`); Temple (ID: `realia:Temple`); Prison (ID: `realia:Prison`); Door (ID: `realia:Door`)

## Acts 5:29 (id: 9619)

See study note on verses 4:19–20.

* **Associated Passages:** Acts 5:29
* **Associated Articles:** Acts 4:19-20 (ID: `9604@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); A Group of Disciples (ID: `group:GenericDisciples`)

## Acts 5:30–32 (id: 9621)

*We are witnesses of these things*: As witnesses, the apostles had the responsibility to speak about what they had seen and heard (see Leviticus 5:1; compare Acts 1:8; Matthew 28:18–20; Luke 24:44–49; John 20:21).

* **Associated Passages:** Acts 5:30-32

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Witness (ID: `keyterm:Witness`); Savior (ID: `keyterm:Savior`); Holy Spirit (ID: `deity:HolySpirit`); Repent (ID: `keyterm:Repent`); Forgive (ID: `keyterm:Forgive`); Jesus (ID: `person:Jesus.2`); Resurrection (ID: `keyterm:Resurrection`); Israel (ID: `person:Israel`); Word (ID: `keyterm:Word`); To Sin (ID: `keyterm:Sin`); Cross (ID: `realia:Cross`)

## Acts 5:33–40 (id: 9622)

God once again stopped the plans of the Jewish leaders to silence the apostles. This time, he did so through the advice of Gamaliel.

* **Associated Passages:** Acts 5:33-40

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Heart (ID: `keyterm:Heart`); Name (ID: `keyterm:Name`); Gamaliel (ID: `person:Gamaliel`); Theudas (ID: `person:Theudas`); Judas (ID: `person:Judas.3`); Census (ID: `keyterm:Census`); Israelites (ID: `group:Israel`); Galilee (ID: `place:Galilee`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Pharisees (ID: `group:Pharisee`); Opposing God (ID: `keyterm:OpposingGod`); Israel (ID: `person:Israel`); Galileans (ID: `group:Galilee`)

## Acts 5:34 (id: 9623)

Gamaliel was a respected Pharisee and a leading member of the Jewish high council. He was also a well\-known teacher of the law. He later taught Paul during his training to become a Jewish rabbi (see 22:3).

* **Associated Passages:** Acts 5:34

* **Associated ACAI Concepts:** Gamaliel (ID: `person:Gamaliel`); Name (ID: `keyterm:Name`); Pharisees (ID: `group:Pharisee`)

## Acts 5:36–37 (id: 9624)

* This Theudas is otherwise unknown, although the Jewish historian Josephus mentions a different man with the same name who led a revolution some years later (see Josephus, *Antiquities* 20\.5\.1\). Theudas was a common Jewish name.
* Judas of Galilee appears in Josephus as a rebel who opposed paying taxes to Caesar around AD 6 or 7, following the census of Quirinius (see Josephus, *Antiquities* 20\.5\.2; compare Luke 2:2). His resistance was stopped by the Roman authorities, who used force to end the revolt

* **Associated Passages:** Acts 5:36-37

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Heart (ID: `keyterm:Heart`); Theudas (ID: `person:Theudas`); Judas (ID: `person:Judas.3`); Census (ID: `keyterm:Census`); Galilee (ID: `place:Galilee`); Galileans (ID: `group:Galilee`)

## Acts 6:1 (id: 9626)

* *the Grecian Jews*: The Greek\-speaking Christians were Jewish Christians from other parts of the Greek and Roman world. They likely followed some Greek customs. The Hebrew\-speaking Christians were more traditional Jews from Palestine. They spoke Hebrew or Aramaic and avoided Greek customs.
* *their widows were being overlooked*: This injustice threatened the church's growth and development. Overlooking needs from one group caused frustration and required an intentional response to correct the behavior.

* **Associated Passages:** Acts 6:1

* **Associated ACAI Concepts:** Hellenists (ID: `group:Hellenist`); Greece (ID: `place:Greece`); Serve (ID: `keyterm:Serve`); Hebrews (ID: `group:Hebrews`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 6:2–6 (id: 9627)

* The apostles showed wisdom by calling a meeting of all the Christians to resolve the dispute. They understood that the problem was very serious. They also knew their priority was teaching God's word. They decided to choose men who were wise and filled with the Holy Spirit to manage the food distribution.

    The community agreed with the plan the apostles proposed. They selected capable leaders from the Christians who spoke Greek (all of their names are Greek). These seven men focused on this special task, allowing the apostles to pray and teach.

* These seven men are sometimes called the first deacons because their role is similar to that of the office of deacon (see 1 Timothy 3:8–13). Although the Greek term translated “deacon” (Greek *diakonos*, “servant”) is not used here, a related Greek word is translated "to wait on tables" (6:2; Greek *diakonein*, “to serve”).

* **Associated Passages:** Acts 6:2-6

* **Associated ACAI Concepts:** Serve (ID: `keyterm:Serve`); Philip (ID: `person:Philip.4`); Pray (ID: `keyterm:Pray`); Holy Spirit (ID: `deity:HolySpirit`); Antiochians (ID: `group:Antioch`); Nicanor (ID: `person:Nicanor`); Parmenas (ID: `person:Parmenas`); Stephen (ID: `person:Stephen`); Prochorus (ID: `person:Prochorus`); Nicolaus (ID: `person:Nicolaus`); Timon (ID: `person:Timon`); A Group of Disciples (ID: `group:GenericDisciples`); Spirit (ID: `keyterm:Spirit.2`); Be a Disciple (ID: `keyterm:Disciple`); Antioch (ID: `place:Antioch`); LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Bear Witness (ID: `keyterm:BearWitness`); Chosen (ID: `keyterm:Chosen`)

## Acts 6:7 (id: 9628)

* The apostles were wise in how they dealt with the issue. This led to an increase in the number of Christians, including many of the Jewish priests.
* his is the first of three places in Acts where Luke notes that God’s message continued to spread (see also 12:24; 19:20). When serious difficulties arose, God’s message overcame internal conflict, idolatry, and pagan practices. As a result, the community continued to grow. The book of Acts tells the story of how the good news about Jesus spread without being stopped (see 28:31).

* **Associated Passages:** Acts 6:7

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Be a Disciple (ID: `keyterm:Disciple`); Jerusalem (ID: `place:Jerusalem`)

## Acts 6:8–15 (id: 9629)

The Jewish leaders did to Stephen as they had done to Jesus (compare Matthew 26:3–4, 59–66):

* They found someone to lie about him,
* they accused him of blasphemy (saying offensive things about God)*,*
* they started a riot,
* they arrested him, and
* they made false charges against him.

* **Associated Passages:** Acts 6:8-15

* **Associated ACAI Concepts:** Stephen (ID: `person:Stephen`); Moses (ID: `person:Moses`); Word (ID: `keyterm:Word`); Alexandrians (ID: `group:Alexandria`); Freedmen (ID: `group:Freedmen`); Alexandria (ID: `place:Alexandria`); Favor (ID: `keyterm:Favor`); Witness (ID: `keyterm:Witness`); Symbol (ID: `keyterm:Example`); An Angel (ID: `deity:Angel`); Cilicia (ID: `place:Cilicia`); Holy (ID: `keyterm:Holy`); Spirit (ID: `keyterm:Spirit.2`); Asia (ID: `place:Asia`); Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Cyrene (ID: `place:Cyrene`); Portent (ID: `keyterm:Portent`); Blasphemous (ID: `keyterm:Blasphemous`); Holy Spirit (ID: `deity:HolySpirit`); Nazareth (ID: `place:Nazareth`); Ability (ID: `keyterm:Ability`); Tabernacle (ID: `realia:Tabernacle`)

## Acts 6:15 (id: 9625)

*his face was like the face of an angel*: This is similar to when the face of Moses "was radiant" after speaking with the Lord (6:14; see Exodus 34:32–35). Stephen, like Moses, was bringing God’s instruction to Israel (see also 2 Corinthians 3:7–18).

* **Associated Passages:** Acts 6:15

* **Associated ACAI Concepts:** An Angel (ID: `deity:Angel`)

## Acts 7:1–53 (id: 9630)

* Stephen answered the accusations by speaking about his Lord (see Luke 21:12–17). Instead of defending himself, he became a witness in God's legal case against them. This revealed how contrary they were and their lack of loyalty. Stephen gave a history of Israel's past and reminded them of their repeated rejections of those who announced God's messages.
* Stephen's review of Israel's history has three main parts:

    1. The work of the patriarchs. This refers to Abraham and those descended from him who founded the nation of Israel (Acts 7:2–16).
        2. The ministry of Moses (7:17–43).
        3. The role of the tabernacle and the temple (7:44–50).

    Stephen ended his historical survey with a strong criticism of his people's lack of obedience. Like a prophet, he challenged them to stop resisting the Holy Spirit. He told them to turn to God with grief for their sins (repentance) and faith.

* **Associated Passages:** Acts 7:1-53

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Egypt (ID: `place:Egypt`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Joseph (ID: `person:Joseph.10`); An Angel (ID: `deity:Angel`); Abraham (ID: `person:Abraham`); To Act Wickedly (ID: `keyterm:ActWickedly`); Heart (ID: `keyterm:Heart`); Egyptians (ID: `group:Egypt`); Rescue (ID: `keyterm:Rescue`); Pharaoh (ID: `person:Pharaoh.4`); Favor (ID: `keyterm:Favor`); Priesthood (ID: `keyterm:Priesthood`); Haran (ID: `place:Haran`); Offering (ID: `keyterm:Offering`); Isaac (ID: `person:Isaac`); Serve (ID: `keyterm:Serve`); Sky (ID: `keyterm:Heaven`); Mount Sinai (ID: `place:SinaiMount`); Judge (ID: `keyterm:Judge`); Promise (ID: `keyterm:Promise`); Hamor (ID: `person:Hamor`); Mesopotamia (ID: `place:Mesopotamia`); Rephan (ID: `deity:Rephan`); Shechem (ID: `place:Shechem`); Gentiles (ID: `group:Gentile`); Bow Down (ID: `keyterm:BowDown`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Red Sea (ID: `place:RedSea`)

## Acts 7:8 (id: 9642)

*the covenant of circumcision*: See Genesis 17:9–14. See also “Circumcision in the New Testament” Theme Note.

* **Associated Passages:** Acts 7:8
* **Associated Articles:** Circumcision in the New Testament (ID: `500387@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Circumcision (ID: `keyterm:Circumcision`); Isaac (ID: `person:Isaac`); Israel (ID: `person:Israel`); Covenant (ID: `keyterm:Covenant`)

## Acts 7:14 (id: 9631)

*seventy\-five*: Compare this number with the seventy mentioned in the Hebrew text of Genesis 46:27. The number seventy\-five probably comes from the Septuagint, the ancient Greek translation of the Old Testament. The Septuagint includes three people descended from Ephraim and two of Manasseh in46:20. This adds five more people. The Hebrew text does not include those descended from Joseph’s sons. New Testament authors mostly quote from the Septuagint, which sometimes differs from the Hebrew Masoretic Text.

* **Associated Passages:** Acts 7:14

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Joseph (ID: `person:Joseph.10`)

## Acts 7:16 (id: 9632)

*a price*: This was 400 pieces of silver, according to Genesis 23:16.

* **Associated Passages:** Acts 7:16

* **Associated ACAI Concepts:** Hamor (ID: `person:Hamor`); Abraham (ID: `person:Abraham`); Shechem (ID: `place:Shechem`); Grave (ID: `realia:Grave`)

## Acts 7:37 (id: 9633)

*a prophet like me:* This verse quotes Deuteronomy 18:15. Moses prophesied that the Messiah would come. It happened as Moses said.

* **Associated Passages:** Acts 7:37

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Israelites (ID: `group:Israel`); Moses (ID: `person:Moses`); Israel (ID: `person:Israel`)

## Acts 7:39–40 (id: 9634)

The ancient Jews rejected the law Moses gave to them for their salvation. Stephen was speaking to the people of his time who were physical and spiritual descendants from those Jews. His point was that they were still rejecting God's messengers. In this case, it was Jesus, the ultimate savior.

* **Associated Passages:** Acts 7:39-40

* **Associated ACAI Concepts:** Egypt (ID: `place:Egypt`); Heart (ID: `keyterm:Heart`); Moses (ID: `person:Moses`); Aaron (ID: `person:Aaron`)

## Acts 7:42–43 (id: 9635)

This passage quotes Amos 5:25–27 (Greek version) to show how the people of Israel rebelled against Moses (Acts 7:39–41). 

Molech was a Canaanite god to whom human sacrifices were offered (Leviticus 18:21; 20:2–5; 1 Kings 11:7; 2 Kings 23:10; Jeremiah 32:35). 

Rephan*,* a star god identified with the planet Saturn, was worshiped by the Israelites during their wilderness wanderings.

* **Associated Passages:** Acts 7:42-43

* **Associated ACAI Concepts:** Rephan (ID: `deity:Rephan`); Serve (ID: `keyterm:Serve`); Babylon (ID: `place:Babylon`); Molech (ID: `deity:Molech`); Bow Down (ID: `keyterm:BowDown`); LORD (ID: `deity:Lord`); Israel (ID: `person:Israel`); Priesthood (ID: `keyterm:Priesthood`); Offering (ID: `keyterm:Offering`); Sky (ID: `keyterm:Heaven`); Tent (ID: `realia:Tent`); Image (ID: `realia:Image`)

## Acts 7:48–50 (id: 9636)

One charge against Stephen was that he spoke against the temple (see 6:14). The leading priests and scribes managed the temple's business and wanted to protect their interests (see Luke 19:45–48). This is why these leaders were concerned about the temple, even though God said that the Most High does not live in temples made by human hands.

* **Associated Passages:** Acts 7:48-50

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Sky (ID: `keyterm:Heaven`); Temple (ID: `realia:Temple`); House (ID: `realia:House`); Footstool (ID: `realia:Footstool`); Throne (ID: `realia:Throne`)

## Acts 7:51 (id: 9637)

Stephen accused the people of being stiff\-necked with uncircumcised hearts and ears.This means they were strong\-willed and ignored God's truth. This was exactly how God had accused them of behaving during the time of wilderness wandering. Here, Stephen links it to rejecting the gospel and resisting the Holy Spirit (see Exodus 32:9; 33:3; 34:9; Deuteronomy 9:6, 13; 31:27; see also Psalm 78:8; Zechariah 7:11–12).

* **Associated Passages:** Acts 7:51

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Heart (ID: `keyterm:Heart`)

## Acts 7:55–56 (id: 9638)

* *standing at the right hand of God*: Usually, the Bible describes Jesus as seated in heaven at God’s right side (2:33–34; 5:31; Luke 20:42; 22:69; Romans 8:34; Ephesians 1:20; Colossians 3:1; Hebrews 1:3, 13; 10:12). One possible explanation has been proposed by some church fathers that Jesus was welcoming Stephen to heaven with honor as the first New Testament martyr. Stephen was loyal in confessing his Lord on earth. Now his Lord honored his promise to confess his loyal servant in heaven. Jesus was standing as a witness to defend him (Matthew 10:32; Luke 12:8).
* *Son of Man standing*: Compare Daniel 7:13–14. The Jewish leaders understood that Stephen was speaking of Jesus as the divine Son of Man (Acts 7:57). This title speaks of Jesus’s power and authority (compare Revelation 1:12–15).

* **Associated Passages:** Acts 7:55-56

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Sky (ID: `keyterm:Heaven`); Holy Spirit (ID: `deity:HolySpirit`); Glory (ID: `keyterm:Glory`); Jesus (ID: `person:Jesus.2`)

## Acts 7:57 (id: 9639)

They covered their ears because they thought comparing Jesus to the divine Son of Man was a terrible offense against God (7:56).

* **Associated Passages:** Acts 7:57


## Acts 7:58 (id: 9640)

*Saul* was his name in Hebrew, and *Paul* was his Greek name (see 13:9).

* **Associated Passages:** Acts 7:58

* **Associated ACAI Concepts:** Witness (ID: `keyterm:Witness`); Paul (ID: `person:Paul`); Cloak (ID: `realia:Cloak`)

## Acts 7:59–60 (id: 9641)

Stephen’s prayer, "Lord, do not hold this sin against them," is very similar to the prayer Jesus prayed when he was on the cross (Luke 23:34). Jesus taught his followers the importance of forgiveness and prayer (Matthew 6:14–15; Mark 11:25; see Luke 11:4; 17:3–4; Luke 11:5–10; 18:1–8; see also Acts 1:12–15; 4:23–31; 12:5; James 5:16–18). The Lord gave a positive answer to Stephen’s prayer as it relates to Saul (Acts 9:1–43).

* **Associated Passages:** Acts 7:59-60

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Stephen (ID: `person:Stephen`); Call Upon (ID: `keyterm:CallUpon`); Spirit (ID: `keyterm:Spirit.2`); To Sin (ID: `keyterm:Sin`)

## Acts 8:1–4 (id: 9643)

*Saul was there*: Compare 9:1–2; 22:4; 26:9–11; Galatians 1:13–14. This major attack against the church caused all believers except the apostles to flee. Instead of losing enthusiasm, they preached the good news about Jesus wherever they went.

* **Associated Passages:** Acts 8:1-4

* **Associated ACAI Concepts:** Church (ID: `keyterm:Church`); Paul (ID: `person:Paul`); Stephen (ID: `person:Stephen`); Judea (ID: `place:Judea`); Jerusalem (ID: `place:Jerusalem`); Samaria (ID: `place:Samaria`); Good News (ID: `keyterm:GoodNews`); Prison (ID: `realia:Prison`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 8:7 (id: 9651)

*Came out … healed*: Miracles often happen when the gospel is preached in the book of Acts (see study note on 3:1–11).

* **Associated Passages:** Acts 8:7
* **Associated Articles:** Acts 3:1-11 (ID: `9590@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Serve (ID: `keyterm:Serve`)

## Acts 8:9–24 (id: 9652)

Simon the sorcerer performed magic and amazed the people of Samaria. He boasted that he was someone great. Simon tried to buy spiritual power. The later corrupt practice of buying and selling positions of leadership in the church was called *simony*. This term refers to the corrupt practice of buying and selling church leadership positions (see 1 Timothy 6:9–10). Peter strongly criticized Simon's evil intentions and actions.

* **Associated Passages:** Acts 8:9-24

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`); Simon (ID: `person:Simon.8`); Name (ID: `keyterm:Name`); Baptism (ID: `keyterm:Baptism`); Ability (ID: `keyterm:Ability`); Philip (ID: `person:Philip.4`); Heart (ID: `keyterm:Heart`); Peter (ID: `person:Peter`); Samaria (ID: `place:Samaria`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Ask for (ID: `keyterm:AskFor`); Money (ID: `realia:Money`); Kingdom (ID: `keyterm:Kingdom`); Straightness (ID: `keyterm:Straightness`); John (ID: `person:John.2`); Symbol (ID: `keyterm:Example`); Pray (ID: `keyterm:Pray`); Simon (ID: `person:Simon.9`); Forgive (ID: `keyterm:Forgive`); Jerusalem (ID: `place:Jerusalem`); Repent (ID: `keyterm:Repent`); Authority (ID: `keyterm:Authority.2`); Unjust Deed (ID: `keyterm:UnjustDeed`); Good News (ID: `keyterm:GoodNews`)

## Acts 8:14–17 (id: 9644)

When the apostles sent two of their own representatives to Samaria, it was an important step in overcoming prejudice (compare Matthew 10:5; Luke 9:52–54; John 4:9; 8:48). It was even more remarkable that Peter and John prayed for the Samaritans who trusted in Christ to receive the Holy Spirit. This event is similar to the day of Pentecost in Jerusalem and marks how the Holy Spirit's power spread from Judea to Samaria (Acts 2:1–47; 1:8).

* **Associated Passages:** Acts 8:14-17

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Pray (ID: `keyterm:Pray`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Samaria (ID: `place:Samaria`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Baptism (ID: `keyterm:Baptism`); John (ID: `person:John.2`)

## Acts 8:24 (id: 9645)

Simon recognized that he needed Peter to pray for him. However, the text does not clearly say that Simon repented or turned away from his evil actions. His request seems focused on avoiding the severe punishment Peter warned about, rather than on a true change in how he thought and acted.

* **Associated Passages:** Acts 8:24

* **Associated ACAI Concepts:** Simon (ID: `person:Simon.8`); LORD (ID: `deity:Lord`); Ask for (ID: `keyterm:AskFor`)

## Acts 8:26–40 (id: 9646)

Philip obeyed the Holy Spirit and followed the Spirit’s direction. He then took the opportunity to share the message of the gospel with an Ethiopian eunuch (a trusted male official who served a king or queen) on the desert road from Jerusalem to Gaza. This event shows how effective Philip was in explaining and sharing the Christian message. It also shows his strong commitment to bringing the good news to all people, no matter their social position or ethnic background.

* **Associated Passages:** Acts 8:26-40

* **Associated ACAI Concepts:** Philip (ID: `person:Philip.4`); Public Official (ID: `person:GenericOfficial`); Chariot (ID: `realia:Chariot`); Good News (ID: `keyterm:GoodNews`); Written (ID: `keyterm:Scripture`); LORD (ID: `deity:Lord`); Jerusalem (ID: `place:Jerusalem`); Baptism (ID: `keyterm:Baptism`); Holy Spirit (ID: `deity:HolySpirit`); Ask for (ID: `keyterm:AskFor`); Isaiah (ID: `person:Isaiah`); Ethiopians (ID: `group:Ethiopia`); Gaza (ID: `place:Gaza`); Ethiopia (ID: `place:Ethiopia`); Ashdod (ID: `place:Ashdod`); Bow Down (ID: `keyterm:BowDown`); An Angel (ID: `deity:Angel`); Life (ID: `keyterm:Life`); Caesarea (ID: `place:Caesarea`); Jesus (ID: `person:Jesus.2`); Sheep (ID: `fauna:Lamb`)

## Acts 8:27 (id: 9647)

* In the ancient world, a eunuch was a royal official. Some eunuchs had been physically altered and could not have children, while others held the title because of their role in the royal court (see 2 Kings 9:30–32; Esther 1:10; 2:3, 14–15, 21; 4:4–5).

    Many Jews looked down on eunuchs. Eunuchs could not have children, so they could not pass on the covenant family line (God’s people through descendants). The law of Moses also excluded men with damaged genitals from the assembly of Israel, which was the gathered worshiping community (Deuteronomy 23:1; compare Leviticus 21:17–23).

    However, the prophet Isaiah spoke of a future time when God would welcome non\-Jewish people and eunuchs who were faithful to him (Isaiah 56:3–8; see also Matthew 19:12). In the new covenant, all people who have genuine faith belong to the people of God.

* The eunuch traveled from Africa to Jerusalem to worship in the temple, probably for a major Jewish feast.

* **Associated Passages:** Acts 8:27

* **Associated ACAI Concepts:** Ethiopians (ID: `group:Ethiopia`); Ethiopia (ID: `place:Ethiopia`); Bow Down (ID: `keyterm:BowDown`); Jerusalem (ID: `place:Jerusalem`)

## Acts 8:29 (id: 9648)

The Holy Spirit guides the servants of God in where, when, and what to preach, teach, or do (Acts 9:15; 10:19–20; 11:12; 16:6; 1 Corinthians 2:13; 1 Peter 1:12).

* **Associated Passages:** Acts 8:29

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Philip (ID: `person:Philip.4`); Chariot (ID: `realia:Chariot`)

## Acts 8:32–33 (id: 9649)

The Scripture passage was Isaiah 53:7–8. It is one of the Servant Songs of Isaiah, a passage that speaks of the suffering servant of the Lord.

* **Associated Passages:** Acts 8:32-33

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`); Life (ID: `keyterm:Life`); Sheep (ID: `fauna:Lamb`)

## Acts 8:39–40 (id: 9650)

* After the Spirit of the Lord carried Philip away, he traveled north along the coast from Azotus. Azotus is another name for Ashdod (see 1 Samuel 5:1–7; Nehemiah 13:23–24; Isaiah 20:1). He preached in every town until he reached Caesarea, where he later settled (see Acts 21:8).
* *Caesarea:* Caesarea Maritima was a major port on the Mediterranean Sea. It was the center of Roman government in Judea. Herod the Great built it around 22 to 9 BC and named it to honor Caesar Augustus, the Roman emperor.

* **Associated Passages:** Acts 8:39-40

* **Associated ACAI Concepts:** Philip (ID: `person:Philip.4`); Ashdod (ID: `place:Ashdod`); Holy Spirit (ID: `deity:HolySpirit`); LORD (ID: `deity:Lord`); Good News (ID: `keyterm:GoodNews`); Caesarea (ID: `place:Caesarea`)

## Acts 9:1–19 (id: 9653)

The conversion of Saul of Tarsus on the Damascus road is central to the story in Acts. Luke tells the story three times (also 22:1–21; 26:1–29). Paul (Saul) mentions this experience several times in his letters (1 Corinthians 15:8–10; Galatians 1:11–17; Philippians 3:4–11; see 1 Timothy 1:12–17). Saul’s conversion was his call as a prophet and commission as an apostle (Acts 9:15; 22:15, 21; 26:15–18). No one is beyond God's power to reach, save, and use for holy purposes. Nothing is impossible with God (Luke 1:37). God prepared Paul through his training, childhood, and experience to play a unique role in proclaiming the gospel to the wider world as the “apostle to the Gentiles” (Romans 11:13; see 1 Corinthians 15:9; 2 Corinthians 12:11–12; Galatians 1:1; Ephesians 3:8).

* **Associated Passages:** Acts 9:1-19

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Damascus (ID: `place:Damascus`); Ananias (ID: `person:Ananias.2`); Be a Disciple (ID: `keyterm:Disciple`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); The Way (ID: `keyterm:TheWay`); Straight Street (ID: `place:Straight`); Judas (ID: `person:Judas.4`); Tarsians (ID: `group:Tarsus`); Gentiles (ID: `group:Gentile`); Straightness (ID: `keyterm:Straightness`); Israel (ID: `person:Israel`); Pray (ID: `keyterm:Pray`); Holy (ID: `keyterm:Holy`); Sky (ID: `keyterm:Heaven`); Tarsus (ID: `place:Tarsus`); Baptism (ID: `keyterm:Baptism`); Holy Spirit (ID: `deity:HolySpirit`); Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`); Call Upon (ID: `keyterm:CallUpon`); Chosen (ID: `keyterm:Chosen`)

## Acts 9:2 (id: 9662)

* A synagogue (Greek *sunagōgē*, meaning “gathering place”) was a local Jewish meeting place. After the exile to Babylon, Jewish people began to meet in synagogues in their towns. These places became centers for teaching and worship. After Israel returned from exile in Babylon, Jews began using synagogues for instruction and worship.

    In the synagogue, people read from the Law and the Prophets. Leaders explained the meaning of the Scriptures. The people also prayed and gave praise and thanks to God (see 13:15; 15:21; Nehemiah 9:5; Matthew 6:5; Luke 4:16–21). Jesus attended, taught, preached, and performed miracles in synagogues (Matthew 12:9–10; Mark 1:21, 39; Luke 4:16; 13:10–13; John 6:59; 18:20). His apostles did as well (see Acts 9:20; 13:5, 14; 14:1; 17:1, 10, 17; 18:4, 19, 26).

* *Damascus:* Damascus, the capital of Syria, was an important center with a long history. It was the nearest major city outside of Palestine. It took four to six days to travel from Jerusalem to Damascus. This emphasizes Saul’s determination (9:1–5; see 22:4–8; 26:9–15).
* *the Way*: The term *the Way* in the book of Acts refers to Christianity (see 19:9, 23; 24:14, 22). It is “the way of God” that teaches people “the way of salvation” (18:26; 16:17). See also John 14:6; 2 Peter 2:2.

* **Associated Passages:** Acts 9:2

* **Associated ACAI Concepts:** The Way (ID: `keyterm:TheWay`); Damascus (ID: `place:Damascus`); Jerusalem (ID: `place:Jerusalem`)

## Acts 9:10 (id: 9654)

* *Ananias*: See also 22:12. Ananias was a common name (consider the husband of Sapphira, 5:1, and the Jewish high priest \[AD 47–59], 23:2).
* *a vision*: See “Visions” Theme Note.

* **Associated Passages:** Acts 9:10
* **Associated Articles:** Visions (ID: `500385@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Ananias (ID: `person:Ananias.2`); Damascus (ID: `place:Damascus`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 9:15 (id: 9655)

*This man is My chosen instrument to carry My name before the Gentiles*: In God’s plan to spread the good news, the next step was to reach the Gentiles, that is, people from other nations (see 1:8; compare chapters 10–11). 

Saul of Tarsus, also called Paul, was the one God chose for this task. God sent him to bring the message about Jesus to Gentiles, to kings, and also to the people of Israel.

The rest of the book of Acts shows how Paul faithfully carried out this mission from God (for example, 26:19–23).

* **Associated Passages:** Acts 9:15

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Israel (ID: `person:Israel`); Chosen (ID: `keyterm:Chosen`)

## Acts 9:16 (id: 9656)

As Jesus predicted, Paul suffered greatly for his faith (see 2 Corinthians 11:23–27; see Acts 13:45; 14:19; 16:22–27; 21:30–31; 26:21; 2 Timothy 1:11–12).

* **Associated Passages:** Acts 9:16

* **Associated ACAI Concepts:** Name (ID: `keyterm:Name`)

## Acts 9:17 (id: 9657)

God gave Ananias a special role. He was to welcome Saul into the Christian community.

Ananias began by placing his hands on Saul. Through this act, Saul was healed and filled with the Holy Spirit.

* **Associated Passages:** Acts 9:17

* **Associated ACAI Concepts:** Ananias (ID: `person:Ananias.2`); Holy Spirit (ID: `deity:HolySpirit`); Jesus (ID: `person:Jesus.2`); Paul (ID: `person:Paul`); House (ID: `realia:House`)

## Acts 9:20–21 (id: 9658)

*Saul promptly began to proclaim Jesus*: Saul's encounter with the heavenly Christ was genuine, shown by his enthusiastic and bold preaching. His direct declaration surprised many because he had previously caused great harm to the followers of Jesus in Jerusalem.

* **Associated Passages:** Acts 9:20-21

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Proclaim (ID: `keyterm:Proclaim`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Call Upon (ID: `keyterm:CallUpon`); Synagogue (ID: `realia:Synagogue`)

## Acts 9:22–25 (id: 9659)

The people who heard Saul were confused and surprised. Even so, Saul clearly showed from the Scriptures that Jesus is the Messiah. His arguments were so strong that the Jews in Damascus who did not believe could not answer him.

This continued for some time (see Galatians 1:18). The people who heard Saul were confused and surprised. Even so, Saul clearly showed from the Scriptures that Jesus is the Messiah. His arguments were so strong that the Jews in Damascus who did not believe could not answer him.

This continued for some time in 2 Corinthians 11:32–33.

* **Associated Passages:** Acts 9:22-25

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Damascus (ID: `place:Damascus`); Jesus (ID: `person:Jesus.2`); Be a Disciple (ID: `keyterm:Disciple`); Basket (ID: `realia:Basket`); Temple (ID: `realia:Temple`); City Wall (ID: `realia:CityWall`)

## Acts 9:26–28 (id: 9660)

*When Saul arrived in Jerusalem:* The believers did not trust him at first. This was understandable. He had recently attacked and imprisoned Christians (8:3; 9:1, 13; 22:3, 4; 26:9–11). 

Barnabas helped him. Barnabas, whose name means “Son of Encouragement” (4:36), brought Saul to the apostles. He told them how Saul had seen the Lord on the road to Damascus. He also explained how Saul had preached boldly in the name of Jesus there.

Barnabas convinced the apostles that Saul’s conversion was real. So Saul stayed with them in Jerusalem. There, he continued to preach boldly about the Lord.

Paul later spoke about this visit in Galatians 1:18–19.

* **Associated Passages:** Acts 9:26-28

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Fear (ID: `keyterm:Fear`); Barnabas (ID: `person:Barnabas`); Believe (ID: `keyterm:Believe`); Be a Disciple (ID: `keyterm:Disciple`); Damascus (ID: `place:Damascus`)

## Acts 9:29 (id: 9661)

Once again, Saul debated with some Greek\-speaking Jews. He spoke boldly and explained why Jesus is the Messiah.

Again, some people became angry and made a plan to kill him (see 9:22–24).

* **Associated Passages:** Acts 9:29

* **Associated ACAI Concepts:** Hellenists (ID: `group:Hellenist`); Greece (ID: `place:Greece`)

## Acts 9:30 (id: 9663)

After the Christians heard about the plot, they sent Saul away again (see 9:25). This time he went to Tarsus, his hometown, the capital of Cilicia (see 21:39; 22:3; 23:34; compare Galatians 1:21).

* **Associated Passages:** Acts 9:30

* **Associated ACAI Concepts:** Tarsus (ID: `place:Tarsus`); Caesarea (ID: `place:Caesarea`)

## Acts 9:31 (id: 9664)

* This verse is one of several in Acts that describe how the church grew in numbers and in faithfulness (see also 2:41; 4:4; 5:14; 6:1, 7; 12:24; 21:20).
* The first Christians were learning an important truth: “The fear of the LORD is the beginning of wisdom.” This means that true knowledge starts with honoring and respecting God (Proverbs 1:7; 9:10; compare Job 28:28; Psalm 111:10; Ecclesiastes 12:13; see Luke 7:16).

    As they learned to honor God, they grew stronger in their faith. Their reverence for the Lord helped the church grow (compare Acts 2:43; 19:17).

* **Associated Passages:** Acts 9:31

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Fear (ID: `keyterm:Fear`); Galilee (ID: `place:Galilee`); Samaria (ID: `place:Samaria`); Jesus (ID: `person:Jesus.2`); Church (ID: `keyterm:Church`); Judea (ID: `place:Judea`); Ask for (ID: `keyterm:AskFor`); Make Peace (ID: `keyterm:Peace`)

## Acts 9:32–43 (id: 9665)

* These verses describe Peter’s traveling ministry in Judea, especially in towns along the seacoast. As he traveled, God gave him power to do miracles. Peter healed a man named Aeneas and raised a woman named Dorcas from the dead.

    Jesus had promised that his disciples would do such works (John 14:12).

* Luke often tells stories in pairs. For example, he tells of a man being healed and then a woman being healed (see Luke 13:10–17; 14:1–6). In this passage, Peter heals both a man and a woman.

    The people in that area were deeply affected by these miracles. Many people turned to the Lord and believed (Acts 9:35, 42).

* **Associated Passages:** Acts 9:32-43

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Joppa (ID: `place:Joppa`); Tabitha (ID: `person:Tabitha`); Jesus (ID: `person:Jesus.2`); Lod (ID: `place:Lod`); Aeneas (ID: `person:Aeneas`); Holy (ID: `keyterm:Holy`); Be a Disciple (ID: `keyterm:Disciple`); Name (ID: `keyterm:Name`); Upstairs Room (ID: `realia:UpstairsRoom`); Sharon Plain (ID: `place:Sharon.2`); Simon (ID: `person:Simon.9`); Pray (ID: `keyterm:Pray`); Body (ID: `keyterm:Body.3`); Life (ID: `keyterm:Life`); Believe (ID: `keyterm:Believe`); Good (ID: `keyterm:Good`); Ask for (ID: `keyterm:AskFor`); Leather (ID: `realia:Leather`); Bed (ID: `realia:Bed`); Cloak (ID: `realia:Cloak`); Tunic (ID: `realia:Tunic.2`)

## Acts 9:43 (id: 9666)

*stayed for several days in Joppa with a tanner named Simon*: A tanner worked with animal skins to make leather. This job required contact with dead animals.

Many Jews considered this kind of work unclean. Because Peter stayed in the home of a tanner, it may show that he was not strict about some Jewish traditions (see Galatians 2:11–14).

* **Associated Passages:** Acts 9:43

* **Associated ACAI Concepts:** Simon (ID: `person:Simon.9`); Joppa (ID: `place:Joppa`); Leather (ID: `realia:Leather`)

## Acts 10:1–8 (id: 9292)

* *a centurion*: This was the highest rank for an officer in the Roman army who was not commissioned. He led a *century,* a group of about 100 men. Luke often describes centurions positively (10:22; 21:32; 22:25–26; 23:17, 23; 27:6, 11, 43; 28:16; Luke 7:1–10; 23:47).

    Luke wanted to show that Christianity was not against Roman officials or institutions. It could, like Judaism, be permitted in the Roman Empire (see Acts Book Introduction, “Purposes of Acts: Politics”).

* *in what was called the Italian Regiment*: A regiment included six centuries; a Roman legion was usually divided into ten regiments. The New Testament mentions the Italian Regiment and the Imperial Regiment (Acts 27:1).

* **Associated Passages:** Acts 10:1-8

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); An Angel (ID: `deity:Angel`); Cornelius (ID: `person:Cornelius`); Joppa (ID: `place:Joppa`); Simon (ID: `person:Simon.9`); Italians (ID: `group:Italy`); Pray (ID: `keyterm:Pray`); Italy (ID: `place:Italy`); Fear (ID: `keyterm:Fear`); Peter (ID: `person:Peter`); Name (ID: `keyterm:Name`); Ask for (ID: `keyterm:AskFor`); Caesarea (ID: `place:Caesarea`); House (ID: `realia:House`); Leather (ID: `realia:Leather`)

## Acts 10:2 (id: 9296)

*devout and God\-fearing*: Gentiles described as God\-fearing admired Judaism's high ethical standards. However, they were not ready to undergo circumcision or fully convert to Judaism (see 13:43). 

Christianity appealed to these Gentiles who worshiped God (18:7; see 13:48; 16:30; 17:4, 12, 17). These individuals were open to the message of the gospel that announced that “there is peace with God through Jesus Christ” (10:36).

* **Associated Passages:** Acts 10:2

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Ask for (ID: `keyterm:AskFor`); Fear (ID: `keyterm:Fear`)

## Acts 10:3 (id: 9299)

In Acts, visions often happen while people are praying (9:3–6, 10–16; 10:2–6, 9–12; 11:5–9; 12:9–17; 18:9–10; 22:17–21; 23:11). These visions are not random. They show God's saving work and give divine guidance and encouragement.

* **Associated Passages:** Acts 10:3

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Cornelius (ID: `person:Cornelius`); An Angel (ID: `deity:Angel`)

## Acts 10:9–16 (id: 9303)

God gave Peter the same vision three times to confirm it was true (see Genesis 41:32; 2 Corinthians 13:1).

* **Associated Passages:** Acts 10:9-16

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Sky (ID: `keyterm:Heaven`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`); Pray (ID: `keyterm:Pray`); To Cleanse (ID: `keyterm:Cleanse.2`); Housetop (ID: `realia:Housetop`); Linen Cloth (ID: `realia:LinenCloth`); Creeping Animal (ID: `fauna:CreepingAnimal`); Snake (ID: `fauna:Snake`); Chameleon (ID: `fauna:Chameleon`); Monitor (ID: `fauna:Iguana`); Lizard (ID: `fauna:Lizard`)

## Acts 10:14 (id: 9293)

See Leviticus 11 for a list of animals that Jewish laws have declared impure and ceremonially unclean.

* **Associated Passages:** Acts 10:14

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Defilement (ID: `keyterm:Defilement`)

## Acts 10:17 (id: 9294)

*Peter was puzzling over*: The events that followed would clarify the meaning of the vision (10:17–48; see 11:1–18). Peter should not hesitate to enter or eat in a non\-Jewish (Gentile) home because God has accepted and cleansed them.

* **Associated Passages:** Acts 10:17

* **Associated ACAI Concepts:** Simon (ID: `person:Simon.9`); Peter (ID: `person:Peter`); Cornelius (ID: `person:Cornelius`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 10:25–26 (id: 9295)

*Cornelius … fell at his feet to worship him*: This act was probably more than just bowing to an important person of rank. The Bible says worship is for God alone (see 14:11–17; Exodus 20:3; Deuteronomy 5:7; Matthew 22:37–38; Mark 12:29–30; Luke 10:27; 1 Corinthians 10:14; Colossians 3:5; 1 Peter 4:3; 1 John 5:21; Revelation 4:10; 9:20; 22:8–9). 

Peter was just a fellow human being whom Cornelius should not worship.

* **Associated Passages:** Acts 10:25-26

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Cornelius (ID: `person:Cornelius`); Bow Down (ID: `keyterm:BowDown`)

## Acts 10:34–35 (id: 9297)

* *God does not show favoritism*: See Deuteronomy 10:17; 2 Chronicles 19:7; Job 34:19; Luke 20:21; Romans 2:11; Galatians 2:6; Colossians 3:25; 1 Peter 1:17.

    The application of this principle is the meaning of Peter’s vision (10:9–16).

* *welcomes those from every nation who fear Him and do what is right*: See Romans 10:11–13.

* **Associated Passages:** Acts 10:34-35

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Fear (ID: `keyterm:Fear`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## Acts 10:36–43 (id: 9298)

Peter often emphasizes the role of the apostles as witnesses to the good news (the message about Jesus’s life, death, and resurrection). He reminds his listeners that they had direct contact with Jesus.

The apostles ate and drank with Jesus after he rose from the dead (see Luke 24:41–43). They saw him alive again. Because they were eyewitnesses of his resurrection, they could testify that Jesus had defeated death (see Acts 3:15; 4:33; 13:30–31). 

Jesus chose the original apostles ahead of time to be his witnesses (see 1:12–26). Later, others continued this work. For example, Paul and Barnabas preached and taught in the name of Jesus Christ (see 9:15; 14:1–3; 26:16).

* **Associated Passages:** Acts 10:36-43

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Proclaim (ID: `keyterm:Proclaim`); Witness (ID: `keyterm:Witness`); Jesus (ID: `person:Jesus.2`); Anointed (ID: `keyterm:Anointed`); Declare (ID: `keyterm:Declare`); Bear Witness (ID: `keyterm:BearWitness`); Resurrection (ID: `keyterm:Resurrection`); Israel (ID: `person:Israel`); Jews (ID: `group:Jews`); Make Peace (ID: `keyterm:Peace`); To Sin (ID: `keyterm:Sin`); John (ID: `person:John`); Forgive (ID: `keyterm:Forgive`); Life (ID: `keyterm:Life`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`); Judea (ID: `place:Judea`); Baptism (ID: `keyterm:Baptism`); Holy Spirit (ID: `deity:HolySpirit`); Nazareth (ID: `place:Nazareth`); Galilee (ID: `place:Galilee`); Ability (ID: `keyterm:Ability`); Word (ID: `keyterm:Word`); Good News (ID: `keyterm:GoodNews`); Cross (ID: `realia:Cross`)

## Acts 10:43 (id: 9300)

* Based on what they saw and heard, the apostles could declare that Jesus of Nazareth was the one about whom all the prophets spoke (10:39–42). The whole message and purpose of Scripture center on Christ (see Luke 24:25–27, 44–47; John 5:39).
* The main point of their message was that everyone who believes in Jesus will have sins forgiven through his name (see Luke 24:47).

* **Associated Passages:** Acts 10:43

* **Associated ACAI Concepts:** Bear Witness (ID: `keyterm:BearWitness`); Forgive (ID: `keyterm:Forgive`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); To Sin (ID: `keyterm:Sin`)

## Acts 10:44–48 (id: 9301)

On the day of Pentecost, Peter told the people that if they stopped sinning, repented, asked forgiveness from God, and received baptism in the name of Jesus Christ, they would "receive the gift of the Holy Spirit" (2:38; see also 19:1–7). 

As Cornelius and his household listened to Peter's message, they received the Holy Spirit just like the Jews had earlier. This event confirmed again that God did not favor one group over another (10:34–35). This event is similar to the day of Pentecost in Jerusalem (2:1–47). It shows that the power of the Holy Spirit now came to the Gentiles as well (1:8; 2:39).

* **Associated Passages:** Acts 10:44-48

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Peter (ID: `person:Peter`); Baptism (ID: `keyterm:Baptism`); Gentiles (ID: `group:Gentile`); Circumcision (ID: `keyterm:Circumcision`); LORD (ID: `deity:Lord`); Faithfulness (ID: `keyterm:Faithfulness`); Name (ID: `keyterm:Name`); Word (ID: `keyterm:Word`); Ask for (ID: `keyterm:AskFor`)

## Acts 10:48 (id: 9302)

Cornelius asked Peter to stay for a few days with them, possibly because he needed more instruction in the Christian faith.

* **Associated Passages:** Acts 10:48

* **Associated ACAI Concepts:** Ask for (ID: `keyterm:AskFor`); Baptism (ID: `keyterm:Baptism`); Name (ID: `keyterm:Name`)

## Acts 11:1–18 (id: 9304)

Jews traditionally kept themselves separate and did not eat or interact in a social setting with Gentiles (10:28; 22:21–22; see John 4:9, 27; 18:28; Galatians 2:12–14). The Jewish Christians in Jerusalem learned that Gentiles had received the word of God. Then they criticized Peter's unusual actions and asked for an explanation, which Peter provided.

* **Associated Passages:** Acts 11:1-18

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Sky (ID: `keyterm:Heaven`); Holy Spirit (ID: `deity:HolySpirit`); House (ID: `realia:House`); Gentiles (ID: `group:Gentile`); Discern (ID: `keyterm:Discern`); Defilement (ID: `keyterm:Defilement`); Jesus (ID: `person:Jesus.2`); Word (ID: `keyterm:Word`); Joppa (ID: `place:Joppa`); Temple (ID: `realia:Temple`); An Angel (ID: `deity:Angel`); Pray (ID: `keyterm:Pray`); John (ID: `person:John`); Salvation (ID: `keyterm:Salvation`); Life (ID: `keyterm:Life`); Caesarea (ID: `place:Caesarea`); Believe (ID: `keyterm:Believe`); Jerusalem (ID: `place:Jerusalem`); Judea (ID: `place:Judea`); Baptism (ID: `keyterm:Baptism`); To Cleanse (ID: `keyterm:Cleanse.2`); Repent (ID: `keyterm:Repent`); Circumcision (ID: `keyterm:Circumcision`); Glory (ID: `keyterm:Glory`); Linen Cloth (ID: `realia:LinenCloth`); Chameleon (ID: `fauna:Chameleon`); Creeping Animal (ID: `fauna:CreepingAnimal`); Snake (ID: `fauna:Snake`); Lizard (ID: `fauna:Lizard`); Monitor (ID: `fauna:Iguana`)

## Acts 11:4–17 (id: 9315)

Peter reviewed what had happened. He explained that God had directed the whole series of events (11:12). Peter ate with Gentiles (non\-Jews) because God made it evident that he should (11:4–12). Peter then saw the Holy Spirit come upon the Gentiles, realizing God accepted and blessed them like Jews who trusted in Jesus (11:15–17; see 1:5). Peter followed God's will by admitting Gentiles to the church.

* **Associated Passages:** Acts 11:4-17

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`); House (ID: `realia:House`); Defilement (ID: `keyterm:Defilement`); Jesus (ID: `person:Jesus.2`); Word (ID: `keyterm:Word`); Joppa (ID: `place:Joppa`); Temple (ID: `realia:Temple`); Discern (ID: `keyterm:Discern`); An Angel (ID: `deity:Angel`); Pray (ID: `keyterm:Pray`); John (ID: `person:John`); Salvation (ID: `keyterm:Salvation`); Caesarea (ID: `place:Caesarea`); Believe (ID: `keyterm:Believe`); Baptism (ID: `keyterm:Baptism`); To Cleanse (ID: `keyterm:Cleanse.2`); Linen Cloth (ID: `realia:LinenCloth`); Chameleon (ID: `fauna:Chameleon`); Creeping Animal (ID: `fauna:CreepingAnimal`); Snake (ID: `fauna:Snake`); Lizard (ID: `fauna:Lizard`); Monitor (ID: `fauna:Iguana`)

## Acts 11:18 (id: 9305)

Peter gave a clear and careful explanation. Those who had objected were convinced. They saw that God was at work in the Gentiles' believing in Jesus and in their receiving eternal life.

However, the question of including Gentiles was not finished. Soon it would lead to a serious crisis in the church (15:1–35; see also Paul's letter to the Galatians).

* **Associated Passages:** Acts 11:18

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Repent (ID: `keyterm:Repent`); Glory (ID: `keyterm:Glory`); LORD (ID: `deity:Lord`); Life (ID: `keyterm:Life`)

## Acts 11:19–26 (id: 9306)

* After Stephen's death, persecution forced Christians to move to other areas (8:1–3). They traveled as far as Phoenicia, Cyprus, and Antioch.
* Antioch in Syria was a busy, diverse city. It was the third\-largest city in the Roman Empire, after Rome and Alexandria. Antioch played a key role in spreading the Christian message to non\-Jewish people.

* **Associated Passages:** Acts 11:19-26

* **Associated ACAI Concepts:** Antioch (ID: `place:Antioch`); LORD (ID: `deity:Lord`); Cyprus (ID: `place:Cyprus`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Church (ID: `keyterm:Church`); Hellenists (ID: `group:Hellenist`); Be a Disciple (ID: `keyterm:Disciple`); Phoenicia (ID: `place:Phoenicia`); Christians (ID: `group:Christ`); Cypriots (ID: `group:Cyprus`); Favor (ID: `keyterm:Favor`); Barnabas (ID: `person:Barnabas`); Heart (ID: `keyterm:Heart`); Jews (ID: `group:Jews`); Greece (ID: `place:Greece`); Paul (ID: `person:Paul`); Tarsus (ID: `place:Tarsus`); Stephen (ID: `person:Stephen`); Jerusalem (ID: `place:Jerusalem`); Cyrene (ID: `place:Cyrene`); Good (ID: `keyterm:Good`); Holy Spirit (ID: `deity:HolySpirit`); Ask for (ID: `keyterm:AskFor`); Good News (ID: `keyterm:GoodNews`)

## Acts 11:20 (id: 9307)

Jewish Christians from Cyprus and Cyrene made the first organized effort to teach Gentiles about Jesus.

* **Associated Passages:** Acts 11:20

* **Associated ACAI Concepts:** Hellenists (ID: `group:Hellenist`); Cypriots (ID: `group:Cyprus`); Greece (ID: `place:Greece`); Antioch (ID: `place:Antioch`); Jesus (ID: `person:Jesus.2`); Cyrene (ID: `place:Cyrene`); Good News (ID: `keyterm:GoodNews`); Cyprus (ID: `place:Cyprus`)

## Acts 11:21–24 (id: 9308)

As with the household of Cornelius, many Gentiles believed in Jesus. The growth of Christian faith among Gentiles had to remain in harmony with the church at Jerusalem. So, the Jerusalem Christians sent Barnabas to Antioch to oversee the developments there. Barnabas could see that God’s blessing was on what was happening, so he supported it with joy.

* **Associated Passages:** Acts 11:21-24

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Holy Spirit (ID: `deity:HolySpirit`); Antioch (ID: `place:Antioch`); Favor (ID: `keyterm:Favor`); Barnabas (ID: `person:Barnabas`); Heart (ID: `keyterm:Heart`); Jerusalem (ID: `place:Jerusalem`); Church (ID: `keyterm:Church`); Ask for (ID: `keyterm:AskFor`); Good (ID: `keyterm:Good`)

## Acts 11:25–26 (id: 9309)

Barnabas saw that Saul had a special gift for preaching and teaching. The wise judgment of Barnabus led to a successful ministry in Antioch.

* **Associated Passages:** Acts 11:25-26

* **Associated ACAI Concepts:** Christians (ID: `group:Christ`); Be a Disciple (ID: `keyterm:Disciple`); Tarsus (ID: `place:Tarsus`); Antioch (ID: `place:Antioch`); Jesus (ID: `person:Jesus.2`); Church (ID: `keyterm:Church`); Paul (ID: `person:Paul`)

## Acts 11:26 (id: 9310)

The term *Christians* might have first been an insult. The Greek text uses this term in only two other New Testament passages (26:28; 1 Peter 4:16).

* **Associated Passages:** Acts 11:26

* **Associated ACAI Concepts:** Christians (ID: `group:Christ`); Be a Disciple (ID: `keyterm:Disciple`); Antioch (ID: `place:Antioch`); Jesus (ID: `person:Jesus.2`); Church (ID: `keyterm:Church`)

## Acts 11:27–28 (id: 9311)

* *some prophets*: See “The Gift of Prophecy” Theme Note.
* *Agabus*: See also 21:10–12.

* **Associated Passages:** Acts 11:27-28
* **Associated Articles:** The Gift of Prophecy (ID: `500390@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Claudius (ID: `person:Claudius`); Agabus (ID: `person:Agabus`); Holy Spirit (ID: `deity:HolySpirit`); Antioch (ID: `place:Antioch`); Ability (ID: `keyterm:Ability`); Name (ID: `keyterm:Name`); Jerusalem (ID: `place:Jerusalem`)

## Acts 11:28 (id: 9312)

Claudius was the nephew of Tiberius Caesar (Luke 3:1). He ruled as Roman emperor from AD 41 to AD 54\.

His last wife was his niece Agrippina. Claudius adopted her son, Nero.

* **Associated Passages:** Acts 11:28

* **Associated ACAI Concepts:** Claudius (ID: `person:Claudius`); Agabus (ID: `person:Agabus`); Holy Spirit (ID: `deity:HolySpirit`); Ability (ID: `keyterm:Ability`); Name (ID: `keyterm:Name`)

## Acts 11:29–30 (id: 9313)

Agabus's prophecy resulted in the Christians in Antioch helping the Jewish Christians in Judea. They gave as much as they could and trusted the funds to Barnabas and Saul. This act of love and support showed the difference Jesus had made in their lives. Saul (also called Paul) mentions this visit in his letter to the Galatians (Galatians 2:1–10).

* **Associated Passages:** Acts 11:29-30

* **Associated ACAI Concepts:** Serve (ID: `keyterm:Serve`); Barnabas (ID: `person:Barnabas`); Judea (ID: `place:Judea`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`)

## Acts 11:30 (id: 9314)

This is the first mention in the book of Acts of elders as officers in the Christian church (see also 14:23; 15:2–23; 16:4; 20:17–35; 21:18; compare 1 Timothy 3:1–7; Titus 1:5–9).

* **Associated Passages:** Acts 11:30

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Barnabas (ID: `person:Barnabas`)

## Acts 12:1–5 (id: 9316)

* Jesus had prophesied that his followers would experience persecution and difficulty (Luke 11:49–51). For the first time since his death, Roman authorities acted violently against the church. James, the brother of John, was among the first disciples Jesus called (Mark 1:16–20; Luke 5:1–11). He was one of the first Christians killed because of his faith.
* King Herod Agrippa took action to intentionally harm the church (Acts 12:20–23). It is possible that doing this was meant to strengthen his political alignment with the Jewish people. The letter of James, the brother of Jesus, was probably written after this persecution effort to the Christians who were scattered (see 8:1–4; James Book Introduction, “Date of Writing”).

* **Associated Passages:** Acts 12:1-5

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Church (ID: `keyterm:Church`); Prison (ID: `realia:Prison`); James (ID: `person:James`); Herod (ID: `person:Herod.3`); John (ID: `person:John.2`); Pray (ID: `keyterm:Pray`); LORD (ID: `deity:Lord`); Passover (ID: `keyterm:Passover`); Jews (ID: `group:Jews`)

## Acts 12:4 (id: 9321)

Herod had Peter guarded by four squads of soldiers, making it impossible for the apostle to escape (compare 12:6). God was in control, even though the circumstances seemed impossible for humans to overcome (compare 4:27–31; Genesis 18:14; Jeremiah 32:17, 27; Matthew 19:26; Mark 10:27; Luke 1:37; 18:27).

* **Associated Passages:** Acts 12:4

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`); Prison (ID: `realia:Prison`)

## Acts 12:5 (id: 9322)

*the church was fervently praying*: God answers the earnest prayers of his people (12:6–17; see Luke 11:1–13; 18:1–8; compare Matthew 7:7–11; John 15:7; Philippians 4:6–7; James 5:16; 1 John 3:22).

* **Associated Passages:** Acts 12:5

* **Associated ACAI Concepts:** Pray (ID: `keyterm:Pray`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Church (ID: `keyterm:Church`); Prison (ID: `realia:Prison`)

## Acts 12:6–19 (id: 9323)

God, through an angel, guided Peter out of prison. Peter reunited with his praying friends, and God sent him to continue proclaiming the good news about Jesus. The message advanced by God's power, even when the church and apostles faced strong opposition and discouragement.

* **Associated Passages:** Acts 12:6-19

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); An Angel (ID: `deity:Angel`); Herod (ID: `person:Herod.3`); LORD (ID: `deity:Lord`); True (ID: `keyterm:True`); Chain (ID: `realia:Chain`); Prison (ID: `realia:Prison`); Temple (ID: `realia:Temple`); Door (ID: `realia:Door`); Rhoda (ID: `person:Rhoda`); Mary (ID: `person:Mary.5`); Mark (ID: `person:Mark`); Discern (ID: `keyterm:Discern`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Pray (ID: `keyterm:Pray`); Rescue (ID: `keyterm:Rescue`); Caesarea (ID: `place:Caesarea`); Name (ID: `keyterm:Name`); Judea (ID: `place:Judea`); James (ID: `person:James.3`); Cause of Joy (ID: `keyterm:CauseOfJoy`); Entrance (ID: `realia:Entrance`); House (ID: `realia:House`); Cloak (ID: `realia:Cloak`); Sandal (ID: `realia:Sandal`)

## Acts 12:7–11 (id: 9324)

See “Angels” Theme Note.

* **Associated Passages:** Acts 12:7-11
* **Associated Articles:** Angels (ID: `500391@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); True (ID: `keyterm:True`); Herod (ID: `person:Herod.3`); Rescue (ID: `keyterm:Rescue`); Jews (ID: `group:Jews`); Temple (ID: `realia:Temple`); Chain (ID: `realia:Chain`); Sandal (ID: `realia:Sandal`); Cloak (ID: `realia:Cloak`)

## Acts 12:12 (id: 9317)

Mary, the mother of John Mark, had a home where Christians often gathered. John Mark later worked with Barnabas and Saul to spread the gospel (12:25).

* **Associated Passages:** Acts 12:12

* **Associated ACAI Concepts:** Mary (ID: `person:Mary.5`); Mark (ID: `person:Mark`); Pray (ID: `keyterm:Pray`); House (ID: `realia:House`)

## Acts 12:13–17 (id: 9318)

Rhoda was very surprised when Peter appeared, so she left him standing at the closed door. She and the other Christians thought God's answer to their prayers was amazing (12:5).

* **Associated Passages:** Acts 12:13-17

* **Associated ACAI Concepts:** Peter (ID: `person:Peter`); Rhoda (ID: `person:Rhoda`); A Group of People (ID: `group:GenericGroup`); LORD (ID: `deity:Lord`); Cause of Joy (ID: `keyterm:CauseOfJoy`); Name (ID: `keyterm:Name`); James (ID: `person:James.3`); An Angel (ID: `deity:Angel`); Temple (ID: `realia:Temple`); Prison (ID: `realia:Prison`); Entrance (ID: `realia:Entrance`); Door (ID: `realia:Door`)

## Acts 12:18–23 (id: 9319)

When Peter was missing after a thorough search, Herod questioned the guards and executed them (see 16:27). However, Herod himself then experienced a painful death. God punished him for his pride when he accepted the people's worship. The ancient Jewish historian Josephus provides more details about the death of Herod Agrippa I (Josephus, *Antiquities* 19\.8\.1–2\).

* **Associated Passages:** Acts 12:18-23

* **Associated ACAI Concepts:** Herod (ID: `person:Herod.3`); Blastus (ID: `person:Blastus`); Tyrians (ID: `group:Tyre`); Discern (ID: `keyterm:Discern`); A Group of People (ID: `group:GenericGroup`); Peter (ID: `person:Peter`); Make Peace (ID: `keyterm:Peace`); Sidon (ID: `place:Sidon`); An Angel (ID: `deity:Angel`); Caesarea (ID: `place:Caesarea`); LORD (ID: `deity:Lord`); Sidonians (ID: `group:Sidon`); Judea (ID: `place:Judea`); Tyre (ID: `place:Tyre`); Persuade (ID: `keyterm:Persuade`); Glory (ID: `keyterm:Glory`); Bedroom (ID: `realia:Bedroom`); Worm (ID: `fauna:Maggot`); Judgment Seat (ID: `realia:JudgmentSeat`); Clothing (ID: `realia:Clothing`)

## Acts 12:24–25 (id: 9320)

Herod died from a terrible illness (12:23). Meanwhile, the Christian church continued to grow and share the message of the gospel freely (28:31).

* **Associated Passages:** Acts 12:24-25

* **Associated ACAI Concepts:** Mark (ID: `person:Mark`); Serve (ID: `keyterm:Serve`); Barnabas (ID: `person:Barnabas`); LORD (ID: `deity:Lord`); Jerusalem (ID: `place:Jerusalem`); Paul (ID: `person:Paul`)

## Acts 13:1 (id: 9333)

* *prophets and teachers*: See “The Gift of Prophecy” Theme Note; also see 1 Corinthians 12:28–29; Ephesians 4:11.
* The name Simeon suggests a Jewish background (see Genesis 29:33; Luke 2:25; 3:30). He is also called Niger (which means "the black mane"). This suggests that he was probably African.
* Lucius is a Latin name. He came from Cyrene, the capital of Libya in North Africa. He was probably one of the preachers from Cyrene who brought the Christian message to Antioch (Acts 11:20).
* Manaen grew up with King Herod Antipas. He was probably Luke’s source for understanding Antipas’s thoughts and actions (see Luke 9:7–9).
* Barnabas and Saul play important roles in the following story.

* **Associated Passages:** Acts 13:1
* **Associated Articles:** The Gift of Prophecy (ID: `500390@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Herod Antipas (ID: `person:Herod.2`); Manaen (ID: `person:Manaen`); Antioch (ID: `place:Antioch`); Simeon (ID: `person:Simeon.4`); Barnabas (ID: `person:Barnabas`); Church (ID: `keyterm:Church`); Cyrene (ID: `place:Cyrene`); Paul (ID: `person:Paul`)

## Acts 13:1–3 (id: 9325)

*prophets and teachers:* these leaders of the church at Antioch spent a lot of time in worship and prayer. They had an intense desire to do the Lord's will, and they fasted to spiritually prepare themselves to receive guidance from God. As they prayed, the Holy Spirit spoke to them. 

The church set apart Barnabas and Saul, recognizing God's clear call for them to do a special work in his name. The inner journey these Christians took to pray and listen to God matched their outer journey of service. Their service included proclaiming the Christian faith and performing works of healing and salvation.

* **Associated Passages:** Acts 13:1-3

* **Associated ACAI Concepts:** Barnabas (ID: `person:Barnabas`); Abstain from Food (ID: `keyterm:Fast`); Paul (ID: `person:Paul`); Herod Antipas (ID: `person:Herod.2`); Manaen (ID: `person:Manaen`); Antioch (ID: `place:Antioch`); Simeon (ID: `person:Simeon.4`); Pray (ID: `keyterm:Pray`); Holy Spirit (ID: `deity:HolySpirit`); LORD (ID: `deity:Lord`); Church (ID: `keyterm:Church`); Cyrene (ID: `place:Cyrene`); Priesthood (ID: `keyterm:Priesthood`)

## Acts 13:3 (id: 9340)

*they laid their hands on them*: They performed this holy act only after more fasting and prayer*.* The Pastoral Letters (1 Timothy, 2 Timothy, and Titus) caution against appointing someone as a Christian leader without proper care and background checks (1 Timothy 5:22). At this point, the church at Antioch sent Barnabas and Saul as its representatives for the mission.

* **Associated Passages:** Acts 13:3

* **Associated ACAI Concepts:** Pray (ID: `keyterm:Pray`); Abstain from Food (ID: `keyterm:Fast`)

## Acts 13:4 (id: 9345)

* Barnabas and Saul were commissioned by the church at Antioch to go on their first missionary journey. They were guided by the Holy Spirit.
* Seleucia was the ancient seaport city of Antioch, about 20 kilometers (12 miles) west of the city at the opening of the Orontes River.
* *from there to Cyprus:* Barnabas was from Cyprus, the next stop on their journey (4:36).

* **Associated Passages:** Acts 13:4

* **Associated ACAI Concepts:** Seleucia (ID: `place:Seleucia`); Cyprus (ID: `place:Cyprus`); Holy Spirit (ID: `deity:HolySpirit`)

## Acts 13:5 (id: 9348)

When they arrived in Salamis on the eastern end of Cyprus, they visited the Jewish synagogues (see study note on 9:2). There, Jews could hear and respond to the Christian message. They would also meet people who had converted to Judaism (those who had chosen to follow Israel’s God; see study note on 13:43). They also met Gentiles (non\-Jews) who were seeking God. These Gentiles are sometimes called *God\-fearers* (people who worshiped the God of Israel but had not fully become Jews; see study note on 10:2).

* **Associated Passages:** Acts 13:5
* **Associated Articles:** Acts 10:2 (ID: `9296@AquiferOpenStudyNotes`); Acts 13:43 (ID: `9343@AquiferOpenStudyNotes`); Acts 9:2 (ID: `9662@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Salamis (ID: `place:Salamis`); Mark (ID: `person:Mark`); LORD (ID: `deity:Lord`); Jews (ID: `group:Jews`); Synagogue (ID: `realia:Synagogue`)

## Acts 13:6 (id: 9350)

* *Paphos:* This major city was on the southwest coast of Cyprus, a location that had strategic significance.
* *a Jewish sorcerer and false prophet named Bar\-Jesus*: People from the east who practiced magic like this man often had significant power to influence people in the Greek and Roman world.

* **Associated Passages:** Acts 13:6

* **Associated ACAI Concepts:** Bar-jesus (ID: `person:Bar-jesus`); Paphos (ID: `place:Paphos`); Jews (ID: `group:Jews`); Name (ID: `keyterm:Name`)

## Acts 13:6–12 (id: 9349)

In Paphos, a false prophet caused a power struggle. This resulted in a display of God's power, and the Roman proconsul (governor) became a Christian.

* **Associated Passages:** Acts 13:6-12

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Bar-jesus (ID: `person:Bar-jesus`); Paul (ID: `person:Paul`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Sergius Paulus (ID: `person:SergiusPaulus`); Paphos (ID: `place:Paphos`); Holy Spirit (ID: `deity:HolySpirit`); The Way (ID: `keyterm:TheWay`); Barnabas (ID: `person:Barnabas`); Straightness (ID: `keyterm:Straightness`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Jews (ID: `group:Jews`)

## Acts 13:7–8 (id: 9351)

Sergius Paulus, the proconsul (governor), was an intelligent person. He had interest in the teachings of Barnabas and Saul. But Elymas (the sorcerer’s Greco\-Roman name) recognized a challenge to his power and strongly opposed the message of Barnabas and Saul.

* **Associated Passages:** Acts 13:7-8

* **Associated ACAI Concepts:** Sergius Paulus (ID: `person:SergiusPaulus`); Bar-jesus (ID: `person:Bar-jesus`); Barnabas (ID: `person:Barnabas`); LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Paul (ID: `person:Paul`)

## Acts 13:9 (id: 9352)

Luke marks an important shift in using the name Saul (a Hebrew name) and Paul (a Greco\-Roman name). This may show that Paul is now focusing on his mission to Gentiles (non\-Jews). For the rest of Acts, Luke calls him Paul, except when Paul tells the story of his conversion (as in 22:7, 13; 26:14).

* **Associated Passages:** Acts 13:9

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Holy Spirit (ID: `deity:HolySpirit`)

## Acts 13:10–11 (id: 9326)

Paul, who had become the main speaker, confronted the sorcerer. He rebuked his false claims, exposed his deceit, and announced God’s judgment (see 8:20–24). The sorcerer became blind immediately, and this condition lasted for a significant period of time. This event showed the power and truthfulness of the gospel message over the sorcerer's false claims.

* **Associated Passages:** Acts 13:10-11

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); The Way (ID: `keyterm:TheWay`); Straightness (ID: `keyterm:Straightness`)

## Acts 13:12 (id: 9327)

The teaching about the Lord included a miracle showing divine power (compare Mark 1:21–27). This was because it was a teaching about the living God (see Acts 14:15).

* **Associated Passages:** Acts 13:12

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`)

## Acts 13:13–14 (id: 9328)

* They arrived at the port of Perga. From there, major roads led to the interior of the land beyond the Taurus Mountains.
* At this point, John Mark left the team for unknown reasons (see study note on 15:36–41). He might have been unhappy that the gospel was advancing into non\-Jewish lands. Perhaps he missed his home or was unable to continue the challenging journey. Whatever the reasons, he returned to the familiar and comfortable Jewish area of Jerusalem.

* **Associated Passages:** Acts 13:13-14
* **Associated Articles:** Acts 15:36-41 (ID: `9375@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Perga (ID: `place:Perga`); Antioch (ID: `place:Antioch.2`); Pamphylia (ID: `place:Pamphylia`); Paphos (ID: `place:Paphos`); Synagogue (ID: `realia:Synagogue`); Mark (ID: `person:Mark`); Sabbath (ID: `keyterm:Sabbath`); Pisidia (ID: `place:Pisidia`); Jerusalem (ID: `place:Jerusalem`); Paul (ID: `person:Paul`)

## Acts 13:14 (id: 9329)

Paul and Barnabas traveled inland to the high country and reached Pisidian Antioch. This Antioch was in the Roman province of Galatia in Asia Minor. This city is not the same one as the city of Antioch in Syria. 

As usual, the apostles began their work at the synagogue (see study note on 9:2; compare 13:5; 14:1; 17:1, 2, 10, 17; 18:4, 19; 19:8).

* **Associated Passages:** Acts 13:14
* **Associated Articles:** Acts 9:2 (ID: `9662@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Antioch (ID: `place:Antioch.2`); Synagogue (ID: `realia:Synagogue`); Sabbath (ID: `keyterm:Sabbath`); Pisidia (ID: `place:Pisidia`); Perga (ID: `place:Perga`)

## Acts 13:15 (id: 9330)

People chosen to read in the synagogue read the usual Scriptures for the day. This included one passage from the books of Moses and another from the prophets (see Luke 4:16–28). Then the synagogue leaders invited the visitors to share a word of encouragement for the people.

* **Associated Passages:** Acts 13:15

* **Associated ACAI Concepts:** Law (ID: `keyterm:Law`); Ask for (ID: `keyterm:AskFor`); Synagogue (ID: `realia:Synagogue`)

## Acts 13:16–41 (id: 9331)

Paul accepted the invitation, and signaled for his audience to be quiet (compare 19:33; 21:40). He began to clearly proclaim the good news about Jesus. This is Paul's first major speech in the book of Acts. It serves as a model of the way he preached to a Jewish audience (see 22:1–21).

* **Associated Passages:** Acts 13:16-41

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Israel (ID: `person:Israel`); David (ID: `person:David`); Resurrection (ID: `keyterm:Resurrection`); John (ID: `person:John`); Holy (ID: `keyterm:Holy`); Fear (ID: `keyterm:Fear`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Promise (ID: `keyterm:Promise`); Jesse (ID: `person:Jesse`); Saul (ID: `person:Saul.2`); Kish (ID: `person:Kish`); Canaan (ID: `place:Canaan`); Heart (ID: `keyterm:Heart`); Benjamin (ID: `person:Benjamin`); Witness (ID: `keyterm:Witness`); Tribe of Benjamin (ID: `group:Benjamin`); To Sin (ID: `keyterm:Sin`); Samuel (ID: `person:Samuel`); Serve (ID: `keyterm:Serve`); Egypt (ID: `place:Egypt`); Forgive (ID: `keyterm:Forgive`); Sabbath (ID: `keyterm:Sabbath`); Abraham (ID: `person:Abraham`); Salvation (ID: `keyterm:Salvation`); Good News (ID: `keyterm:GoodNews`); Pilate (ID: `person:Pilate`); Paul (ID: `person:Paul`); Law (ID: `keyterm:Law`); To Cause to Happen (ID: `keyterm:Happen`); Baptism (ID: `keyterm:Baptism`); Moses (ID: `person:Moses`); Repent (ID: `keyterm:Repent`); Bear Witness (ID: `keyterm:BearWitness`); Israelites (ID: `group:Israel`); Galilee (ID: `place:Galilee`); Faithfulness (ID: `keyterm:Faithfulness`); Decided (ID: `keyterm:Decided`); Savior (ID: `keyterm:Savior`); Chosen (ID: `keyterm:Chosen`); Grave (ID: `realia:Grave`); Sandal (ID: `realia:Sandal`)

## Acts 13:17–22 (id: 9332)

To establish common ground with his audience, Paul traced Jewish history starting from the Israelite exodus from Egypt. He emphasized: 

* the Israelites' escape from Egyptian slavery,
* their settlement in Canaan,
* the establishment of the kingdom of Israel,
* the removal of King Saul, and
* the special role of King David.

* **Associated Passages:** Acts 13:17-22

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Jesse (ID: `person:Jesse`); Kish (ID: `person:Kish`); Saul (ID: `person:Saul.2`); Canaan (ID: `place:Canaan`); Egypt (ID: `place:Egypt`); Bear Witness (ID: `keyterm:BearWitness`); Heart (ID: `keyterm:Heart`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); Tribe of Benjamin (ID: `group:Benjamin`); David (ID: `person:David`); Chosen (ID: `keyterm:Chosen`); Samuel (ID: `person:Samuel`)

## Acts 13:22 (id: 9334)

Even though he sinned, David was a man who desired to completely follow God's ways (for example, 2 Samuel 11–12).

* **Associated Passages:** Acts 13:22

* **Associated ACAI Concepts:** Jesse (ID: `person:Jesse`); Bear Witness (ID: `keyterm:BearWitness`); David (ID: `person:David`); Heart (ID: `keyterm:Heart`)

## Acts 13:23–25 (id: 9335)

At this point, Paul shifted to the main point of his message. Jesus, descended from King David's family line, was the Savior God promised to Israel. John the Baptist prepared the way for the Messiah (God's chosen leader). John called Israel to repent and be baptized. John was a humble servant who prepared the way for the one whose coming he announced.

* **Associated Passages:** Acts 13:23-25

* **Associated ACAI Concepts:** John (ID: `person:John`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Promise (ID: `keyterm:Promise`); Baptism (ID: `keyterm:Baptism`); Savior (ID: `keyterm:Savior`); Sandal (ID: `realia:Sandal`)

## Acts 13:26–37 (id: 9336)

Paul reviewed how Jesus had been treated with shame, including an unjust sentence and death. But God raised Jesus from the dead, and many witnesses saw him. This message is good news, because through Jesus people can receive forgiveness of sins. But people must respond with faith, or they will face serious consequences.

* **Associated Passages:** Acts 13:26-37

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Resurrection (ID: `keyterm:Resurrection`); Holy (ID: `keyterm:Holy`); David (ID: `person:David`); Jerusalem (ID: `place:Jerusalem`); Witness (ID: `keyterm:Witness`); Serve (ID: `keyterm:Serve`); Sabbath (ID: `keyterm:Sabbath`); Abraham (ID: `person:Abraham`); Salvation (ID: `keyterm:Salvation`); Pilate (ID: `person:Pilate`); Fear (ID: `keyterm:Fear`); To Cause to Happen (ID: `keyterm:Happen`); Jesus (ID: `person:Jesus.2`); Galilee (ID: `place:Galilee`); Faithfulness (ID: `keyterm:Faithfulness`); Promise (ID: `keyterm:Promise`); Decided (ID: `keyterm:Decided`); Good News (ID: `keyterm:GoodNews`); Grave (ID: `realia:Grave`)

## Acts 13:31 (id: 9337)

The resurrection of Jesus was confirmed by many witnesses.

* **Associated Passages:** Acts 13:31

* **Associated ACAI Concepts:** Witness (ID: `keyterm:Witness`); Galilee (ID: `place:Galilee`); Jerusalem (ID: `place:Jerusalem`)

## Acts 13:38–41 (id: 9338)

Paul urged his audience to believe the message about Jesus, through whom there is forgiveness for your sins.

* **Associated Passages:** Acts 13:38-41

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`); Law (ID: `keyterm:Law`); Forgive (ID: `keyterm:Forgive`); To Sin (ID: `keyterm:Sin`); Moses (ID: `person:Moses`)

## Acts 13:39 (id: 9339)

A life of faith is necessary to be right with God. The law of Moses alone could not provide this ability to have a right relationship (Psalm 14:1–3; see Romans 3:9–20).

* **Associated Passages:** Acts 13:39

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`)

## Acts 13:40–41 (id: 9341)

Paul ended his message with a strong warning (see Hebrews 2:3). Do not ignore, neglect, or reject the gospel, or you will encounter serious consequences.

* **Associated Passages:** Acts 13:40-41

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`)

## Acts 13:42–43 (id: 9342)

The message attracted many people, and many of them chose to believe.

* **Associated Passages:** Acts 13:42-43

* **Associated ACAI Concepts:** Worship (ID: `keyterm:Worship`); Persuade (ID: `keyterm:Persuade`); Favor (ID: `keyterm:Favor`); Barnabas (ID: `person:Barnabas`); LORD (ID: `deity:Lord`); Sabbath (ID: `keyterm:Sabbath`); Word (ID: `keyterm:Word`); Ask for (ID: `keyterm:AskFor`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 13:43 (id: 9343)

* Full converts to Judaism (also called *proselytes*) were Gentiles who had gone through the rite of circumcision to become full members of the Jewish community, observing the Jewish law (see also 2:11; 6:5; Matthew 23:15).
* *the grace of God*: The grace of God is an important idea in the New Testament. It describes God’s kindness and favor that people do not earn, shown most clearly through Jesus Christ.

* **Associated Passages:** Acts 13:43

* **Associated ACAI Concepts:** Worship (ID: `keyterm:Worship`); Persuade (ID: `keyterm:Persuade`); Favor (ID: `keyterm:Favor`); Barnabas (ID: `person:Barnabas`); LORD (ID: `deity:Lord`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 13:44–49 (id: 9344)

People were excited by the preaching of the apostles. A large gathering formed the next week. This response made some Jews jealous (see 4:1–2; 5:17). This was because Paul's ministry was attracting more converts than Judaism (13:43). These Jews made verbal attacks against Paul and his ministry (see 6:8–12; 18:6; 19:9; Matthew 23:13). 

Paul responded to the attacks with a bold statement that the jealous Jews had rejected their past opportunities to hear the word of God. Paul would now proclaim the message of salvation to Gentiles, following the Lord's command in Scripture (see Acts 10:34–35). The local Gentiles welcomed the gospel, and many accepted it. They spread the Lord's message throughout the region.

* **Associated Passages:** Acts 13:44-49

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Gentiles (ID: `group:Gentile`); Always (ID: `keyterm:Always`); Life (ID: `keyterm:Life`); Paul (ID: `person:Paul`); Resentment (ID: `keyterm:Resentment`); Glory (ID: `keyterm:Glory`); Sabbath (ID: `keyterm:Sabbath`); Barnabas (ID: `person:Barnabas`); Believe (ID: `keyterm:Believe`); Blaspheme (ID: `keyterm:Blaspheme`); Decided (ID: `keyterm:Decided`); Salvation (ID: `keyterm:Salvation`); Jews (ID: `group:Jews`); To Command (ID: `keyterm:Command`)

## Acts 13:50–51 (id: 9346)

Jewish opposition once again forced Paul and Barnabas out of their district. They shook the dust off their feet as a sign of rejection, as Jesus had taught his disciples (see Matthew 10:14–15; Mark 6:11–12; Luke 9:5–6; 10:10–11). That place was then treated as pagan (not worshiping the true God). Paul and Barnabas left that place in order to reach other people who might be more open to the message of new life in Christ.

* **Associated Passages:** Acts 13:50-51

* **Associated ACAI Concepts:** Iconium (ID: `place:Iconium`); Barnabas (ID: `person:Barnabas`); Paul (ID: `person:Paul`); Jews (ID: `group:Jews`); Worship (ID: `keyterm:Worship`)

## Acts 13:52 (id: 9347)

Even though they probably encountered harassment and persecution, the disciples *were filled with joy and with the Holy Spirit* (compare 5:41; 16:23–25; Matthew 5:10–12; 2 Corinthians 8:2; 1 Thessalonians 1:6).

* **Associated Passages:** Acts 13:52

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Cause of Joy (ID: `keyterm:CauseOfJoy`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 14:1 (id: 9356)

* Paul and Barnabas traveled on their mission to the next town, Iconium (now called Konya). This city is east of Pisidian Antioch on a high plateau in south\-central Asia Minor. Iconium was near several important trade routes.
* In their usual way, Paul and Barnabas began their ministry by speaking powerfully in the Jewish synagogue. Their teachings led to a positive response from both Jews and Greeks.

* **Associated Passages:** Acts 14:1

* **Associated ACAI Concepts:** Iconium (ID: `place:Iconium`); Javan (ID: `person:Javan`); Believe (ID: `keyterm:Believe`); Jews (ID: `group:Jews`); Greeks (ID: `group:Greek`); Synagogue (ID: `realia:Synagogue`)

## Acts 14:2 (id: 9360)

Once again, Paul and Barnabas faced Jews who opposed them. These people rejected the Christian message (the good news about Jesus) and also influenced the Gentiles to think wrongly about it.

* **Associated Passages:** Acts 14:2

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Jews (ID: `group:Jews`)

## Acts 14:3 (id: 9361)

* Because the apostles were sustained by the Holy Spirit, they were strong enough to confront verbal attacks. They continued to preach the good news that God's grace was for both Gentiles and Jews.
* *boldly*: The book of Acts often shows how the Holy Spirit gave courage to Christians (see 2:14; 4:9–10, 13; 7:2–53; 8:30–35; 9:27–28; 18:26; 19:8; 22:3–21; 23:1–6; 28:16–20, 23–31).
* The Holy Spirit also affirmed the Christian message with signs and wonders (compare 5:12–16; 15:12; 16:18; 19:11).

* **Associated Passages:** Acts 14:3

* **Associated ACAI Concepts:** Bear Witness (ID: `keyterm:BearWitness`); Favor (ID: `keyterm:Favor`); LORD (ID: `deity:Lord`); Symbol (ID: `keyterm:Example`); Portent (ID: `keyterm:Portent`)

## Acts 14:4 (id: 9362)

* The message preached by the apostles required people to decide whether to trust in Jesus. Opinions were clearly divided.
* Paul and Barnabas are called apostles for the first time (also in 14:14). This broadened the meaning of the word *apostle*. It no longer referred only to the Twelve. The Gospel of Luke shows an earlier example of this. Jesus sent out 70 (or 72\) others as messengers (see Luke 10:1, 17).

    Paul and Barnabas's message continued the work of the original apostles. They were ready to encounter difficulty and abuse for it, just as the earlier apostles had done (5:41; 14:19; 20:24; 21:13–14; see also Romans 8:35–38; 2 Corinthians 4:8–17).

* **Associated Passages:** Luke 10:1; Luke 10:17; Acts 14:4

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Acts 14:6 (id: 9363)

* Lycaonia was a southern region of the Roman province of Galatia. Its major cities were Lystra, Derbe, Laranda, and Iconium. Acts reports two more visits by Paul to Lycaonia (16:1–5; 18:23). Paul’s letter to the Galatians was probably addressed to scattered believers in the churches of Lycaonia (see Galatians Book Introduction, “Recipients”).
* Lystra was about 40 kilometers (25 miles) south or southwest of Iconium.
* Derbe was about 50 kilometers (30 miles) southeast of Lystra. It was on a busy main road that went from Iconium and Lystra towards the east to Tarsus, the capital of Cilicia.

* **Associated Passages:** Acts 14:6

* **Associated ACAI Concepts:** Lycaonia (ID: `place:Lycaonia`); Lystra (ID: `place:Lystra`); Derbe (ID: `place:Derbe`)

## Acts 14:8–20 (id: 9364)

Paul healed a man who could not use his feet. This is similar to an earlier event in 3:1–12. In Acts, Paul’s work is shown as similar to Peter’s work. The many signs and wonders done among the Jews were also done among the Gentiles.

* **Associated Passages:** Acts 14:8-20

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Barnabas (ID: `person:Barnabas`); Zeus (ID: `deity:Zeus`); Offering (ID: `keyterm:Offering`); Worthless (ID: `keyterm:Worthless`); Antioch (ID: `place:Antioch.2`); Hermes (ID: `deity:Hermes`); Derbe (ID: `place:Derbe`); Do Good (ID: `keyterm:DoGood`); Lycaonians (ID: `group:Lycaonia`); Iconium (ID: `place:Iconium`); Heart (ID: `keyterm:Heart`); Jews (ID: `group:Jews`); Lycaonia (ID: `place:Lycaonia`); Salvation (ID: `keyterm:Salvation`); Life (ID: `keyterm:Life`); Be a Disciple (ID: `keyterm:Disciple`); Sky (ID: `keyterm:Heaven`); From Heaven (ID: `keyterm:FromHeaven`); LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Persuade (ID: `keyterm:Persuade`); Lystra (ID: `place:Lystra`); Good News (ID: `keyterm:GoodNews`); Garland (ID: `realia:Garland`); Cattle (ID: `fauna:Bull`); Temple (ID: `realia:Temple`); Clothing (ID: `realia:Clothing`)

## Acts 14:9 (id: 9365)

*saw that he had faith to be healed*: Paul, filled with the Holy Spirit, sensed the man's spiritual openness and hope for God's help and intervention (see 9:17).

* **Associated Passages:** Acts 14:9

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`)

## Acts 14:11–13 (id: 9353)

Zeus was the main god worshiped in Lystra, and the city had a temple for him. The people thought Barnabas was Zeus, likely because he seemed more impressive or powerful. They thought Paul was Hermes because he was the main speaker.

The people of Lystra believed the gods were visiting them again. This idea comes from a well\-known story by the poet Ovid. In that story, Zeus and Hermes visited the area but were not recognized, except by an old couple (see Ovid, Metamorphoses 8\.616–724\). Because of this, the people tried to honor Paul and Barnabas as gods.

* **Associated Passages:** Acts 14:11-13

* **Associated ACAI Concepts:** Zeus (ID: `deity:Zeus`); Paul (ID: `person:Paul`); Hermes (ID: `deity:Hermes`); Lycaonians (ID: `group:Lycaonia`); Garland (ID: `realia:Garland`); Barnabas (ID: `person:Barnabas`); Lycaonia (ID: `place:Lycaonia`); Offering (ID: `keyterm:Offering`); Temple (ID: `realia:Temple`); Cattle (ID: `fauna:Bull`)

## Acts 14:13–18 (id: 9354)

The apostles opposed the people's worship of idols and their attempt to offer sacrifices to them. They directed the people's worship to the living God. The apostles were representatives of God as they shared the good news of the Christian message.

* **Associated Passages:** Acts 14:13-18

* **Associated ACAI Concepts:** Offering (ID: `keyterm:Offering`); Do Good (ID: `keyterm:DoGood`); Worthless (ID: `keyterm:Worthless`); Zeus (ID: `deity:Zeus`); Life (ID: `keyterm:Life`); Heart (ID: `keyterm:Heart`); Barnabas (ID: `person:Barnabas`); From Heaven (ID: `keyterm:FromHeaven`); LORD (ID: `deity:Lord`); Good News (ID: `keyterm:GoodNews`); Sky (ID: `keyterm:Heaven`); Paul (ID: `person:Paul`); Garland (ID: `realia:Garland`); Temple (ID: `realia:Temple`); Cattle (ID: `fauna:Bull`); Clothing (ID: `realia:Clothing`)

## Acts 14:19–20 (id: 9355)

* The crowd changed their minds when some Jews came from Antioch and Iconium and easily turned them against the apostles.
* *They stoned Paul and dragged him outside the city*: Paul later referred to this time of persecution to teach a lesson (2 Timothy 3:11–12).
* *presuming he was dead*: That Paul got up and went back into the city suggests that God healed his wounds with a miracle.

* **Associated Passages:** Acts 14:19-20

* **Associated ACAI Concepts:** Antioch (ID: `place:Antioch.2`); Derbe (ID: `place:Derbe`); Persuade (ID: `keyterm:Persuade`); Iconium (ID: `place:Iconium`); Barnabas (ID: `person:Barnabas`); Jews (ID: `group:Jews`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`)

## Acts 14:20 (id: 9357)

*the next day he left*: Paul later returned to Lystra on his second missionary journey (16:1).

* **Associated Passages:** Acts 14:20

* **Associated ACAI Concepts:** Derbe (ID: `place:Derbe`); Be a Disciple (ID: `keyterm:Disciple`); Barnabas (ID: `person:Barnabas`)

## Acts 14:22–23 (id: 9358)

* The apostles worked hard to stay in contact with those who had declared their Christian faith. These new disciples needed care, support, and encouragement (15:32, 41; 16:40; 18:23; see 1 Thessalonians 3:2; 4:18; 5:14).
* *Paul and Barnabas appointed elders*: See “Church Leaders” Theme Note.

* **Associated Passages:** Acts 14:22-23
* **Associated Articles:** Church Leaders (ID: `500427@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Abstain from Food (ID: `keyterm:Fast`); Pray (ID: `keyterm:Pray`); Kingdom (ID: `keyterm:Kingdom`); Church (ID: `keyterm:Church`); Ask for (ID: `keyterm:AskFor`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 14:26–28 (id: 9359)

When Paul and Barnabas returned to Antioch after their first missionary journey (a trip to share the good news and start churches), they gathered the church and gave a full report. They told how they had shared the good news and helped new believers grow as disciples. They gave credit to God, who had opened the way for Gentiles to believe (see 11:18; 1 Corinthians 16:9; 2 Corinthians 2:12). 

Similar reports always emphasized God's work through the ministries of his servants (Acts 15:4, 12; 21:19; see Romans 15:17–18; 1 Corinthians 3:5–9; 15:10–11).

* **Associated Passages:** Acts 14:26-28

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Gentiles (ID: `group:Gentile`); Antioch (ID: `place:Antioch`); Favor (ID: `keyterm:Favor`); Believe (ID: `keyterm:Believe`); Church (ID: `keyterm:Church`); Be a Disciple (ID: `keyterm:Disciple`); Door (ID: `realia:Door`)

## Acts 15:1 (id: 9369)

These men from Judea were Jewish Christians who taught that circumcision and conversion to Judaism were necessary for salvation. The main issue was how to welcome non\-Jews into the Christian community. Paul wrote his letter to the Galatians around this time to oppose the same teaching in Galatia (see Galatians Book Introduction, “Date of Writing”).

* **Associated Passages:** Acts 15:1

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Circumcision (ID: `keyterm:Circumcision`); Salvation (ID: `keyterm:Salvation`); Men (ID: `group:GenericMales`); Moses (ID: `person:Moses`)

## Acts 15:2–3 (id: 9370)

The church in Antioch decided to send some delegates (representatives) to Jerusalem. They went to discuss this issue with the apostles and elders there. On the way, they visited believers in Phoenicia and Samaria. In these places, the Christian faith had already spread widely (chapter 8).

* **Associated Passages:** Acts 15:2-3

* **Associated ACAI Concepts:** Phoenicia (ID: `place:Phoenicia`); Gentiles (ID: `group:Gentile`); Barnabas (ID: `person:Barnabas`); Samaria (ID: `place:Samaria`); Cause of Joy (ID: `keyterm:CauseOfJoy`); Jerusalem (ID: `place:Jerusalem`); Church (ID: `keyterm:Church`); Paul (ID: `person:Paul`)

## Acts 15:4–21 (id: 9376)

The first council of the church met to resolve the dispute concerning Gentiles and circumcision (15:1–2).

* **Associated Passages:** Acts 15:4-21

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Gentiles (ID: `group:Gentile`); Believe (ID: `keyterm:Believe`); Peter (ID: `person:Peter`); Name (ID: `keyterm:Name`); Moses (ID: `person:Moses`); James (ID: `person:James.3`); Discern (ID: `keyterm:Discern`); Favor (ID: `keyterm:Favor`); Barnabas (ID: `person:Barnabas`); Heart (ID: `keyterm:Heart`); Pharisees (ID: `group:Pharisee`); Proclaim (ID: `keyterm:Proclaim`); Symbol (ID: `keyterm:Example`); Fornication (ID: `keyterm:Fornication`); Sabbath (ID: `keyterm:Sabbath`); Salvation (ID: `keyterm:Salvation`); Be a Disciple (ID: `keyterm:Disciple`); David (ID: `person:David`); Paul (ID: `person:Paul`); Knower of Hearts (ID: `keyterm:KnowerOfHearts`); Law (ID: `keyterm:Law`); Always (ID: `keyterm:Always`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Church (ID: `keyterm:Church`); To Cleanse (ID: `keyterm:Cleanse.2`); Portent (ID: `keyterm:Portent`); Holy Spirit (ID: `deity:HolySpirit`); Bear Witness (ID: `keyterm:BearWitness`); Circumcision (ID: `keyterm:Circumcision`); Authority (ID: `keyterm:Authority.2`); Blood (ID: `keyterm:Blood`); Decided (ID: `keyterm:Decided`); Good News (ID: `keyterm:GoodNews`); Call Upon (ID: `keyterm:CallUpon`); Chosen (ID: `keyterm:Chosen`); Yoke (ID: `realia:Yoke`); Image (ID: `realia:Image`); Synagogue (ID: `realia:Synagogue`); Tabernacle (ID: `realia:Tabernacle`)

## Acts 15:5 (id: 9378)

These Pharisees believed Jesus was the Messiah, but they still insisted on strict adherence to the law of Moses. Paul strongly opposed their perspective (see Galatians 1:6–9; 2:14–3:14).

* **Associated Passages:** Acts 15:5

* **Associated ACAI Concepts:** Law (ID: `keyterm:Law`); Circumcision (ID: `keyterm:Circumcision`); Believe (ID: `keyterm:Believe`); Pharisees (ID: `group:Pharisee`); Moses (ID: `person:Moses`)

## Acts 15:7–11 (id: 9379)

Peter recalled his experience in Cornelius's household (chapter 10). He argued that God had accepted the Gentiles without circumcision by giving them the Holy Spirit. The grace of the Lord Jesus was available to people from all nations.

* **Associated Passages:** Acts 15:7-11

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Knower of Hearts (ID: `keyterm:KnowerOfHearts`); Gentiles (ID: `group:Gentile`); Holy Spirit (ID: `deity:HolySpirit`); Bear Witness (ID: `keyterm:BearWitness`); Favor (ID: `keyterm:Favor`); Discern (ID: `keyterm:Discern`); Heart (ID: `keyterm:Heart`); Peter (ID: `person:Peter`); Jesus (ID: `person:Jesus.2`); Authority (ID: `keyterm:Authority.2`); Salvation (ID: `keyterm:Salvation`); Good News (ID: `keyterm:GoodNews`); To Cleanse (ID: `keyterm:Cleanse.2`); Be a Disciple (ID: `keyterm:Disciple`); Chosen (ID: `keyterm:Chosen`); Yoke (ID: `realia:Yoke`)

## Acts 15:13–19 (id: 9366)

James, the brother of Jesus, said that the turning of the Gentiles (non\-Jewish people) to God agreed with Scripture. He strongly argued that it was against God’s will to place extra rules on Gentiles who wanted to join God’s people.

* **Associated Passages:** Acts 15:13-19

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); LORD (ID: `deity:Lord`); Name (ID: `keyterm:Name`); James (ID: `person:James.3`); Peter (ID: `person:Peter`); Always (ID: `keyterm:Always`); Decided (ID: `keyterm:Decided`); Call Upon (ID: `keyterm:CallUpon`); David (ID: `person:David`); Tabernacle (ID: `realia:Tabernacle`)

## Acts 15:14 (id: 9367)

*Simon*: This was Peter's original name (see Matthew 16:17–19).

* **Associated Passages:** Acts 15:14

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Peter (ID: `person:Peter`); Gentiles (ID: `group:Gentile`); Name (ID: `keyterm:Name`)

## Acts 15:15–19 (id: 9368)

Because God’s prophets had predicted the conversion and inclusion of the Gentiles*,* James argued that the Gentiles should be accepted as Gentiles, without requiring them to practice Judaism (15:1).

* **Associated Passages:** Acts 15:15-19

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); LORD (ID: `deity:Lord`); Always (ID: `keyterm:Always`); Name (ID: `keyterm:Name`); Decided (ID: `keyterm:Decided`); Call Upon (ID: `keyterm:CallUpon`); David (ID: `person:David`); Tabernacle (ID: `realia:Tabernacle`)

## Acts 15:20 (id: 9371)

* Eating food offered to idols is sinful when it involves active participation in idol worship (see Exodus 20:4; Deuteronomy 5:8; Revelation 2:14, 20). However, Paul teaches that eating such food is not always wrong in itself. The concern is whether eating causes a fellow believer with a weak conscience to sin (1 Corinthians 8:4–13). Paul also warns against eating at a pagan temple, since that amounts to sharing in idol worship. But he permits eating food bought in the market or served at a meal without asking about its origin (10:14–30).
* *sexual immorality*: This was a common practice in the Greek world but is always sinful (Exodus 20:14; Deuteronomy 5:18; Galatians 5:19).
* *the meat of strangled animals*: The law of Moses prohibited eating meat with blood in it or consuming blood (Leviticus 17:10–12, 13–14). This was because “the life of all flesh is its blood” (Leviticus 17:14). Also, “it is the blood that makes atonement for the soul” (see Leviticus 17:11 and study note). Noah, the ancestor of both Gentiles and Jews, first received this command from God (Genesis 9:4).

* **Associated Passages:** Acts 15:20
* **Associated Articles:** Leviticus 17:11 (ID: `20167@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Fornication (ID: `keyterm:Fornication`); Blood (ID: `keyterm:Blood`); Image (ID: `realia:Image`)

## Acts 15:22–29 (id: 9372)

The Jerusalem church selected two leaders to report its decision. They carried a letter from the apostles and elders in Jerusalem that explained the terms of the agreement.

* **Associated Passages:** Acts 15:22-29

* **Associated ACAI Concepts:** Judas (ID: `person:Judas.5`); Silas (ID: `person:Silas`); Antioch (ID: `place:Antioch`); Barnabas (ID: `person:Barnabas`); Chosen (ID: `keyterm:Chosen`); Paul (ID: `person:Paul`); Holy Spirit (ID: `deity:HolySpirit`); Gentiles (ID: `group:Gentile`); Cilicia (ID: `place:Cilicia`); Jesus (ID: `person:Jesus.2`); Blood (ID: `keyterm:Blood`); Name (ID: `keyterm:Name`); Church (ID: `keyterm:Church`); Beloved (ID: `keyterm:Beloved`); Life (ID: `keyterm:Life`); Fornication (ID: `keyterm:Fornication`)

## Acts 15:29 (id: 9373)

*Farewell*: This is a common Greek greeting used to end a letter. In the New Testament, it appears only here. Paul often ended his letters with a stronger focus on God’s grace and peace (for example, 1 Corinthians 16:23–24; 2 Corinthians 13:13; see Ephesians 6:23–24; Philippians 4:23; Colossians 4:18; 1 Thessalonians 5:23–28; 1 Timothy 6:21).

* **Associated Passages:** Acts 15:29

* **Associated ACAI Concepts:** Fornication (ID: `keyterm:Fornication`); Blood (ID: `keyterm:Blood`)

## Acts 15:30–31 (id: 9374)

The Christians in Antioch were very happy to learn they did not need to convert to Judaism or follow all the laws of Moses. With the dispute settled, they could continue teaching and preaching without interruption.

* **Associated Passages:** Acts 15:30-31

* **Associated ACAI Concepts:** Antioch (ID: `place:Antioch`); Ask for (ID: `keyterm:AskFor`)

## Acts 15:36–41 (id: 9375)

The discordant separation of Paul and Barnabas is indicated by a very strong word in Greek (*Their disagreement was so sharp*). They were two dedicated men who disagreed about giving John Mark, Barnabas’s cousin, a second chance to join their missionary journey (Colossians 4:10). John Mark had left them in Pamphylia (see Acts 13:13). Eventually, Paul and John Mark made peace (2 Timothy 4:11).

* **Associated Passages:** Acts 15:36-41

* **Associated ACAI Concepts:** Barnabas (ID: `person:Barnabas`); Paul (ID: `person:Paul`); Mark (ID: `person:Mark`); LORD (ID: `deity:Lord`); Cilicia (ID: `place:Cilicia`); Favor (ID: `keyterm:Favor`); Church (ID: `keyterm:Church`); Pamphylia (ID: `place:Pamphylia`); Silas (ID: `person:Silas`); Cyprus (ID: `place:Cyprus`)

## Acts 15:40–41 (id: 9377)

After disagreeing with Barnabas about John Mark, Paul chose Silas as his partner for a second missionary journey through Syria and Cilicia. The two men traveled by land to visit Derbe and Lystra. They explained the decision of the Jerusalem council and strengthened believers in their faith.

* **Associated Passages:** Acts 15:40-41

* **Associated ACAI Concepts:** Cilicia (ID: `place:Cilicia`); Favor (ID: `keyterm:Favor`); LORD (ID: `deity:Lord`); Church (ID: `keyterm:Church`); Silas (ID: `person:Silas`); Paul (ID: `person:Paul`)

## Acts 16:1–3 (id: 9380)

Paul returned to churches that he and Barnabas had established on their previous journey.

* **Associated Passages:** Acts 16:1-3

* **Associated ACAI Concepts:** Lystra (ID: `place:Lystra`); Javan (ID: `person:Javan`); Jews (ID: `group:Jews`); Greeks (ID: `group:Greek`); Timothy (ID: `person:Timothy`); Derbe (ID: `place:Derbe`); Bear Witness (ID: `keyterm:BearWitness`); Iconium (ID: `place:Iconium`); Faithfulness (ID: `keyterm:Faithfulness`); Name (ID: `keyterm:Name`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`)

## Acts 16:3 (id: 9397)

Paul had Timothy circumcised to enhance Timothy’s acceptance and effectiveness as a Jew in Jewish circles. This was not the same as Titus, who was a full Gentile ("Greek"; Galatians 2:3). 

Elsewhere Paul clarified that it makes no difference to God whether one is circumcised or not circumcised (Romans 2:25–29; Galatians 5:6; 6:15). Paul was prepared to use any legitimate means to communicate the good news about Jesus to different audiences (1 Corinthians 9:20–21).

* **Associated Passages:** Acts 16:3

* **Associated ACAI Concepts:** Javan (ID: `person:Javan`); Jews (ID: `group:Jews`); Greeks (ID: `group:Greek`); Paul (ID: `person:Paul`)

## Acts 16:4–5 (id: 9398)

Paul and Silas faithfully communicated the decisions of the Jerusalem council. The wisdom of the decision was indicated as the churches were strengthened in their faith and grew larger every day (compare 2:41; 4:4; 5:14; 6:1; 9:31; 21:20).

* **Associated Passages:** Acts 16:4-5

* **Associated ACAI Concepts:** Decided (ID: `keyterm:Decided`); Believe (ID: `keyterm:Believe`); Jerusalem (ID: `place:Jerusalem`); Church (ID: `keyterm:Church`)

## Acts 16:6–10 (id: 9400)

God directed the missionaries’ travels: The Holy Spirit had prevented them from proceeding westward into the province of Asia, the Spirit of Jesus did not allow them to go north to Bithynia*,* and then Paul had a vision calling them to go northwest over the Aegean Sea to Macedonia. 

In the book of Acts, God's Spirit guided his servants in many ways, including: 

* through visions from God (see also 9:10; 10:9–16; 22:18),
* direct intuition (compare 8:29, 39; 10:19; 20:22),
* advice from other believers (compare 15:29),
* guidance through prayer (13:2, 4),
* understanding through Scripture (28:25–27), and
* prophecy (11:28).

The Holy Spirit's guidance helped them accomplish their mission to be Christ's witnesses (1:8; see also 4:8, 31; 11:24; 13:9).

* **Associated Passages:** Acts 16:6-10

* **Associated ACAI Concepts:** Mysia (ID: `place:Mysia`); Macedonia (ID: `place:Macedonia`); Holy Spirit (ID: `deity:HolySpirit`); Asia (ID: `place:Asia`); Troas (ID: `place:Troas`); Bithynia (ID: `place:Bithynia`); Phrygia (ID: `place:Phrygia`); Galatia (ID: `place:Galatia`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Ask for (ID: `keyterm:AskFor`); Good News (ID: `keyterm:GoodNews`); Paul (ID: `person:Paul`)

## Acts 16:8 (id: 9401)

Troas was a major port on the Aegean Sea and an important city in the Roman Empire. Paul often visited major cities like Troas, Athens, Corinth, and Ephesus. Once the Christian faith was established in larger cities, local Christian workers could continue to share the good news with smaller nearby communities (for example, Epaphras took the message from Ephesus to Colossae (Colossians 1:7; 4:12–13).

* **Associated Passages:** Acts 16:8

* **Associated ACAI Concepts:** Troas (ID: `place:Troas`); Mysia (ID: `place:Mysia`)

## Acts 16:9–10 (id: 9402)

*a vision*: See "Visions" Theme Note.

* **Associated Passages:** Acts 16:9-10
* **Associated Articles:** Visions (ID: `500385@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Macedonia (ID: `place:Macedonia`); LORD (ID: `deity:Lord`); Ask for (ID: `keyterm:AskFor`); Good News (ID: `keyterm:GoodNews`); Paul (ID: `person:Paul`)

## Acts 16:10 (id: 9381)

*we*: The shift from *they* to *we* suggests that Luke traveled with Paul from Troas to Philippi (16:10–17). Later, Luke joined Paul again at Philippi and sailed with him to Troas. Then they went to Miletus, and from Miletus to Jerusalem (20:5–15; 21:1–18). 

After Paul spent two years in prison in Caesarea, Luke traveled with him to Rome (27:1–28:16).

* **Associated Passages:** Acts 16:10

* **Associated ACAI Concepts:** Macedonia (ID: `place:Macedonia`); Good News (ID: `keyterm:GoodNews`); LORD (ID: `deity:Lord`)

## Acts 16:11 (id: 9382)

* Samothrace is a small island with many mountains. It is west or northwest of the Hellespont (the strait between Asia Minor and Europe). It is about 32 kilometers (20 miles) from the coast of Thrace.
* Neapolis, now known as Kavala, was the seaport for Philippi and the eastern end of the famous *Via Egnatia* (the Egnatian Way), which extended from Rome to Asia.

* **Associated Passages:** Acts 16:11

* **Associated ACAI Concepts:** Neapolis (ID: `place:Neapolis`); Samothrace (ID: `place:Samothrace`); Troas (ID: `place:Troas`)

## Acts 16:12 (id: 9383)

At Philippi, about 17 kilometers (10 miles) from the coast of Neapolis, Paul began his mission work in Europe.

* **Associated Passages:** Acts 16:12

* **Associated ACAI Concepts:** Macedonia (ID: `place:Macedonia`); Philippi (ID: `place:Philippi`); Rome (ID: `place:Rome`)

## Acts 16:13 (id: 9385)

The Jewish community at Philippi was too small to have a synagogue, which required ten adult males. Instead, Jews met for prayer in an open space by the Gangites (now called Angista) River that provided privacy, quiet, and water for Jewish purification rites.

* **Associated Passages:** Acts 16:13

* **Associated ACAI Concepts:** Pray (ID: `keyterm:Pray`); Sabbath (ID: `keyterm:Sabbath`); Temple (ID: `realia:Temple`)

## Acts 16:13–36 (id: 9384)

Luke shares stories of three people in Philippi affected by the gospel: 

* a wealthy woman (16:14–15),
* a demon\-possessed slave girl exploited for money (16:16–21), and
* a middle\-ranking officer (16:23–36).

* **Associated Passages:** Acts 16:13-36

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); LORD (ID: `deity:Lord`); Prison (ID: `realia:Prison`); Silas (ID: `person:Silas`); Pray (ID: `keyterm:Pray`); Salvation (ID: `keyterm:Salvation`); Temple (ID: `realia:Temple`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Baptism (ID: `keyterm:Baptism`); House (ID: `realia:House`); Door (ID: `realia:Door`); Philippian Jailer (ID: `person:PhilippianJailer`); Lydia (ID: `person:Lydia`); Thyatira (ID: `place:Thyatira`); Heart (ID: `keyterm:Heart`); Hope (ID: `keyterm:Hope`); Jews (ID: `group:Jews`); Make Peace (ID: `keyterm:Peace`); Sabbath (ID: `keyterm:Sabbath`); Rome (ID: `place:Rome`); Faithfulness (ID: `keyterm:Faithfulness`); Ask for (ID: `keyterm:AskFor`); Decided (ID: `keyterm:Decided`); Be Lawful (ID: `keyterm:Lawful`); Worship (ID: `keyterm:Worship`); Torch (ID: `realia:Torch`); Foundation (ID: `realia:Foundation`); Chain (ID: `realia:Chain`); Clothing (ID: `realia:Clothing`)

## Acts 16:14–15 (id: 9386)

* The first person whose life Christ changed in Philippi was Lydia. She was a successful business owner from Thyatira (modern Akhisar). This city in western Asia Minor was well\-known for exporting fine fabrics, woven cloth, and linen items. Lydia accepted the gospel message, and Paul baptized her and her household (compare 16:32–33).
* One of Lydia’s first actions as a true believer in the Lord was to extend hospitality to the visiting missionaries. Hospitality is an important Christian virtue (see Matthew 25:31–46; Romans 12:13; 16:23; 1 Timothy 3:2; Titus 1:8; Hebrews 13:2; 1 Peter 4:9; 3 John 1:5–8; compare Genesis 18:1–8; 19:1–3; 24:23–33).

* **Associated Passages:** Acts 16:14-15

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Lydia (ID: `person:Lydia`); Thyatira (ID: `place:Thyatira`); Heart (ID: `keyterm:Heart`); Faithfulness (ID: `keyterm:Faithfulness`); Name (ID: `keyterm:Name`); Worship (ID: `keyterm:Worship`); Ask for (ID: `keyterm:AskFor`); Baptism (ID: `keyterm:Baptism`); Decided (ID: `keyterm:Decided`); Paul (ID: `person:Paul`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 16:16–18 (id: 9387)

* The second example of a changed life in Philippi is a slave girl who had a spirit that enabled her to tell the future (a demon that gave her special knowledge).
* Even though the demon inside her spoke the truth, Paul, like Jesus, did not allow it to share the Christian message (compare Mark 1:25, 34; 3:11–12; Luke 4:35, 41). The Lord commanded that his disciples, not those who oppose the message, should proclaim the gospel (Acts 1:8; 9:15; 26:15–18; Matthew 28:18–20; Luke 24:46–49; John 20:21; see Acts 22:15).

* **Associated Passages:** Acts 16:16-18

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Pray (ID: `keyterm:Pray`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Salvation (ID: `keyterm:Salvation`)

## Acts 16:19–21 (id: 9388)

* As in the ministry of Jesus, the gospel sometimes threatened existing business interests (see also 19:25–27; Mark 5:1–20). Those who were exploiting the slave girl attacked the Christian missionaries and dragged them to the authorities as criminals. They blamed them for causing trouble, used racial bias (*"These men are Jews"*), and appealed to the Philippians’ pride (*"us Romans"*). The Philippians were proud Roman citizens of a Roman colony.
* *customs that are unlawful*: By law, Jews were not permitted to make converts of Romans.

* **Associated Passages:** Acts 16:19-21

* **Associated ACAI Concepts:** Hope (ID: `keyterm:Hope`); Rome (ID: `place:Rome`); Be Lawful (ID: `keyterm:Lawful`); Silas (ID: `person:Silas`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 16:22–24 (id: 9389)

They took all necessary security measures. They wanted to guarantee that Paul and Silas did not escape after they stripped and beat them with wooden rods (see 22:24–26; 2 Corinthians 6:5; 11:23–25). However, as at Christ’s tomb, human efforts could not stop God's intervention (Matthew 27:65).

* **Associated Passages:** Acts 16:22-24

* **Associated ACAI Concepts:** Prison (ID: `realia:Prison`); Clothing (ID: `realia:Clothing`)

## Acts 16:25 (id: 9390)

Paul and Silas, like the apostles suffering abuse in Jerusalem, were full of joy. They rejoiced because God considered them "worthy of suffering disgrace for the Name" (5:41).

* **Associated Passages:** Acts 16:25

* **Associated ACAI Concepts:** Silas (ID: `person:Silas`); LORD (ID: `deity:Lord`); Pray (ID: `keyterm:Pray`); Paul (ID: `person:Paul`)

## Acts 16:26 (id: 9391)

The *strong earthquake* reminds us of the great earthquake that happened at the resurrection of Jesus (Matthew 28:2–3).

* **Associated Passages:** Acts 16:26

* **Associated ACAI Concepts:** Prison (ID: `realia:Prison`); Chain (ID: `realia:Chain`); Door (ID: `realia:Door`); Foundation (ID: `realia:Foundation`)

## Acts 16:27 (id: 9393)

The Roman jailer knew he could lose his life if the prisoners he guarded escaped. This was a common practice (12:19; 27:42; compare *Code of Justinian* 9\.4\.4\).

* **Associated Passages:** Acts 16:27

* **Associated ACAI Concepts:** Prison (ID: `realia:Prison`); Door (ID: `realia:Door`)

## Acts 16:27–36 (id: 9392)

Luke’s third story in Philippi is of the Philippian jailer who, shaken by what had happened, responded in faith when challenged to accept the Lord Jesus and be saved. He was baptized with his household and rejoiced in his newfound faith as he reached out with hospitality to the preachers.

* **Associated Passages:** Acts 16:27-36

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Believe (ID: `keyterm:Believe`); LORD (ID: `deity:Lord`); Salvation (ID: `keyterm:Salvation`); Philippian Jailer (ID: `person:PhilippianJailer`); Make Peace (ID: `keyterm:Peace`); Jesus (ID: `person:Jesus.2`); Baptism (ID: `keyterm:Baptism`); Silas (ID: `person:Silas`); Torch (ID: `realia:Torch`); Temple (ID: `realia:Temple`); Prison (ID: `realia:Prison`); House (ID: `realia:House`); Door (ID: `realia:Door`)

## Acts 16:29–30 (id: 9394)

Paul and Silas impressed the jailer with their cheerful faith, calm attitude during the crisis, and concern for his safety (16:25, 28). Whatever the jailer had understood of Paul's message before the crisis, God inspired him to cry out for help.

* **Associated Passages:** Acts 16:29-30

* **Associated ACAI Concepts:** Torch (ID: `realia:Torch`); Silas (ID: `person:Silas`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`)

## Acts 16:31–34 (id: 9395)

Paul and Silas directed the jailer to a faith in Christ that brings blessing both to him and to his family. The whole household received Christian instruction, and their response was expressed in baptism and the offering of hospitality to Paul and Silas.

* **Associated Passages:** Acts 16:31-34

* **Associated ACAI Concepts:** Believe (ID: `keyterm:Believe`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Baptism (ID: `keyterm:Baptism`); Salvation (ID: `keyterm:Salvation`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 16:37–39 (id: 9396)

* Paul and Silas were Roman citizens, so it was illegal to beat and imprison them (16:22–24). When the city officials learned this, they were understandably alarmed because they had broken Roman law.
* *Let them come themselves and escort us out!*: Paul used his rights as a Roman citizen to save his reputation and protect the Christian message from receiving a bad name. Luke wanted to show that Christianity should have the same status as Judaism in the Roman Empire as a legal religion.

    The Christian faith was not at odds with Roman citizenship. It was important to note that Paul used his rights as a Roman citizen. He was willing to use his privileges as a citizen to promote Christianity in a hostile world (comparing 22:25–27).

* **Associated Passages:** Acts 16:37-39

* **Associated ACAI Concepts:** Rome (ID: `place:Rome`); Fear (ID: `keyterm:Fear`); Word (ID: `keyterm:Word`); Ask for (ID: `keyterm:AskFor`); Paul (ID: `person:Paul`); Prison (ID: `realia:Prison`)

## Acts 16:40 (id: 9399)

This meeting in Lydia’s home encouraged the Philippian Christians. They had to deal with the results of the missionaries’ work in their city.

* **Associated Passages:** Acts 16:40

* **Associated ACAI Concepts:** Lydia (ID: `person:Lydia`); Encourage (ID: `keyterm:Encourage`); Ask for (ID: `keyterm:AskFor`); Prison (ID: `realia:Prison`)

## Acts 17:1–3 (id: 9403)

* Amphipolis was a Roman military post on the Egnatian Way in northeastern Macedonia.
* Apollonia, named after the Greek god Apollo, was also on the Egnatian Way.
* Thessalonica was a good place to begin ministry because it had a Jewish synagogue (see study note on 9:2). Paul preached there for three consecutive Sabbath days. He explained the Scriptures and showed how they are fulfilled in Jesus (how they come true in him).

* **Associated Passages:** Acts 17:1-3
* **Associated Articles:** Acts 9:2 (ID: `9662@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Apollonia (ID: `place:Apollonia`); Amphipolis (ID: `place:Amphipolis`); Thessalonica (ID: `place:Thessalonica`); Written (ID: `keyterm:Scripture`); Sabbath (ID: `keyterm:Sabbath`); Jesus (ID: `person:Jesus.2`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Synagogue (ID: `realia:Synagogue`)

## Acts 17:1–9 (id: 9404)

After traveling through Macedonia to Thessalonica*,* Paul first preached to the Jews in the synagogue (Romans 1:16). As in other places, there was a mixed response.

* **Associated Passages:** Acts 17:1-9

* **Associated ACAI Concepts:** Jason (ID: `person:Jason`); Jesus (ID: `person:Jesus.2`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Apollonia (ID: `place:Apollonia`); Amphipolis (ID: `place:Amphipolis`); Thessalonica (ID: `place:Thessalonica`); Javan (ID: `person:Javan`); Written (ID: `keyterm:Scripture`); Persuade (ID: `keyterm:Persuade`); Ability (ID: `keyterm:Ability`); Sabbath (ID: `keyterm:Sabbath`); Worship (ID: `keyterm:Worship`); Silas (ID: `person:Silas`); Greeks (ID: `group:Greek`); Claudius (ID: `person:Claudius`); House (ID: `realia:House`); Synagogue (ID: `realia:Synagogue`)

## Acts 17:4 (id: 9414)

* *God\-fearing Greeks*: See study note on 10:2.
* Luke often draws attention to women who joined the Christian movement (17:4, 12, 34; see also 5:14; 8:12; 16:13–15, 31–34; 18:26; 21:9).

* **Associated Passages:** Acts 17:4
* **Associated Articles:** Acts 10:2 (ID: `9296@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Javan (ID: `person:Javan`); Persuade (ID: `keyterm:Persuade`); Worship (ID: `keyterm:Worship`); Silas (ID: `person:Silas`); Greeks (ID: `group:Greek`); Paul (ID: `person:Paul`)

## Acts 17:5–7 (id: 9415)

Once again, the opponents of the Christian faith regarded it as a disruption and a threat (compare 16:19–21). Ironically, they formed a mob and then accused Paul and Silas of causing trouble by disturbing the peace and defying Caesar. These were serious charges that threatened the stability of the Roman Empire (compare Luke 23:2).

* **Associated Passages:** Acts 17:5-7

* **Associated ACAI Concepts:** Jason (ID: `person:Jason`); Ability (ID: `keyterm:Ability`); Jesus (ID: `person:Jesus.2`); Jews (ID: `group:Jews`); Claudius (ID: `person:Claudius`); House (ID: `realia:House`)

## Acts 17:8–9 (id: 9416)

The charges did not stand up when the officials examined them. So they released Jason and the other believers after they paid a bond (money given as a promise to return if required).

Luke shows that Christianity was not a political threat to the Roman Empire. For this reason, he argues that it should be treated as a permitted religion and not attacked for political reasons.

* **Associated Passages:** Acts 17:8-9

* **Associated ACAI Concepts:** Jason (ID: `person:Jason`)

## Acts 17:10–12 (id: 9405)

Because of strong opposition in Thessalonica, the believers sent Paul and Silas to Berea, about 80\.5 kilometers (50 miles) west of Thessalonica. They had a better reception in Berea than in Thessalonica. Many Jews and prominent Greek women and men became Christians. 

The Bereans were good examples because they were open\-minded, eager to learn, good listeners, thoughtful, and paid attention to the teachings of Scripture. Their resulting faith had a strong foundation.

* **Associated Passages:** Acts 17:10-12

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`); Discern (ID: `keyterm:Discern`); Berea (ID: `place:Berea`); Greeks (ID: `group:Greek`); Javan (ID: `person:Javan`); Believe (ID: `keyterm:Believe`); Silas (ID: `person:Silas`); Thessalonica (ID: `place:Thessalonica`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Synagogue (ID: `realia:Synagogue`)

## Acts 17:13–15 (id: 9406)

Paul recognized how strong and ongoing this persecution (hostile treatment for following Jesus) was when he wrote to the Thessalonians (1 Thessalonians 2:14–16).

* **Associated Passages:** Acts 17:13-15

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Timothy (ID: `person:Timothy`); Silas (ID: `person:Silas`); Thessalonica (ID: `place:Thessalonica`); Commandment (ID: `keyterm:Commandment`); LORD (ID: `deity:Lord`); Berea (ID: `place:Berea`); Jews (ID: `group:Jews`); Athens (ID: `place:Athens`)

## Acts 17:16–17 (id: 9407)

Athens, like Alexandria and Tarsus, was proud of its learning and love of ideas. People there spent time studying and discussing different philosophies (ways of thinking about life and the world) that were popular at that time.

* **Associated Passages:** Acts 17:16-17

* **Associated ACAI Concepts:** Athens (ID: `place:Athens`); Worship (ID: `keyterm:Worship`); Jews (ID: `group:Jews`); Spirit (ID: `keyterm:Spirit.2`); Paul (ID: `person:Paul`); Image (ID: `realia:Image`); Synagogue (ID: `realia:Synagogue`)

## Acts 17:16–34 (id: 9408)

In this chapter, we see Paul presented as a model witness for Christ, engaging the thinkers of his day and challenging them with the Christian message. Paul quoted writers his audience would be familiar with and showed the relevance of the gospel by dialoguing with them, critiquing their assumptions, and offering Jesus as a constructive alternative (see Colossians 1:28). 

Paul reminded these proud intellectuals that there is a living God to whom all human beings are answerable; that they will be judged by him through Jesus, whom God raised from the dead; and that they should therefore repent and put their faith in Jesus.

* **Associated Passages:** Acts 17:16-34

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Areopagus (ID: `place:Areopagus`); Athens (ID: `place:Athens`); Paul (ID: `person:Paul`); Worship (ID: `keyterm:Worship`); Resurrection (ID: `keyterm:Resurrection`); Life (ID: `keyterm:Life`); Ability (ID: `keyterm:Ability`); Image (ID: `realia:Image`); Stoics (ID: `group:Stoic`); Dionysius (ID: `person:Dionysius`); Epicureans (ID: `group:Epicurean`); Divine (ID: `keyterm:Divine`); Athenians (ID: `group:Athens`); Areopagites (ID: `group:Areopagus`); Damaris (ID: `person:Damaris`); Decided (ID: `keyterm:Decided`); Bring to the Attention Of (ID: `keyterm:BringToTheAttentionOf`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Jews (ID: `group:Jews`); Serve (ID: `keyterm:Serve`); Obligated (ID: `keyterm:Ought`); Sky (ID: `keyterm:Heaven`); Spirit (ID: `keyterm:Spirit.2`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Repent (ID: `keyterm:Repent`); Proof (ID: `keyterm:Proof`); Object of Worship (ID: `keyterm:ObjectOfWorship`); Good News (ID: `keyterm:GoodNews`); Demons (ID: `deity:Demons`); Synagogue (ID: `realia:Synagogue`); Temple (ID: `realia:Temple`); Stone Altar (ID: `realia:StoneAltar`)

## Acts 17:18 (id: 9409)

* *Epicurean philosophers* followed Epicureanism, a Greek school of thought founded by Epicurus, who lived from 341 to 270 BC. Epicureans believed that the main goal of life was happiness. They did not define pleasure as physical excess, as some critics said. Instead, they valued peace of mind and freedom from fear. Many people in their time called them atheists because they did not believe the gods controlled human life. They also believed that death was simply the end of a person’s existence. For safety and peace, they formed organized communities and lived apart from wider society.
* *Stoic philosophers* followed Stoicism, founded by Zeno of Citium, who lived from 335 to 263 BC. Stoicism became one of the most influential philosophies in the Greco\-Roman world. Stoics believed that the universe was filled with Reason, which they sometimes called God or Providence. They believed this divine Reason could also be seen in human reason. As people grew in understanding, they could move from ignorance, which leads to vice, to true knowledge, which leads to virtue.

    Stoics made detailed lists of virtues and vices. They also wrote household codes, which were instructions for family life. In some ways, Paul’s teaching sounds similar, especially in his household codes and lists of virtues and vices (Galatians 5:19–23; Ephesians 5:22–33; Colossians 3:18–4:1; 1 Timothy 3:1–13; 5:1–6:1). However, Paul's message about the life, death, and resurrection of Jesus Christ was unfamiliar to these Greek philosophers.

* Their insulting use of the word *babbler* for Paul also shows that they looked down on him.

* **Associated Passages:** Acts 17:18

* **Associated ACAI Concepts:** Stoics (ID: `group:Stoic`); Epicureans (ID: `group:Epicurean`); Demons (ID: `deity:Demons`); Jesus (ID: `person:Jesus.2`); Resurrection (ID: `keyterm:Resurrection`); Good News (ID: `keyterm:GoodNews`)

## Acts 17:22–31 (id: 9410)

* Paul’s sermon in Athens shows how well he could adapt his preaching of the good news (Romans 11:14; 1 Corinthians 9:19–23; 10:33; compare Acts 16:3; 17:2–3; 21:20–26). His Greek audience did not know the Scriptures or believe in one God like the Jews. However, they had a strong intellectual background. Paul connected with them using an Athenian inscription on an altar *TO AN UNKNOWN GOD*. He then explained God’s nature as the Creator (17:22–29), followed by God’s purpose as the Redeemer (17:30–31).
* As he did in his letters (see 1 Corinthians 15:33; Titus 1:12), Paul also used lines from Greek poets. His sermon has several ideas similar to the *Hymn to Zeus* by Cleanthes, who lived around 315 to 240 BC. Paul likely quoted Epimenides, who wrote, "In him we live and move and exist" (Epimenides, *Cretica,* about 600 BC). Paul also quoted Aratus, a Stoic (a follower of Stoicism, a Greek philosophy that valued reason and virtue) from Cilicia, who wrote, "We are his offspring" (Aratus, *Phaenomena* 5\).

    By using these familiar sayings, Paul connected with his audience. Then he moved to his main message. He taught that God is one and not like idols made by humans (Acts 17:29). He also warned that God will judge the world through Jesus Christ, whom he raised from the dead.

    Even though Paul spoke in ways that related to Greek culture and thought, his message clearly pointed to Christ. He challenged an educated but idol\-worshiping audience to respond to the truth.

* **Associated Passages:** Acts 17:22-31

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Life (ID: `keyterm:Life`); Ability (ID: `keyterm:Ability`); Areopagus (ID: `place:Areopagus`); Divine (ID: `keyterm:Divine`); Decided (ID: `keyterm:Decided`); Worship (ID: `keyterm:Worship`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Athens (ID: `place:Athens`); Serve (ID: `keyterm:Serve`); Obligated (ID: `keyterm:Ought`); Sky (ID: `keyterm:Heaven`); Paul (ID: `person:Paul`); Repent (ID: `keyterm:Repent`); Proof (ID: `keyterm:Proof`); Object of Worship (ID: `keyterm:ObjectOfWorship`); Image (ID: `realia:Image`); Temple (ID: `realia:Temple`); Stone Altar (ID: `realia:StoneAltar`)

## Acts 17:30 (id: 9411)

The idea of people’s ignorance is carefully discussed both in the Old Testament law (Leviticus 4:2, 22, 27; 5:15, 17; Numbers 15:25, 27). It is also present in the New Testament (Ephesians 4:18; 1 Peter 1:14; 2:15; 2 Peter 3:5, 8). Paul was especially eager to correct ignorance (see Romans 10:13–15; 11:25; 1 Corinthians 10:1; 12:1; 2 Corinthians 1:8; 1 Thessalonians 4:13). The message of the gospel overcomes ignorance and calls all people to repent "and be baptized … and receive the gift of the Holy Spirit" (see Acts 2:38).

* **Associated Passages:** Acts 17:30

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Repent (ID: `keyterm:Repent`)

## Acts 17:32 (id: 9412)

The Athenians listened carefully to Paul until he spoke about the resurrection of the dead (17:31). At that point, some began to laugh and mock him. To Greek listeners, the idea of resurrection seemed foolish (see 1 Corinthians 15:12–19). In a similar way, the Jews in Jerusalem listened to Paul until he spoke about God accepting the Gentiles (Acts 22:22). Then they also rejected his message.

These examples show how the message of the good news can offend people because of their prejudices (strong opinions that keep them from accepting the truth; see 1 Corinthians 1:20–25).

* **Associated Passages:** Acts 17:32

* **Associated ACAI Concepts:** Resurrection (ID: `keyterm:Resurrection`)

## Acts 17:34 (id: 9413)

* *some joined him and believed*: Some people joined Paul and became believers. His sermon did not only bring rejection; it also led to faith.

    Two important people who believed are named: Dionysius and Damaris. Luke often places a man and a woman side by side in his writing to show that the message of the good news is for all people. By pairing men and women, he shows that both are equally important in God’s work (for example, 5:1–11; 17:4, 12; Luke 14:1–6; 15:3–10).

* Dionysius was a member of the council of the Areopagus, the highest governing body of Athens.

* **Associated Passages:** Acts 17:34

* **Associated ACAI Concepts:** Damaris (ID: `person:Damaris`); Areopagites (ID: `group:Areopagus`); Dionysius (ID: `person:Dionysius`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Areopagus (ID: `place:Areopagus`)

## Acts 18:1 (id: 9423)

In Roman times, political power in Achaia was centered in Corinth, an important city\-state and a major center for trade. Corinth became known for widespread prostitution, sexual immorality (wrong sexual behavior), and drunkenness (see 1 Corinthians Book Introduction, “Setting”). 

Paul spent a great deal of time and effort building a Christian community there (see study note on Acts 16:8).

* **Associated Passages:** Acts 18:1
* **Associated Articles:** Acts 16:8 (ID: `9401@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Corinth (ID: `place:Corinth`); Athens (ID: `place:Athens`)

## Acts 18:1–17 (id: 9417)

Paul spent 18 productive months preaching and teaching in Corinth. He started in the synagogue and then moved next door to the house of Titius Justus (18:1–11). Then Paul won an important case in court against his opponents (18:12–17).

* **Associated Passages:** Acts 18:1-17

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); LORD (ID: `deity:Lord`); Gallio (ID: `person:Gallio`); Synagogue (ID: `realia:Synagogue`); Name (ID: `keyterm:Name`); Judgment Seat (ID: `realia:JudgmentSeat`); Corinth (ID: `place:Corinth`); Worship (ID: `keyterm:Worship`); Law (ID: `keyterm:Law`); Jesus (ID: `person:Jesus.2`); Corinthians (ID: `group:Corinth`); Pontus (ID: `group:Pontus`); Priscilla (ID: `person:Priscilla`); Achaia (ID: `place:Achaia`); Pontus (ID: `place:Pontus`); Rome (ID: `place:Rome`); Crispus (ID: `person:Crispus`); Aquila (ID: `person:Aquila`); Justus (ID: `person:Justus.2`); Sosthenes (ID: `person:Sosthenes`); Claudius (ID: `person:Claudius`); Javan (ID: `person:Javan`); Gentiles (ID: `group:Gentile`); Athens (ID: `place:Athens`); Sabbath (ID: `keyterm:Sabbath`); Clean (ID: `keyterm:Clean`); Unrighteous Act (ID: `keyterm:UnrighteousAct`); Fear (ID: `keyterm:Fear`); Declare (ID: `keyterm:Declare`); Believe (ID: `keyterm:Believe`); Macedonia (ID: `place:Macedonia`); Baptism (ID: `keyterm:Baptism`); Greeks (ID: `group:Greek`); Persuade (ID: `keyterm:Persuade`); Italy (ID: `place:Italy`); Blaspheme (ID: `keyterm:Blaspheme`); Blood (ID: `keyterm:Blood`); Timothy (ID: `person:Timothy`); Silas (ID: `person:Silas`); Tent (ID: `realia:Tent`); Clothing (ID: `realia:Clothing`); House (ID: `realia:House`)

## Acts 18:2–3 (id: 9424)

* The Roman historian Suetonius mentions that the emperor Claudius Caesar ordered all Jews to leave Rome around AD 49 (*Life of Claudius* 25\).
* Aquila and Priscilla were a remarkable husband and wife team in the early church. They worked as tentmakers, as did Paul. Paul probably lived and worked with Aquila and Priscilla during his time in Corinth (Acts 18:11; see Romans 16:3–4; 1 Corinthians 16:19).
* Paul had probably learned tentmaking when he was young. It was a Jewish custom to teach sons a manual trade, even if they planned to become rabbis or other kinds of professionals.

* **Associated Passages:** Acts 18:2-3

* **Associated ACAI Concepts:** Pontus (ID: `place:Pontus`); Rome (ID: `place:Rome`); Pontus (ID: `group:Pontus`); Priscilla (ID: `person:Priscilla`); Claudius (ID: `person:Claudius`); Aquila (ID: `person:Aquila`); Italy (ID: `place:Italy`); Name (ID: `keyterm:Name`); Jews (ID: `group:Jews`); Tent (ID: `realia:Tent`)

## Acts 18:4–6 (id: 9429)

* Paul followed his usual practice of preaching to the Jews first. After encountering rejection and opposition, he then reached out to the Gentiles (13:42–49; see 3:25–26; 26:20; Romans 1:16; 2:10; 3:29, 30; 4:9–12).
* Paul probably wrote his letters to the Christians in Thessalonica after Silas and Timothy returned from Macedonia with a report on the situation there (see 1 Thessalonians 3:6; 2 Thessalonians Book Introduction, “Setting”).

* **Associated Passages:** Acts 18:4-6

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Javan (ID: `person:Javan`); Persuade (ID: `keyterm:Persuade`); Gentiles (ID: `group:Gentile`); Sabbath (ID: `keyterm:Sabbath`); Declare (ID: `keyterm:Declare`); Blaspheme (ID: `keyterm:Blaspheme`); Jesus (ID: `person:Jesus.2`); Blood (ID: `keyterm:Blood`); Macedonia (ID: `place:Macedonia`); Timothy (ID: `person:Timothy`); Silas (ID: `person:Silas`); Clean (ID: `keyterm:Clean`); Greeks (ID: `group:Greek`); Paul (ID: `person:Paul`); Synagogue (ID: `realia:Synagogue`); Clothing (ID: `realia:Clothing`)

## Acts 18:7 (id: 9430)

Titius Justus (“Titus the Just”) was a Gentile who worshiped God. People like him are often called “God\-fearers” (non\-Jews who worship the God of Israel; see study note on 10:2). The name *Titus* was a common Roman name, so the second name or title *Justus* helps to tell him apart from the better\-known Titus, who worked with Paul ("Titus" in 2 Corinthians 2:13; 7:6, 13; 8:6, 16, 23).

* **Associated Passages:** Acts 18:7
* **Associated Articles:** Acts 10:2 (ID: `9296@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Justus (ID: `person:Justus.2`); Worship (ID: `keyterm:Worship`); LORD (ID: `deity:Lord`); Name (ID: `keyterm:Name`); House (ID: `realia:House`); Synagogue (ID: `realia:Synagogue`)

## Acts 18:9–10 (id: 9431)

Paul encountered strong opposition in Corinth and feared another attack (18:6). The divine message comforted him (see 23:11; Psalm 34:4, 7, 19; Matthew 28:20). It encouraged him to continue his public ministry and promised God's protection (see Psalm 91:11; 2 Timothy 4:17).

* **Associated Passages:** Acts 18:9-10

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Fear (ID: `keyterm:Fear`); Paul (ID: `person:Paul`)

## Acts 18:12–13 (id: 9418)

The governor (or proconsul) of Achaia, Junius Gallio, was the older brother of the Roman philosopher Seneca. Seneca later served as a tutor to Emperor Nero.

An ancient inscription shows that Gallio was governor around AD 51–52\. This helps scholars date Paul’s visit to Corinth to about the same time.

* **Associated Passages:** Acts 18:12-13

* **Associated ACAI Concepts:** Gallio (ID: `person:Gallio`); Achaia (ID: `place:Achaia`); Judgment Seat (ID: `realia:JudgmentSeat`); Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Worship (ID: `keyterm:Worship`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 18:14–17 (id: 9419)

Gallio’s ruling indicated that the charges against Paul were unjustified. The Roman government had nothing to fear from acknowledging Christianity as a legal religion.

* **Associated Passages:** Acts 18:14-17

* **Associated ACAI Concepts:** Gallio (ID: `person:Gallio`); Judgment Seat (ID: `realia:JudgmentSeat`); Sosthenes (ID: `person:Sosthenes`); Law (ID: `keyterm:Law`); Name (ID: `keyterm:Name`); Unrighteous Act (ID: `keyterm:UnrighteousAct`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Synagogue (ID: `realia:Synagogue`)

## Acts 18:17 (id: 9420)

The Greeks probably beat Sosthenes to show their anger towards the Jews. It is also possible that the Jews attacked their own synagogue leader because he failed to defend them. Sosthenes might be the same person Paul later calls "our brother" in 1 Corinthians 1:1, but this is uncertain since the name was common.

* **Associated Passages:** Acts 18:17

* **Associated ACAI Concepts:** Sosthenes (ID: `person:Sosthenes`); Gallio (ID: `person:Gallio`); Judgment Seat (ID: `realia:JudgmentSeat`); Synagogue (ID: `realia:Synagogue`)

## Acts 18:18 (id: 9421)

Cenchrea was a seaport on the Aegean Sea, about eight kilometers (five miles) east of Corinth. Here, Paul had his head shaved following Jewish custom to mark the completion of a temporary Nazirite vow (compare 21:23–24; and Numbers 6:1–21; Judges 13:4–7; 16:1; Amos 2:11–12; Luke 1:15).

* **Associated Passages:** Acts 18:18

* **Associated ACAI Concepts:** Cenchreae (ID: `place:Cenchreae`); Vow (ID: `keyterm:Vow`); Priscilla (ID: `person:Priscilla`); Aquila (ID: `person:Aquila`); Paul (ID: `person:Paul`)

## Acts 18:19–23 (id: 9422)

Paul made a quick stop at Ephesus*,* the most important city in the Roman province of Asia. There he left the others behind*,* including Priscilla and Aquila (see 18:26). 

He sailed to Judea, landing at Caesarea*,* the headquarters of the Roman forces of occupation. After a visit to the church at Jerusalem*,* Paul returned to Antioch*,* the church that had originally commissioned him. This marked the end of his second missionary journey.

In Antioch, he spent valuable time reporting what God had done through him and those who ministered with him. He shared the excitement and challenges of their work with the home church. 

After some time, Paul began his third mission journey. He traveled by land through Galatia and Phrygia, again visiting Christians he had led to faith in Christ on his previous trips. It was important to him that these new believers did not lose their way or abandon their faith (see Ephesians 6:10–20; 1 Timothy 1:18–20; 2 Timothy 1:15; 4:10).

* **Associated Passages:** Acts 18:19-23

* **Associated ACAI Concepts:** Ephesus (ID: `place:Ephesus`); Phrygia (ID: `place:Phrygia`); Galatia (ID: `place:Galatia`); Antioch (ID: `place:Antioch`); LORD (ID: `deity:Lord`); Church (ID: `keyterm:Church`); Ask for (ID: `keyterm:AskFor`); Jews (ID: `group:Jews`); Be a Disciple (ID: `keyterm:Disciple`); Caesarea (ID: `place:Caesarea`); Synagogue (ID: `realia:Synagogue`)

## Acts 18:21 (id: 9425)

*“I will come back to you”*: Paul spent a lot of time in Ephesus during his third mission journey (19:1–20:1; see 19:8, 10).

* **Associated Passages:** Acts 18:21

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Ephesus (ID: `place:Ephesus`)

## Acts 18:23 (id: 9427)

Paul wanted to remain connected with earlier converts and strengthen their faith.

* **Associated Passages:** Acts 18:23

* **Associated ACAI Concepts:** Galatia (ID: `place:Galatia`); Phrygia (ID: `place:Phrygia`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 18:23–19.41 (id: 9426)

This section describes Paul's third mission journey, which happened around AD 53 to 57\. Paul visited Galatia and Phrygia again to build stronger relationships with the disciples (18:23). He then traveled to Ephesus, where he stayed for more than two years (19:1,8–10).

* **Associated Passages:** Acts 18:23-19:41

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Jews (ID: `group:Jews`); Ephesus (ID: `place:Ephesus`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Artemis (ID: `deity:Artemis`); Be a Disciple (ID: `keyterm:Disciple`); Name (ID: `keyterm:Name`); Asia (ID: `place:Asia`); The Way (ID: `keyterm:TheWay`); Believe (ID: `keyterm:Believe`); Baptism (ID: `keyterm:Baptism`); Ephesians (ID: `group:Ephesus`); Macedonia (ID: `place:Macedonia`); John (ID: `person:John`); Demetrius (ID: `person:Demetrius`); Apollos (ID: `person:Apollos`); Achaia (ID: `place:Achaia`); Javan (ID: `person:Javan`); Spirit (ID: `keyterm:Spirit.2`); Written (ID: `keyterm:Scripture`); Greeks (ID: `group:Greek`); Persuade (ID: `keyterm:Persuade`); Holy Spirit (ID: `deity:HolySpirit`); Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`); Theater (ID: `realia:Theater`); Tyrannus (ID: `person:Tyrannus`); Demons (ID: `deity:Demons`); Sceva (ID: `person:Sceva`); Gaius (ID: `person:Gaius`); Alexandrians (ID: `group:Alexandria`); Phrygia (ID: `place:Phrygia`); Alexander (ID: `person:Alexander.3`); Aristarchus (ID: `person:Aristarchus`); Aquila (ID: `person:Aquila`); Alexandria (ID: `place:Alexandria`); Erastus (ID: `person:Erastus`); Galatia (ID: `place:Galatia`); Kingdom (ID: `keyterm:Kingdom`); Favor (ID: `keyterm:Favor`); Corinth (ID: `place:Corinth`); Proclaim (ID: `keyterm:Proclaim`); Magic (ID: `keyterm:Magic`); Serve (ID: `keyterm:Serve`); Speak as a Prophet (ID: `keyterm:Speak`); Public Official (ID: `person:GenericOfficial`); Priscilla (ID: `person:Priscilla`); Fear (ID: `keyterm:Fear`); Jerusalem (ID: `place:Jerusalem`); Rome (ID: `place:Rome`); Repent (ID: `keyterm:Repent`); Blaspheme (ID: `keyterm:Blaspheme`); Set Free (ID: `keyterm:SetFree`); Ask for (ID: `keyterm:AskFor`); Timothy (ID: `person:Timothy`); Be Lawful (ID: `keyterm:Lawful`); Worship (ID: `keyterm:Worship`); Synagogue (ID: `realia:Synagogue`); Temple (ID: `realia:Temple`); Lecture Hall (ID: `realia:LectureHall`); Replica Temple (ID: `realia:ReplicaTemple`); Apron (ID: `realia:Apron`); Band of Cloth (ID: `realia:BandOfCloth`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Acts 18:24–26 (id: 9428)

Alexandria was the second\-largest city in the Roman Empire. It was known for its strong tradition of public speaking and for the writings of Philo, a Jewish philosopher.

Apollos was a skilled speaker and knew the Scriptures (the Old Testament) very well. However, his understanding of Jesus and the Holy Spirit was incomplete. He did not yet understand that believers can experience the Holy Spirit’s power in their lives now.

Priscilla and Aquila kindly took him aside and explained the way of God more accurately.

* **Associated Passages:** Acts 18:24-26

* **Associated ACAI Concepts:** The Way (ID: `keyterm:TheWay`); LORD (ID: `deity:Lord`); Alexandria (ID: `place:Alexandria`); Alexandrians (ID: `group:Alexandria`); Apollos (ID: `person:Apollos`); Aquila (ID: `person:Aquila`); John (ID: `person:John`); Written (ID: `keyterm:Scripture`); Priscilla (ID: `person:Priscilla`); Name (ID: `keyterm:Name`); Baptism (ID: `keyterm:Baptism`); Jews (ID: `group:Jews`); Spirit (ID: `keyterm:Spirit.2`); Ephesus (ID: `place:Ephesus`); Synagogue (ID: `realia:Synagogue`)

## Acts 19:1–7 (id: 9432)

* Paul traveled to Ephesus after Apollos had left. Later, Apollos returned to Ephesus while Paul was still there (1 Corinthians 16:12). The two men had different personalities, gifts, and roles, but God worked through both of them.
* Some Christians in Ephesus did not fully understand the Christian faith (Acts 18:26). They did not receive the Holy Spirit when they believed the good news about Jesus. They had accepted John's baptism, which called for repentance from sin, but had not received Christian baptism, which included the gift of the Holy Spirit. When Paul had instructed them further, they were able to receive Christian baptism and the Holy Spirit (19:5,6).

* **Associated Passages:** Acts 19:1-7

* **Associated ACAI Concepts:** Baptism (ID: `keyterm:Baptism`); Paul (ID: `person:Paul`); John (ID: `person:John`); Holy Spirit (ID: `deity:HolySpirit`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Ephesus (ID: `place:Ephesus`); Repent (ID: `keyterm:Repent`); Corinth (ID: `place:Corinth`); Speak as a Prophet (ID: `keyterm:Speak`); Name (ID: `keyterm:Name`); Apollos (ID: `person:Apollos`); Be a Disciple (ID: `keyterm:Disciple`)

## Acts 19:6 (id: 9448)

When Paul laid his hands on them, they received the Holy Spirit. They spoke in other tongues (languages given by the Spirit) and prophesied (spoke messages from God). In this way, they experienced the same filling with God’s power and presence that the disciples received on the day of Pentecost (2:4, 11).

* **Associated Passages:** Acts 19:6

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Speak as a Prophet (ID: `keyterm:Speak`); Paul (ID: `person:Paul`)

## Acts 19:10 (id: 9433)

Paul probably wrote 1 Corinthians during this time (see 1 Corinthians Book Introduction, “Date and Occasion of Writing”).

* **Associated Passages:** Acts 19:10

* **Associated ACAI Concepts:** Asia (ID: `place:Asia`); Javan (ID: `person:Javan`); LORD (ID: `deity:Lord`); Jews (ID: `group:Jews`); Greeks (ID: `group:Greek`)

## Acts 19:11–12 (id: 9434)

* Paul, like other Christian messengers who preached the good news, performed unusual miracles (3:1–11; 4:22; 5:12–16; 6:8; 8:6–7, 13; 9:33–42).
* *evil spirits left them*: Evil spirits were driven out, just as Jesus and Peter had done before (compare 5:16; Mark 1:21–34; Luke 4:31–37).

* **Associated Passages:** Acts 19:11-12

* **Associated ACAI Concepts:** Apron (ID: `realia:Apron`); Band of Cloth (ID: `realia:BandOfCloth`); Ability (ID: `keyterm:Ability`); LORD (ID: `deity:Lord`); Set Free (ID: `keyterm:SetFree`); Paul (ID: `person:Paul`)

## Acts 19:13–16 (id: 9435)

Paul’s ministry was effective, but the traveling Jewish exorcists (people who try to drive out evil spirits) had no real power. They used the name of Jesus as if it were a magic word. However, they did not have a personal relationship with him or the indwelling Holy Spirit (God’s Spirit living in believers).

This contrast shows the difference between true faith and empty practice (compare Matthew 12:27; Mark 9:38–39; Luke 9:49–50; 11:19).

* **Associated Passages:** Acts 19:13-16

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); Demons (ID: `deity:Demons`); Sceva (ID: `person:Sceva`); Authority (ID: `keyterm:Authority.2`); Name (ID: `keyterm:Name`); Proclaim (ID: `keyterm:Proclaim`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 19:17–19 (id: 9436)

The people of Ephesus, both Jews and Greeks, noticed the difference between Paul and the Jewish exorcists. This realization caused them to honor and respect the name of the Lord Jesus. Many people confessed and gave up their sinful practices, including magic.

* **Associated Passages:** Acts 19:17-19

* **Associated ACAI Concepts:** Javan (ID: `person:Javan`); Fear (ID: `keyterm:Fear`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Jews (ID: `group:Jews`); Greeks (ID: `group:Greek`); Ephesus (ID: `place:Ephesus`); Magic (ID: `keyterm:Magic`); Scroll (ID: `realia:Scroll`)

## Acts 19:20 (id: 9437)

Luke’s summary of the success of the gospel in Ephesus is short but powerful (compare 2:43–47; 5:14; 6:7; 9:31; 12:24; 16:5; and Romans 1:16). When the Christian faith overcame problems inside the church, such as division, idolatry (worship of false gods), and pagan practices (religious customs not centered on the true God), it spread widely and grew quickly.

* **Associated Passages:** Acts 19:20

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Authority (ID: `keyterm:Authority.2`)

## Acts 19:21–22 (id: 9438)

* *Paul resolved in the Spirit*:He was probably troubled by what he heard about Corinth (see 1 Corinthians Book Introduction). Instead of going right away, he sent Timothy. Timothy carried a letter, 1 Corinthians, from Paul to the Corinthians (1 Corinthians 4:17; 16:10–11). Paul later traveled to Macedonia and Achaia (Acts 20:1–3).
* *“I must see Rome as well”*: Paul, perhaps inspired by his successful strategy, wanted to share the gospel in the world's most important city (see study note on 16:8).

* **Associated Passages:** Acts 19:21-22
* **Associated Articles:** Acts 16:8 (ID: `9401@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Macedonia (ID: `place:Macedonia`); Asia (ID: `place:Asia`); Erastus (ID: `person:Erastus`); Achaia (ID: `place:Achaia`); Serve (ID: `keyterm:Serve`); Jerusalem (ID: `place:Jerusalem`); Rome (ID: `place:Rome`); Timothy (ID: `person:Timothy`); Spirit (ID: `keyterm:Spirit.2`); Paul (ID: `person:Paul`)

## Acts 19:23 (id: 9440)

*the Way*: See study note on 9:2.

* **Associated Passages:** Acts 19:23
* **Associated Articles:** Acts 9:2 (ID: `9662@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** The Way (ID: `keyterm:TheWay`)

## Acts 19:23–41 (id: 9439)

The main god worshiped in Ephesus was the Greek goddess Artemis. People believed she was born there, so the city of Ephesus was known as the guardian of her temple.

Each year, the people held large festivals in her honor. These included sports, music, and theater. During these events, people would shout, “Great is Artemis of the Ephesians!”

The temple of Artemis in Ephesus was one of the seven wonders of the ancient world.

As more people became Christians, the worship of Artemis began to decline. This also affected the local economy, since many people earned money from her temple. However, it became clear that Paul and his coworkers had not committed any crime.

* **Associated Passages:** Acts 19:23-41

* **Associated ACAI Concepts:** Artemis (ID: `deity:Artemis`); Ephesus (ID: `place:Ephesus`); Ephesians (ID: `group:Ephesus`); Paul (ID: `person:Paul`); Demetrius (ID: `person:Demetrius`); Jews (ID: `group:Jews`); Asia (ID: `place:Asia`); Theater (ID: `realia:Theater`); Gaius (ID: `person:Gaius`); Alexander (ID: `person:Alexander.3`); Aristarchus (ID: `person:Aristarchus`); The Way (ID: `keyterm:TheWay`); Public Official (ID: `person:GenericOfficial`); Be a Disciple (ID: `keyterm:Disciple`); Name (ID: `keyterm:Name`); Macedonia (ID: `place:Macedonia`); Persuade (ID: `keyterm:Persuade`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Ask for (ID: `keyterm:AskFor`); Be Lawful (ID: `keyterm:Lawful`); Worship (ID: `keyterm:Worship`); Replica Temple (ID: `realia:ReplicaTemple`); Temple (ID: `realia:Temple`)

## Acts 19:24 (id: 9442)

People in Ephesus made silver coins and small shrines with the image of Artemis. Small statues of the goddess were carried in city processions and are still sold there today.

* **Associated Passages:** Acts 19:24

* **Associated ACAI Concepts:** Artemis (ID: `deity:Artemis`); Demetrius (ID: `person:Demetrius`); Replica Temple (ID: `realia:ReplicaTemple`); Name (ID: `keyterm:Name`)

## Acts 19:24–34 (id: 9441)

Demetrius, a silversmith whose business was harmed by Paul’s message about one God, stirred up a large riot against him.

* **Associated Passages:** Acts 19:24-34

* **Associated ACAI Concepts:** Artemis (ID: `deity:Artemis`); Ephesus (ID: `place:Ephesus`); Paul (ID: `person:Paul`); Jews (ID: `group:Jews`); Ephesians (ID: `group:Ephesus`); Asia (ID: `place:Asia`); Theater (ID: `realia:Theater`); Demetrius (ID: `person:Demetrius`); Gaius (ID: `person:Gaius`); Alexander (ID: `person:Alexander.3`); Aristarchus (ID: `person:Aristarchus`); Be a Disciple (ID: `keyterm:Disciple`); Name (ID: `keyterm:Name`); Macedonia (ID: `place:Macedonia`); Persuade (ID: `keyterm:Persuade`); Ability (ID: `keyterm:Ability`); Ask for (ID: `keyterm:AskFor`); Worship (ID: `keyterm:Worship`); Replica Temple (ID: `realia:ReplicaTemple`); Temple (ID: `realia:Temple`)

## Acts 19:27 (id: 9443)

*the whole world*: The ancient geographer Strabo reports that temples dedicated to Artemis existed in cities from Asia Minor (now Turkey) to what is now France and Spain (Strabo, *Geography* 3\.4\.8; 4\.1\.4\).

* **Associated Passages:** Acts 19:27

* **Associated ACAI Concepts:** Asia (ID: `place:Asia`); Ability (ID: `keyterm:Ability`); Artemis (ID: `deity:Artemis`); Worship (ID: `keyterm:Worship`); Temple (ID: `realia:Temple`)

## Acts 19:29 (id: 9444)

Archaeologists have uncovered the amphitheater where the whole city gathered. It could hold about 24,000 people.

* **Associated Passages:** Acts 19:29

* **Associated ACAI Concepts:** Gaius (ID: `person:Gaius`); Aristarchus (ID: `person:Aristarchus`); Theater (ID: `realia:Theater`); Macedonia (ID: `place:Macedonia`); Paul (ID: `person:Paul`)

## Acts 19:31 (id: 9445)

*officials of the province of Asia*: These officials, called *Asiarchs* ("rulers of Asia"), were the appointed leaders in the Roman province of Asia. They served as financial supporters of the city and often supported the worship of the emperor. Some of these high\-ranking officials were friends of Paul. Luke shows that the Christian faith also attracted people of high social status.

* **Associated Passages:** Acts 19:31

* **Associated ACAI Concepts:** Theater (ID: `realia:Theater`); Ask for (ID: `keyterm:AskFor`)

## Acts 19:35 (id: 9447)

The tradition said that the image (statue) of Artemis had fallen from heaven. This may mean that it was made from a meteor (a rock that falls to earth from space).

* **Associated Passages:** Acts 19:35

* **Associated ACAI Concepts:** Ephesians (ID: `group:Ephesus`); Public Official (ID: `person:GenericOfficial`); Ephesus (ID: `place:Ephesus`); Artemis (ID: `deity:Artemis`)

## Acts 19:35–41 (id: 9446)

The city clerk stopped the demonstration to prevent the city from being accused of rioting by the Roman government. This story shows that Christians in the Roman world had the right to a fair legal process.

* **Associated Passages:** Acts 19:35-41

* **Associated ACAI Concepts:** Ephesians (ID: `group:Ephesus`); Demetrius (ID: `person:Demetrius`); Blaspheme (ID: `keyterm:Blaspheme`); Artemis (ID: `deity:Artemis`); Public Official (ID: `person:GenericOfficial`); Be Lawful (ID: `keyterm:Lawful`); Ephesus (ID: `place:Ephesus`)

## Acts 20:1–2 (id: 9463)

Paul traveled to Macedonia, where he encouraged Christians in towns like Thessalonica, Philippi, and Berea. He also continued collecting donations for needy Christians in Jerusalem (see Romans 15:25–28; 2 Corinthians 8:1–9:15). 

Titus met Paul in Macedonia with news from Corinth, prompting Paul to write 2 Corinthians and send Titus back with the letter (2 Corinthians 7:5–7; 8:6).

* **Associated Passages:** Acts 20:1-2

* **Associated ACAI Concepts:** Encourage (ID: `keyterm:Encourage`); Ask for (ID: `keyterm:AskFor`); Greece (ID: `place:Greece`); Macedonia (ID: `place:Macedonia`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`)

## Acts 20:2–3 (id: 9468)

* Paul traveled from Macedonia to Greece, specifically Corinth in Achaia, where he stayed for three months (see study note on 18:1 and 2 Corinthians 13:1). During this time, Paul probably wrote his letter to the Romans (see Romans Book Introduction, “Date, Place, and Occasion of Writing”).
* *a plot against him*: See Acts 9:23–25, 28–30; 23:12–35; 25:3.

* **Associated Passages:** Acts 20:2-3
* **Associated Articles:** Acts 18:1 (ID: `9423@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Greece (ID: `place:Greece`); Macedonia (ID: `place:Macedonia`); Encourage (ID: `keyterm:Encourage`); Ask for (ID: `keyterm:AskFor`); Jews (ID: `group:Jews`)

## Acts 20:4 (id: 9475)

Paul's travel companions were disciples from Berea, Thessalonica, Derbe, and Asia. He was teaching and preparing them to lead (compare 2 Timothy 2:2).

* **Associated Passages:** Acts 20:4

* **Associated ACAI Concepts:** Gaius (ID: `person:Gaius.2`); Bereans (ID: `group:Berea`); Trophimus (ID: `person:Trophimus`); Pyrrhus (ID: `person:Pyrrhus`); Asians (ID: `group:Asia`); Sopater (ID: `person:Sopater`); Tychicus (ID: `person:Tychicus`); Secundus (ID: `person:Secundus`); Asia (ID: `place:Asia`); Berea (ID: `place:Berea`); Thessalonians (ID: `group:Thessalonica`); Timothy (ID: `person:Timothy`); Aristarchus (ID: `person:Aristarchus`); Derbe (ID: `place:Derbe`); Thessalonica (ID: `place:Thessalonica`)

## Acts 20:5–15 (id: 9476)

This is another of the “we” passages in Acts (sections where Luke includes himself in the story; see study note on 16:10). Luke likely joined Paul again at Philippi, where he had stayed some years earlier. He then traveled with Paul to Jerusalem (21:1–18).

* **Associated Passages:** Acts 20:5-15
* **Associated Articles:** Acts 16:10 (ID: `9381@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Troas (ID: `place:Troas`); Assos (ID: `place:Assos`); Life (ID: `keyterm:Life`); Miletus (ID: `place:Miletus`); Mitylene (ID: `place:Mitylene`); Eutychus (ID: `person:Eutychus`); Philippi (ID: `place:Philippi`); Week (ID: `keyterm:Week`); Samos (ID: `place:Samos`); Chios (ID: `place:Chios`); Encourage (ID: `keyterm:Encourage`); Name (ID: `keyterm:Name`); Ask for (ID: `keyterm:AskFor`); Second Story (ID: `realia:SecondStory`); Window (ID: `realia:Window`); Lamp (ID: `realia:Lamp`); Ship (ID: `realia:Ship`); Upstairs Room (ID: `realia:UpstairsRoom`)

## Acts 20:7–12 (id: 9477)

* On the first day of the week, the early church remembered the resurrection of Jesus (see Mark 16:9; compare John 20:19; Revelation 1:10).
* This was Paul’s last visit to Troas. The believers met to share the Lord’s Supper (a meal that remembers Jesus’s death). This likely included both the bread and cup (communion) and a shared meal (compare Acts 2:42, 46; Jude 1:12).
* One notable part of this gathering was the quiet miracle involving Eutychus. He was a young man who fell asleep while sitting in a window. He fell three stories and died. Paul went down, bent over him, and took him in his arms. Then Eutychus was restored to life (God brought him back to life through Paul; compare Acts 9:36–41).

* **Associated Passages:** Acts 20:7-12

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Life (ID: `keyterm:Life`); Eutychus (ID: `person:Eutychus`); Week (ID: `keyterm:Week`); Name (ID: `keyterm:Name`); Encourage (ID: `keyterm:Encourage`); Ask for (ID: `keyterm:AskFor`); Second Story (ID: `realia:SecondStory`); Window (ID: `realia:Window`); Lamp (ID: `realia:Lamp`); Upstairs Room (ID: `realia:UpstairsRoom`)

## Acts 20:13–15 (id: 9464)

* Assos was an important city in Mysia on the east coast of the Aegean Sea.
* Mitylene was the most strategic city on the island of Lesbos.
* The island of Samos was crucial for trade routes from Asia Minor to the west and from the Aegean Sea to Egypt.

* **Associated Passages:** Acts 20:13-15

* **Associated ACAI Concepts:** Assos (ID: `place:Assos`); Samos (ID: `place:Samos`); Miletus (ID: `place:Miletus`); Mitylene (ID: `place:Mitylene`); Chios (ID: `place:Chios`); Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`)

## Acts 20:16 (id: 9465)

The Feast of Pentecost was one of the three Jewish feasts that required Jewish men to travel to Jerusalem and worship (see study note on 2:1–4).

* **Associated Passages:** Acts 20:16
* **Associated Articles:** Acts 2:1-4 (ID: `9449@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Ephesus (ID: `place:Ephesus`); Asia (ID: `place:Asia`); Jerusalem (ID: `place:Jerusalem`); Decided (ID: `keyterm:Decided`); Paul (ID: `person:Paul`)

## Acts 20:17 (id: 9466)

Miletus was a major sea port on the western coast of Asia Minor at the opening of the Meander River. Paul met briefly with the elders of the church of Ephesus there on his way to Jerusalem.

* **Associated Passages:** Acts 20:17

* **Associated ACAI Concepts:** Miletus (ID: `place:Miletus`); Ephesus (ID: `place:Ephesus`); Church (ID: `keyterm:Church`)

## Acts 20:18–38 (id: 9467)

Paul’s message to the elders (leaders) of the church in Ephesus describes his life and work among them. He also calls these leaders to show the same kind of commitment as they continue the ministry.

In this message, Paul points to his own life as an example. He shows his honesty and care for people (20:18–21, 26, 31). He also speaks about what will happen in the future (20:22–23, 25, 29–30). He warns them about false teachers (people who teach wrong ideas about God; 20:29–30). And he urges the elders to stay alert and faithful (20:28, 31). 

Through his example, Paul shows what it means to lead as a servant, with sacrifice and careful attention.

* **Associated Passages:** Acts 20:18-38

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Declare (ID: `keyterm:Declare`); Jesus (ID: `person:Jesus.2`); Holy Spirit (ID: `deity:HolySpirit`); Favor (ID: `keyterm:Favor`); Jews (ID: `group:Jews`); Serve (ID: `keyterm:Serve`); Blood (ID: `keyterm:Blood`); Goat (ID: `fauna:Goat`); Flock (ID: `fauna:Flock`); Be Enslaved (ID: `keyterm:Serve.2`); Javan (ID: `person:Javan`); Kingdom (ID: `keyterm:Kingdom`); Proclaim (ID: `keyterm:Proclaim`); Pray (ID: `keyterm:Pray`); Clean (ID: `keyterm:Clean`); Life (ID: `keyterm:Life`); Spirit (ID: `keyterm:Spirit.2`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`); Asia (ID: `place:Asia`); Believe (ID: `keyterm:Believe`); Jerusalem (ID: `place:Jerusalem`); Church (ID: `keyterm:Church`); Greeks (ID: `group:Greek`); Happy (ID: `keyterm:Happy`); Repent (ID: `keyterm:Repent`); Good News (ID: `keyterm:GoodNews`); Consecration (ID: `keyterm:Consecration.2`); Ship (ID: `realia:Ship`); House (ID: `realia:House`); Temple (ID: `realia:Temple`); Wolf (ID: `fauna:Wolf`); Clothing (ID: `realia:Clothing`)

## Acts 20:23 (id: 9469)

*the Holy Spirit warns me*: See, for example, 21:10–12.

* **Associated Passages:** Acts 20:23

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); Declare (ID: `keyterm:Declare`)

## Acts 20:26 (id: 9470)

*Therefore I testify to you this day that I am innocent of the blood of all men*: Compare Ezekiel 3:16–21.

* **Associated Passages:** Acts 20:26

* **Associated ACAI Concepts:** Declare (ID: `keyterm:Declare`); Blood (ID: `keyterm:Blood`); Clean (ID: `keyterm:Clean`)

## Acts 20:28 (id: 9471)

* Paul describes the church as God’s people and God’s flock (a group cared for by a shepherd; compare 1 Peter 2:25; 5:2, 4). In other places, the church is described as:

    + "the body of Christ" (1 Corinthians 12:27; Ephesians 1:23; 4:12; Colossians 1:24);
        + the "bride" of Christ (2 Corinthians 11:2; Revelation 19:7; see Ephesians 5:25–32;
        + "God's temple" (1 Corinthians 3:16; 2 Corinthians 6:16).
        + "a chosen people, a royal priesthood, a holy nation, a people for God’s own possession" (1 Peter 2:9); and
        + "God's field, God's building" (1 Corinthians 3:9).

* Paul expected the leaders to care for and guide the church, like shepherds care for their flock. The Holy Spirit had appointed them to this role(see 1 Timothy 3:1–7; Titus 1:5–7; 1 Peter 5:1–4; compare Acts 6:2–4).

* **Associated Passages:** Acts 20:28

* **Associated ACAI Concepts:** Holy Spirit (ID: `deity:HolySpirit`); LORD (ID: `deity:Lord`); Blood (ID: `keyterm:Blood`); Church (ID: `keyterm:Church`); Goat (ID: `fauna:Goat`); Flock (ID: `fauna:Flock`)

## Acts 20:29–30 (id: 9472)

*savage wolves*: Paul’s prophecy did in fact happen, prompting him to write to Timothy in Ephesus some five years later (see 1 Timothy 1:3–7, 19–20; 4:1–5; see also Matthew 7:15; 10:16; Mark 13:22; Luke 10:3; 2 Peter 2:1–22; 3:3).

* **Associated Passages:** Acts 20:29-30

* **Associated ACAI Concepts:** Be a Disciple (ID: `keyterm:Disciple`); Goat (ID: `fauna:Goat`); Wolf (ID: `fauna:Wolf`); Flock (ID: `fauna:Flock`)

## Acts 20:35 (id: 9473)

*‘It is more blessed to give than to receive’*: This saying of Jesus does not appear in the Gospels.

* **Associated Passages:** Acts 20:35

* **Associated ACAI Concepts:** Happy (ID: `keyterm:Happy`); Jesus (ID: `person:Jesus.2`)

## Acts 20:38 (id: 9474)

The occasion felt more emotional because the church at Ephesus knew they would never see Paul again (20:25).

* **Associated Passages:** Acts 20:38

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`)

## Acts 21:1 (id: 9482)

* Cos was an island in the Aegean Sea with a major trade port.
* Rhodes is a large Aegean island. The Colossus, a huge statue 30 meters (100 feet) tall, once stood at the city's entrance. In Paul’s time, the statue lay where it had fallen during an earthquake over 200 years earlier. It would remain there for another 600 years.
* Patara was the main port of Lycia, located on the coast across from Rhodes.

* **Associated Passages:** Acts 21:1

* **Associated ACAI Concepts:** Cos (ID: `place:Cos`); Rhodes (ID: `place:Rhodes`); Patara (ID: `place:Patara`)

## Acts 21:1–18 (id: 9478)

This “we” passage (see study notes on 16:10; 20:5–15) covers Paul’s journey from Miletus to Jerusalem at the close of the third missionary journey.

* **Associated Passages:** Acts 21:1-18
* **Associated Articles:** Acts 16:10 (ID: `9381@AquiferOpenStudyNotes`); Acts 20:5-15 (ID: `9476@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`); Caesarea (ID: `place:Caesarea`); Be a Disciple (ID: `keyterm:Disciple`); Name (ID: `keyterm:Name`); Tyre (ID: `place:Tyre`); Cyprus (ID: `place:Cyprus`); Holy Spirit (ID: `deity:HolySpirit`); Patara (ID: `place:Patara`); Mnason (ID: `person:Mnason`); Phoenicia (ID: `place:Phoenicia`); Agabus (ID: `person:Agabus`); Cos (ID: `place:Cos`); Philip (ID: `person:Philip.4`); Cypriots (ID: `group:Cyprus`); Rhodes (ID: `place:Rhodes`); Gentiles (ID: `group:Gentile`); Heart (ID: `keyterm:Heart`); Jews (ID: `group:Jews`); Pray (ID: `keyterm:Pray`); Speak as a Prophet (ID: `keyterm:Speak`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Judea (ID: `place:Judea`); James (ID: `person:James.3`); Persuade (ID: `keyterm:Persuade`); Ask for (ID: `keyterm:AskFor`); Acco (ID: `place:Acco`); Belt (ID: `realia:Belt`); House (ID: `realia:House`); Temple (ID: `realia:Temple`); Bag (ID: `realia:Bag`)

## Acts 21:2–3 (id: 9483)

Tyre was an important port in Phoenicia with an empire centered on sea activity and widespread trade interests (see Isaiah 23; Jeremiah 25:15–38; 47; Zechariah 9; Matthew 15:21–28; Mark 7:24–31).

* **Associated Passages:** Acts 21:2-3

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`); Phoenicia (ID: `place:Phoenicia`); Tyre (ID: `place:Tyre`); Cyprus (ID: `place:Cyprus`)

## Acts 21:4–6 (id: 9491)

* The disciples in Tyre gave Paul a warm and emotional farewell. This shows the deep fellowship (shared life and unity) among Christians.
* *Through the Spirit they kept telling Paul not to go up to Jerusalem**:* The disciples in Tyre clearly understood the danger he would face there. Because they cared for him, they tried to persuade him not to go.

    However, Paul accepted these risks. He chose to go in order to complete the mission God had given him (compare Philippians 3:7–10). See also study note on Acts 21:11–14.

* **Associated Passages:** Acts 21:4-6
* **Associated Articles:** Acts 21:11-14 (ID: `9480@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Pray (ID: `keyterm:Pray`); Holy Spirit (ID: `deity:HolySpirit`); Jerusalem (ID: `place:Jerusalem`); Be a Disciple (ID: `keyterm:Disciple`); Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`)

## Acts 21:7 (id: 9492)

Paul visited Ptolemais*,* an important city on the coast of the Mediterranean, as he made his way from Tyre to Caesarea.

* **Associated Passages:** Acts 21:7

* **Associated ACAI Concepts:** Tyre (ID: `place:Tyre`); Acco (ID: `place:Acco`)

## Acts 21:8 (id: 9493)

Philip was called an evangelist (someone who shares the good news about Jesus) because of his work in preaching the gospel (see 8:4–40; compare Ephesians 4:11; 2 Timothy 4:5).

* **Associated Passages:** Acts 21:8

* **Associated ACAI Concepts:** Caesarea (ID: `place:Caesarea`); Philip (ID: `person:Philip.4`); Temple (ID: `realia:Temple`); House (ID: `realia:House`)

## Acts 21:9 (id: 9494)

Philip’s daughters had the gift of prophecy (the ability to speak messages from God). This showed that Joel’s prophecy (Joel 2:28–32) had been fulfilled, just as Peter had said at Pentecost (2:17–21).

* **Associated Passages:** Acts 21:9

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`)

## Acts 21:10 (id: 9479)

Luke places female prophets (21:9) alongside a male prophet (see study note on 17:34). This shows that both women and men were used by God to speak his message in the early church.

Agabus, like the Old Testament prophets (God’s messengers before the time of Jesus), used symbolic actions (visible actions that show a message) to communicate what God had revealed to him (see “Prophetic Sign Acts” Theme Note).

* **Associated Passages:** Acts 21:10
* **Associated Articles:** Acts 17:34 (ID: `9413@AquiferOpenStudyNotes`); Prophetic Sign Acts (ID: `500313@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Agabus (ID: `person:Agabus`); Judea (ID: `place:Judea`); Name (ID: `keyterm:Name`)

## Acts 21:11–14 (id: 9480)

Despite Agabus’s prediction of suffering and the believers’ begging Paul not to go to Jerusalem, Paul was ready even to die for the sake of the Lord Jesus. Luke emphasizes Paul’s courage, determination, and heroism as a Christian missionary who would let nothing interfere with his mission. See study note on 21:4–6.

* **Associated Passages:** Acts 21:11-14
* **Associated Articles:** Acts 21:4-6 (ID: `9491@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Paul (ID: `person:Paul`); Persuade (ID: `keyterm:Persuade`); Gentiles (ID: `group:Gentile`); Holy Spirit (ID: `deity:HolySpirit`); Heart (ID: `keyterm:Heart`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Name (ID: `keyterm:Name`); Ask for (ID: `keyterm:AskFor`); Jews (ID: `group:Jews`); Belt (ID: `realia:Belt`); Bag (ID: `realia:Bag`)

## Acts 21:18–19 (id: 9481)

It was important for James and all the elders (leaders) of the church in Jerusalem to hear about Paul’s successful work among the Gentiles. It was also important for the Jerusalem church, the first main church, to continue to support this mission (21:20; see 15:7–21). 

Paul’s report showed that God had carried out his plan among the Gentiles through Paul’s work.

* **Associated Passages:** Acts 21:18-19

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Serve (ID: `keyterm:Serve`); LORD (ID: `deity:Lord`); James (ID: `person:James.3`); Paul (ID: `person:Paul`)

## Acts 21:20–25 (id: 9484)

The leaders of the Jerusalem church urged Paul to strengthen his credibility. They encouraged him to demonstrate that he was not teaching Jews to abandon the laws of Moses. Meanwhile, the leaders did not force Jewish rules on Gentile converts. They had determined these terms earlier and would respect them (15:22–29).

* **Associated Passages:** Acts 21:20-25

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Law (ID: `keyterm:Law`); Believe (ID: `keyterm:Believe`); Jews (ID: `group:Jews`); Glory (ID: `keyterm:Glory`); LORD (ID: `deity:Lord`); Blood (ID: `keyterm:Blood`); Vow (ID: `keyterm:Vow`); Live (ID: `keyterm:Live.3`); Pure (ID: `keyterm:Pure`); Decided (ID: `keyterm:Decided`); Fornication (ID: `keyterm:Fornication`); Moses (ID: `person:Moses`)

## Acts 21:26–36 (id: 9485)

The concerns of the Christian leaders were well\-founded (21:20–25). When Paul and the other men had almost completed their vow, some Jews from Asia falsely accused Paul and raised a mob against him. They intended to kill Paul, but the Roman commander rescued him.

* **Associated Passages:** Acts 21:26-36

* **Associated ACAI Concepts:** Temple (ID: `realia:Temple`); Paul (ID: `person:Paul`); Trophimus (ID: `person:Trophimus`); Javan (ID: `person:Javan`); Defilement (ID: `keyterm:Defilement`); Israel (ID: `person:Israel`); Pure (ID: `keyterm:Pure`); Offering (ID: `keyterm:Offering`); Jews (ID: `group:Jews`); Something Definite (ID: `keyterm:SomethingDefinite`); Holy (ID: `keyterm:Holy`); Ephesus (ID: `place:Ephesus`); Asia (ID: `place:Asia`); Ephesians (ID: `group:Ephesus`); Law (ID: `keyterm:Law`); Jerusalem (ID: `place:Jerusalem`); Greeks (ID: `group:Greek`); Israelites (ID: `group:Israel`); Step (ID: `realia:Step`); Tabernacle (ID: `realia:Tabernacle`); Chain (ID: `realia:Chain`); Door (ID: `realia:Door`)

## Acts 21:28–29 (id: 9486)

*Greeks*: It was a crime punishable by death to bring Gentiles (non\-Jewish people) into the inner areas of the temple, beyond the Court of the Gentiles (see also Josephus, *War* 5\.5\.2; 6\.2\.4\). The Jews believed Paul broke this holy law by bringing Trophimus, a Gentile from Ephesus, into the area reserved for only Jews (see also Acts 20:4; 2 Timothy 4:20).

* **Associated Passages:** Acts 21:28-29

* **Associated ACAI Concepts:** Temple (ID: `realia:Temple`); Trophimus (ID: `person:Trophimus`); Javan (ID: `person:Javan`); Ephesians (ID: `group:Ephesus`); Israelites (ID: `group:Israel`); Defilement (ID: `keyterm:Defilement`); Law (ID: `keyterm:Law`); Israel (ID: `person:Israel`); Holy (ID: `keyterm:Holy`); Greeks (ID: `group:Greek`); Ephesus (ID: `place:Ephesus`); Paul (ID: `person:Paul`); Tabernacle (ID: `realia:Tabernacle`)

## Acts 21:30 (id: 9487)

This supposed defilement of the temple made many of the Jews very angry. They believed the temple had been made unclean by a Greek (non\-Jew), so they quickly shut the temple gates.

* **Associated Passages:** Acts 21:30

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Temple (ID: `realia:Temple`); Door (ID: `realia:Door`)

## Acts 21:31 (id: 9488)

*commander*: Or *tribune*, a Roman officer who commanded 1,000 men.

* **Associated Passages:** Acts 21:31

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Acts 21:34 (id: 9489)

Herod the Great built the Antonia Fortress, a large military base, on the northwest corner of the Temple Mount. It housed the Roman soldiers stationed there to keep the peace.

* **Associated Passages:** Acts 21:34

* **Associated ACAI Concepts:** Something Definite (ID: `keyterm:SomethingDefinite`)

## Acts 21:37–40 (id: 9490)

The commander had mistaken Paul for an Egyptian false messiah who had planned to seize power from the Romans around AD 54 (roughly three years earlier; see Josephus, *War* 2\.13\.5\). Paul corrected the mistake, gained permission to speak to the people, and addressed the crowd in Aramaic, the common language of Judea. He gave a strong statement of his faith in Jesus as the Messiah (Acts 22:1–21).

* **Associated Passages:** Acts 21:37-40

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Tarsus (ID: `place:Tarsus`); Cilicia (ID: `place:Cilicia`); Tarsians (ID: `group:Tarsus`); Egypt (ID: `place:Egypt`); Military Member (ID: `person:MilitaryMember`); Egyptians (ID: `group:Egypt`); Be Lawful (ID: `keyterm:Lawful`); Jews (ID: `group:Jews`); Step (ID: `realia:Step`)

## Acts 22:1–21 (id: 9495)

Paul gives his most complete defense of his life and faith before his own people in Jerusalem. This speech shows his flexibility as a missionary, just as his message to the Greek philosophers in Athens also did (17:22–31; see 1 Corinthians 9:20–23). 

He begins by showing his close connection with his audience. He explains his Jewish background and his training under Gamaliel the Elder (a respected Jewish teacher). He also describes his strong desire to honor God, a desire they shared (Acts 22:1–3). 

Paul then recounts how he once persecuted Christians (22:4–5). He tells about seeing Jesus on the road to Damascus (22:6–10). and explains his conversion (his turning to follow Jesus; 22:11–16). 

He ends by describing a vision in the temple, where the Lord spoke to him. The Lord told him that many Jews would reject the message and that he should go to the Gentiles (22:17–21).

* **Associated Passages:** Acts 22:1-21

* **Associated ACAI Concepts:** Damascus (ID: `place:Damascus`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Bear Witness (ID: `keyterm:BearWitness`); Paul (ID: `person:Paul`); Witness (ID: `keyterm:Witness`); Jews (ID: `group:Jews`); Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Gamaliel (ID: `person:Gamaliel`); Ananias (ID: `person:Ananias.2`); Gentiles (ID: `group:Gentile`); The Way (ID: `keyterm:TheWay`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); To Sin (ID: `keyterm:Sin`); Pray (ID: `keyterm:Pray`); Cilicia (ID: `place:Cilicia`); Sky (ID: `keyterm:Heaven`); Tarsus (ID: `place:Tarsus`); Stephen (ID: `person:Stephen`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Baptism (ID: `keyterm:Baptism`); Nazareth (ID: `place:Nazareth`); Glory (ID: `keyterm:Glory`); Blood (ID: `keyterm:Blood`); Call Upon (ID: `keyterm:CallUpon`); Prison (ID: `realia:Prison`); Cloak (ID: `realia:Cloak`); Synagogue (ID: `realia:Synagogue`); Temple (ID: `realia:Temple`)

## Acts 22:3 (id: 9503)

*at the feet of Gamaliel*: See study note on 5:34.

* **Associated Passages:** Acts 22:3
* **Associated Articles:** Acts 5:34 (ID: `9623@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Gamaliel (ID: `person:Gamaliel`); Tarsus (ID: `place:Tarsus`); Law (ID: `keyterm:Law`); Cilicia (ID: `place:Cilicia`); LORD (ID: `deity:Lord`); Jews (ID: `group:Jews`)

## Acts 22:12–16 (id: 9496)

*Ananias*: See study note on 9:17.

* **Associated Passages:** Acts 22:12-16
* **Associated Articles:** Acts 9:17 (ID: `9657@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Ananias (ID: `person:Ananias.2`); Bear Witness (ID: `keyterm:BearWitness`); Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Name (ID: `keyterm:Name`); Baptism (ID: `keyterm:Baptism`); Witness (ID: `keyterm:Witness`); Jews (ID: `group:Jews`); To Sin (ID: `keyterm:Sin`); Call Upon (ID: `keyterm:CallUpon`); Paul (ID: `person:Paul`)

## Acts 22:14 (id: 9497)

*the Righteous One*: See also 3:14; 7:52; 1 John 2:1. Righteousness was a key characteristic of the Messiah (see Isaiah 32:1; 53:11).

* **Associated Passages:** Acts 22:14

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## Acts 22:16 (id: 9498)

*be baptized, and wash your sins away*: See “Baptism” Theme Note.

* **Associated Passages:** Acts 22:16
* **Associated Articles:** Baptism (ID: `500380@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** To Sin (ID: `keyterm:Sin`); Baptism (ID: `keyterm:Baptism`); Name (ID: `keyterm:Name`); Call Upon (ID: `keyterm:CallUpon`)

## Acts 22:17–22 (id: 9499)

While Paul was praying in the temple*,* he saw a vision of Jesus. Jesus told him that the people of Jerusalem would not accept his message. The Lord then sent him to the Gentiles. The crowd’s reaction to this report confirmed this reality (22:22–23).

* **Associated Passages:** Acts 22:17-22

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Pray (ID: `keyterm:Pray`); Gentiles (ID: `group:Gentile`); Bear Witness (ID: `keyterm:BearWitness`); Stephen (ID: `person:Stephen`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Blood (ID: `keyterm:Blood`); Witness (ID: `keyterm:Witness`); Life (ID: `keyterm:Life`); Temple (ID: `realia:Temple`); Cloak (ID: `realia:Cloak`); Synagogue (ID: `realia:Synagogue`)

## Acts 22:23 (id: 9500)

The crowd shouted, threw off their coats, and tossed dust into the air. These actions were likely ritual responses to what they thought was blasphemy (speech that dishonors God). They strongly opposed Paul’s message, especially his claim that Gentiles were included among God's people (22:21). Compare with Luke 4:16–30.

* **Associated Passages:** Acts 22:23

* **Associated ACAI Concepts:** Cloak (ID: `realia:Cloak`)

## Acts 22:25–29 (id: 9501)

Paul asserted his Roman citizenship at a crucial moment when he encountered torture to make him confess a supposed crime. Roman citizenship was valuable. Claiming it falsely was a capital offense. Its main benefits included protection from whipping and the right to appeal to the emperor (25:11). The commander was afraid because he had almost violated Roman law (compare 16:35–39).

* **Associated Passages:** Acts 22:25-29

* **Associated ACAI Concepts:** Rome (ID: `place:Rome`); Military Member (ID: `person:MilitaryMember`); Paul (ID: `person:Paul`); Fear (ID: `keyterm:Fear`); Be Lawful (ID: `keyterm:Lawful`); Strap (ID: `realia:Strap`)

## Acts 22:28 (id: 9502)

*I paid a high price*: During the early rule of Roman emperor Claudius (AD 41–54\), people could buy Roman citizenship, but it was expensive.

* **Associated Passages:** Acts 22:28

* **Associated ACAI Concepts:** Military Member (ID: `person:MilitaryMember`); Paul (ID: `person:Paul`)

## Acts 23:1 (id: 9507)

When speaking to the Jewish high council (the Sanhedrin), Paul said he had lived with a clear conscience before God. He had not broken God’s law or done what they accused him of doing.

* **Associated Passages:** Acts 23:1

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Conscience (ID: `keyterm:Conscience`); Good (ID: `keyterm:Good`); Live (ID: `keyterm:Live.3`); Paul (ID: `person:Paul`)

## Acts 23:2 (id: 9511)

* Ananias was the Jewish high priest from AD 47 to 58\.
* *strike him:* Ananias had the apostle struck because he thought Paul was lying and wanted to scare him.

* **Associated Passages:** Acts 23:2

* **Associated ACAI Concepts:** Ananias (ID: `person:Ananias.3`)

## Acts 23:3 (id: 9514)

*you whitewashed wall*: Compare Ezekiel 13:10–17; Matthew 23:27.

* **Associated Passages:** Acts 23:3

* **Associated ACAI Concepts:** Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Decided (ID: `keyterm:Decided`); Disobey (ID: `keyterm:Disobey`); Paul (ID: `person:Paul`)

## Acts 23:5 (id: 9515)

* It is unclear why Paul did not recognize the high priest.
* Paul responded by quoting Exodus 22:28, showing respect for the high priest's position.

* **Associated Passages:** Acts 23:5

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`)

## Acts 23:6 (id: 9516)

Paul focused on the main issue in his trial: the hope of the resurrection (the belief that the dead will be raised to life). His preaching came from this hope and from the fact that Jesus had risen from the dead.

However, his message was not accepted by either group of Jews because of what it meant. The Pharisees believed in the resurrection, but they could not accept that Gentiles could be included among God’s people without circumcision and without keeping the law of Moses (see 15:5). Yet this is exactly what the resurrection of Jesus and the gift of the Holy Spirit had made possible (see Acts 2:39; 10:34–48). 

The Sadducees, on the other hand, did not believe in the resurrection at all. Because of this, they strongly opposed any teaching about the resurrection (see 4:1–2).

* **Associated Passages:** Acts 23:6

* **Associated ACAI Concepts:** Resurrection (ID: `keyterm:Resurrection`); Pharisees (ID: `group:Pharisee`); Hope (ID: `keyterm:Hope`); Paul (ID: `person:Paul`); Decided (ID: `keyterm:Decided`); Sadducees (ID: `group:Sadducee`)

## Acts 23:7–10 (id: 9517)

Paul's statement divided the Sanhedrin (23:6). The Pharisees supported Paul against the Sadducees. The disagreement was so intense that the commander rescued Paul and took him back to the fortress of Antonia.

* **Associated Passages:** Acts 23:7-10

* **Associated ACAI Concepts:** Pharisees (ID: `group:Pharisee`); Sadducees (ID: `group:Sadducee`); An Angel (ID: `deity:Angel`); Fear (ID: `keyterm:Fear`); Resurrection (ID: `keyterm:Resurrection`); Paul (ID: `person:Paul`)

## Acts 23:11 (id: 9504)

At this important moment, Jesus encouraged Paul to continue as his faithful witness by assuring him that he would go to Rome (see 19:21).

* **Associated Passages:** Acts 23:11

* **Associated ACAI Concepts:** Cheer Up (ID: `keyterm:CheerUp`); Rome (ID: `place:Rome`); Bear Witness (ID: `keyterm:BearWitness`); Declare (ID: `keyterm:Declare`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`)

## Acts 23:12–15 (id: 9505)

The plan to kill Paul was desperate. More than 40 men took an oath (a strong promise) not to eat or drink until they had killed him (compare 1 Samuel 14:24–46; Matthew 14:6–11; see also Deuteronomy 23:21–23; Matthew 5:33–37; James 5:12).

* **Associated Passages:** Acts 23:12-15

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Decision (ID: `keyterm:Decision`); Jews (ID: `group:Jews`)

## Acts 23:16–22 (id: 9506)

Paul’s nephew stopped the plan to kill him by telling a Roman officer.

* **Associated Passages:** Acts 23:16-22

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Ask for (ID: `keyterm:AskFor`); Persuade (ID: `keyterm:Persuade`); Military Member (ID: `person:MilitaryMember`); Promise (ID: `keyterm:Promise`); Jews (ID: `group:Jews`)

## Acts 23:23–35 (id: 9508)

A large group riding horses safely brought Paul to the Roman governor Felix in Caesarea, the Roman headquarters for Judea. Paul would have more protection there than in Jerusalem. The soldiers carried out the mission that night with great care and secrecy (23:31).

* **Associated Passages:** Acts 23:23-35

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Felix (ID: `person:Felix`); Caesarea (ID: `place:Caesarea`); Cilicia (ID: `place:Cilicia`); Antipatris (ID: `place:Antipatris`); Lysias (ID: `person:Lysias`); Law (ID: `keyterm:Law`); Rescue (ID: `keyterm:Rescue`); Herod (ID: `person:Herod`); Rome (ID: `place:Rome`); Salvation (ID: `keyterm:Salvation`); Jews (ID: `group:Jews`); Palace (ID: `realia:Palace`)

## Acts 23:24 (id: 9509)

Antonius Felix was the governor of Judea from about AD 52 to 59\. He was in charge of both military and civil matters. Felix had a bad reputation and was eventually called back to Rome by Emperor Nero (see 24:24–27).

* **Associated Passages:** Acts 23:24

* **Associated ACAI Concepts:** Felix (ID: `person:Felix`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`)

## Acts 23:26–30 (id: 9510)

* The letter from Claudius Lysias to Governor Felix follows the common form of a Hellenistic letter (a standard Greek\-style letter used in that time). It names the writer and the recipient, gives a greeting, and explains the main issue. However, it does not include a closing farewell (see study note on 15:29).

    The letter also summarizes the events leading up to it and explains the actions taken. It shows that the commander believed he had followed proper Roman legal procedures.

* *His Excellency*: This title was often used for people with high social, political, or economic status (Luke 1:3).

* **Associated Passages:** Acts 23:26-30
* **Associated Articles:** Acts 15:29 (ID: `9373@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Lysias (ID: `person:Lysias`); Law (ID: `keyterm:Law`); Rescue (ID: `keyterm:Rescue`); Felix (ID: `person:Felix`); Rome (ID: `place:Rome`); Jews (ID: `group:Jews`)

## Acts 23:31 (id: 9512)

Antipatris was a city rebuilt by Herod the Great in 9 BC on the Plain of Sharon. It served as a strategic military control point between Jerusalem and Caesarea.

* **Associated Passages:** Acts 23:31

* **Associated ACAI Concepts:** Antipatris (ID: `place:Antipatris`); Paul (ID: `person:Paul`)

## Acts 23:35 (id: 9513)

* The governor followed the correct steps and waited for Paul's accusers to arrive before having an official legal hearing.
* *Herod's Praetorium:* Herod’s headquarters was the palace built by Herod the Great in Caesarea. Later, this palace became the residence (official home) of the Roman governors of Judea.

* **Associated Passages:** Acts 23:35

* **Associated ACAI Concepts:** Palace (ID: `realia:Palace`); Herod (ID: `person:Herod`)

## Acts 24:1–4 (id: 9519)

Tertullus presented the case for the prosecution. He began with the usual words of praise to gain the governor’s attention and favor. Then he listed the charges against Paul (24:5–8).

* **Associated Passages:** Acts 24:1-4

* **Associated ACAI Concepts:** Tertullus (ID: `person:Tertullus`); Felix (ID: `person:Felix`); Ananias (ID: `person:Ananias.3`); Ask for (ID: `keyterm:AskFor`); Make Peace (ID: `keyterm:Peace`); Kindness (ID: `keyterm:Kindness`); Paul (ID: `person:Paul`)

## Acts 24:1–27 (id: 9518)

Tertullus presented a legal case against Paul in a Roman court on behalf of the high priest (24:1–9). Then Paul cheerfully defended himself and his faith (24:10–21). The governor paused the hearing without making a decision. He kept Paul in prison for two years (24:22–27).

* **Associated Passages:** Acts 24:1-27

* **Associated ACAI Concepts:** Felix (ID: `person:Felix`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`); LORD (ID: `deity:Lord`); Tertullus (ID: `person:Tertullus`); The Way (ID: `keyterm:TheWay`); Temple (ID: `realia:Temple`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Resurrection (ID: `keyterm:Resurrection`); Hope (ID: `keyterm:Hope`); Serve (ID: `keyterm:Serve`); Believe (ID: `keyterm:Believe`); Lysias (ID: `person:Lysias`); Conscience (ID: `keyterm:Conscience`); Drusilla (ID: `person:Drusilla`); Festus (ID: `person:Festus`); Ananias (ID: `person:Ananias.3`); Discern (ID: `keyterm:Discern`); Bow Down (ID: `keyterm:BowDown`); Defilement (ID: `keyterm:Defilement`); Favor (ID: `keyterm:Favor`); Decision (ID: `keyterm:Decision`); Pure (ID: `keyterm:Pure`); Priesthood (ID: `keyterm:Priesthood`); Make Peace (ID: `keyterm:Peace`); Unrighteous Act (ID: `keyterm:UnrighteousAct`); Judgment (ID: `keyterm:Judgment`); Kindness (ID: `keyterm:Kindness`); Asia (ID: `place:Asia`); Law (ID: `keyterm:Law`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Nazareth (ID: `place:Nazareth`); Ability (ID: `keyterm:Ability`); Ask for (ID: `keyterm:AskFor`); Decided (ID: `keyterm:Decided`); Money (ID: `realia:Money`); Synagogue (ID: `realia:Synagogue`)

## Acts 24:5 (id: 9531)

* *a pestilence, stirring up dissension among the Jews all over the world*: A Roman court would have taken this charge very seriously. This language presents him as someone who causes unrest. It reflects a charge of political sedition (acting against the state), which a Roman court would have treated as a serious crime (see 16:21; 17:7; 18:13). People made similar accusations against Jesus to Pontius Pilate (Luke 23:2, 5, 14).
* *sect:* This term is used here in a negative way. It suggests that Paul's faith was suspicious or even unlawful (see also Acts 24:14).

* **Associated Passages:** Acts 24:5

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Nazareth (ID: `place:Nazareth`); Ability (ID: `keyterm:Ability`)

## Acts 24:6 (id: 9532)

*desecrate the temple*: See study note on 21:28–29.

* **Associated Passages:** Acts 24:6
* **Associated Articles:** Acts 21:28-29 (ID: `9486@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Temple (ID: `realia:Temple`)

## Acts 24:10–21 (id: 9520)

Paul defended himself by saying: 

1. he was not in Jerusalem long enough to start a riot;
2. none of those who accused him saw him causing a riot; and
3. he worshiped according to Jewish law and the writings of the prophets.

* **Associated Passages:** Acts 24:10-21

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Resurrection (ID: `keyterm:Resurrection`); Temple (ID: `realia:Temple`); The Way (ID: `keyterm:TheWay`); Conscience (ID: `keyterm:Conscience`); Bow Down (ID: `keyterm:BowDown`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Hope (ID: `keyterm:Hope`); Pure (ID: `keyterm:Pure`); Priesthood (ID: `keyterm:Priesthood`); Jews (ID: `group:Jews`); Serve (ID: `keyterm:Serve`); Unrighteous Act (ID: `keyterm:UnrighteousAct`); Paul (ID: `person:Paul`); Asia (ID: `place:Asia`); Law (ID: `keyterm:Law`); Believe (ID: `keyterm:Believe`); Jerusalem (ID: `place:Jerusalem`); Decided (ID: `keyterm:Decided`); Synagogue (ID: `realia:Synagogue`)

## Acts 24:14 (id: 9521)

* *the Way*: See study note on 9:2.
* Paul emphasized what he shared in common with his Jewish audience, including his worship*,* belief in the Jewish law*,* acceptance of the prophets*,* and hope in the resurrection (24:14, 15; see 24:21).

* **Associated Passages:** Acts 24:14
* **Associated Articles:** Acts 9:2 (ID: `9662@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** The Way (ID: `keyterm:TheWay`); Serve (ID: `keyterm:Serve`); Law (ID: `keyterm:Law`); LORD (ID: `deity:Lord`); Believe (ID: `keyterm:Believe`); Priesthood (ID: `keyterm:Priesthood`)

## Acts 24:15 (id: 9522)

At the final judgment (the time when God judges all people), God will raise both the righteous (those who follow him) and the unrighteous (those who do not). Paul kept this coming meeting with God in mind at all times (24:16). Felix became afraid when Paul spoke about “the coming judgment” in a later conversation (24:25).

* **Associated Passages:** Acts 24:15

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Hope (ID: `keyterm:Hope`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Resurrection (ID: `keyterm:Resurrection`)

## Acts 24:16 (id: 9523)

Paul emphasized that he had acted with a clear conscience (see 20:27, 33; 23:1; 1 Corinthians 4:4; 2 Corinthians 1:12; 4:2; see also 2 Timothy 1:3). He remained true to his Jewish heritage and did not fear God's judgment (Acts 24:14,15).

* **Associated Passages:** Acts 24:16

* **Associated ACAI Concepts:** Conscience (ID: `keyterm:Conscience`); LORD (ID: `deity:Lord`)

## Acts 24:17 (id: 9524)

* *alms to my people*: See study note on 20:1–2; also see Romans 15:25–29; 1 Corinthians 16:1–2; 2 Corinthians 9:1–7.
* *to present offerings*: See Acts 21:23–26.

* **Associated Passages:** Acts 24:17
* **Associated Articles:** Acts 20:1-2 (ID: `9463@AquiferOpenStudyNotes`)


## Acts 24:22 (id: 9525)

The governorFelix might have delayed his decision, hoping Paul would offer him a bribe (24:26).

* **Associated Passages:** Acts 24:22

* **Associated ACAI Concepts:** Lysias (ID: `person:Lysias`); Decision (ID: `keyterm:Decision`); Felix (ID: `person:Felix`); The Way (ID: `keyterm:TheWay`)

## Acts 24:23 (id: 9526)

Proper treatment of a Roman citizen involved giving him some freedom and allowing his friends to visit and care for him.

* **Associated Passages:** Acts 24:23

* **Associated ACAI Concepts:** Serve (ID: `keyterm:Serve`)

## Acts 24:24 (id: 9527)

Drusilla was the sister of Herod Agrippa II and Bernice (25:13). She left her first husband, Azizus, the King of Emessa, to marry Felix. Drusilla was Jewish, so by leaving her original husband and marrying Felix, she violated God's commands (Exodus 20:14; Deuteronomy 5:18; compare Malachi 2:16; Mark 10:12). The Jewish historian Josephus criticized her for doing this (*Antiquities* 20\.7\.1–2\). During their conversation with Paul, Drusilla and her husband were faced with the reality of God’s judgment (his just decision about human actions).

* **Associated Passages:** Acts 24:24

* **Associated ACAI Concepts:** Drusilla (ID: `person:Drusilla`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Felix (ID: `person:Felix`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 24:25 (id: 9528)

Paul's words about righteousness, self\-control, and the coming judgment scared Felix, who was notably corrupt (24:26–27; see study notes on 23:24; 24:24).

* **Associated Passages:** Acts 24:25
* **Associated Articles:** Acts 23:24 (ID: `9509@AquiferOpenStudyNotes`); Acts 24:24 (ID: `9527@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Felix (ID: `person:Felix`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Judgment (ID: `keyterm:Judgment`)

## Acts 24:26–27 (id: 9529)

Felix kept Paul in custody, hoping that Paul would offer him a bribe. When this did not work and his term as governor ended, he left Paul in prison to please the Jewish people.

* **Associated Passages:** Acts 24:26-27

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Festus (ID: `person:Festus`); Felix (ID: `person:Felix`); Favor (ID: `keyterm:Favor`); Hope (ID: `keyterm:Hope`); Jews (ID: `group:Jews`); Money (ID: `realia:Money`)

## Acts 24:27–25.5 (id: 9530)

Felix’s successor was Porcius Festus. Nero appointed him as governor of Judea around AD 59–62\. Josephus describes Festus as a careful and honest leader. However, he could not stop the growing unrest among the Jewish people, even though he took strong action against the Assassins (a violent group also called the Sicarii; Josephus, *Antiquities* 20\.8\.10; see also Acts 21:38). 

Festus refused the Jewish leaders’ request to move Paul’s trial to Jerusalem, but he still felt their pressure (25:9).

* **Associated Passages:** Acts 24:27-25:5

* **Associated ACAI Concepts:** Festus (ID: `person:Festus`); Paul (ID: `person:Paul`); Caesarea (ID: `place:Caesarea`); Favor (ID: `keyterm:Favor`); Jerusalem (ID: `place:Jerusalem`); Jews (ID: `group:Jews`); Felix (ID: `person:Felix`); Ask for (ID: `keyterm:AskFor`)

## Acts 25:7 (id: 9540)

The serious accusations were likely the ones mentioned earlier (21:27–28; 24:5–9). But there was no evidence to support them (24:10–13).

* **Associated Passages:** Acts 25:7

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`)

## Acts 25:9 (id: 9541)

*Festus, wishing to do the Jews a favor*: The governor later gave an official reason for delaying Paul's trial and asking to move it to Jerusalem (25:17–20). However, politics, not justice, motivated his request.

* **Associated Passages:** Acts 25:9

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Festus (ID: `person:Festus`); Jerusalem (ID: `place:Jerusalem`); Decided (ID: `keyterm:Decided`); Jews (ID: `group:Jews`); Paul (ID: `person:Paul`)

## Acts 25:10–11 (id: 9533)

Festus suggested that Paul have his trial in Jerusalem, which led Paul to appeal to Caesar. Paul did not fear death, but he opposed being transferred to a court biased toward condemnation, not justice (25:7).

* **Associated Passages:** Acts 25:10-11

* **Associated ACAI Concepts:** To Act Wickedly (ID: `keyterm:ActWickedly`); Claudius (ID: `person:Claudius`); Forgive (ID: `keyterm:Forgive`); Decided (ID: `keyterm:Decided`); Jews (ID: `group:Jews`); Call Upon (ID: `keyterm:CallUpon`); Paul (ID: `person:Paul`); Judgment Seat (ID: `realia:JudgmentSeat`)

## Acts 25:12 (id: 9534)

Festus approved Paul's request. This confirmed Paul's belief that he must visit Rome (19:21; see also 23:11; 27:24; Romans 1:13–15; 15:22–29).

* **Associated Passages:** Acts 25:12

* **Associated ACAI Concepts:** Festus (ID: `person:Festus`); Call Upon (ID: `keyterm:CallUpon`); Claudius (ID: `person:Claudius`)

## Acts 25:13–22 (id: 9535)

Festus discussed Paul’s case with Herod Agrippa II, who came to Caesarea to pay a courtesy visit to the new governor. Herod Agrippa II ruled parts of the region from AD 50–100\. 

* **Associated Passages:** Acts 25:13-22

* **Associated ACAI Concepts:** Festus (ID: `person:Festus`); Paul (ID: `person:Paul`); Agrippa (ID: `person:Agrippa`); Jerusalem (ID: `place:Jerusalem`); Augustus (ID: `person:Augustus`); Bernice (ID: `person:Bernice`); Decision (ID: `keyterm:Decision`); Jews (ID: `group:Jews`); Forgive (ID: `keyterm:Forgive`); Life (ID: `keyterm:Life`); Caesarea (ID: `place:Caesarea`); Jesus (ID: `person:Jesus.2`); Rome (ID: `place:Rome`); Claudius (ID: `person:Claudius`); Felix (ID: `person:Felix`); Decided (ID: `keyterm:Decided`); Call Upon (ID: `keyterm:CallUpon`); Condemn (ID: `keyterm:Condemn`); Judgment Seat (ID: `realia:JudgmentSeat`)

## Acts 25:16 (id: 9536)

This was a fundamental principle: Roman law did not convict people without a trial. They must be given an opportunity to confront their accusers and defend themselves (see 19:38–39). This showed Roman legal procedures in a positive way.

* **Associated Passages:** Acts 25:16

* **Associated ACAI Concepts:** Rome (ID: `place:Rome`); Forgive (ID: `keyterm:Forgive`)

## Acts 25:17 (id: 9537)

*I did not delay*: This is different from Felix, the previous governor, who delayed making a decision (24:22–27).

* **Associated Passages:** Acts 25:17

* **Associated ACAI Concepts:** Judgment Seat (ID: `realia:JudgmentSeat`)

## Acts 25:18–20 (id: 9538)

These verses provide the official reasons Festus gave for his actions (see also 25:9). There were no criminal charges against Paul. The objections were about their religion and focused on Paul's claim that Jesus is alive.

* **Associated Passages:** Acts 25:18-20

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Decided (ID: `keyterm:Decided`); Life (ID: `keyterm:Life`); Paul (ID: `person:Paul`)

## Acts 25:23–27 (id: 9539)

Paul's legal hearing before King Agrippa included all the ceremony of a royal visit. The main purpose was for Agrippa to advise Festus on what to write in the appeal to Caesar. There was no clear charge against Paul. Festus believed Paul had done nothing deserving death.

* **Associated Passages:** Acts 25:23-27

* **Associated ACAI Concepts:** Agrippa (ID: `person:Agrippa`); Festus (ID: `person:Festus`); Bernice (ID: `person:Bernice`); Discern (ID: `keyterm:Discern`); Augustus (ID: `person:Augustus`); Jerusalem (ID: `place:Jerusalem`); Decided (ID: `keyterm:Decided`); Life (ID: `keyterm:Life`); Jews (ID: `group:Jews`); Call Upon (ID: `keyterm:CallUpon`); Paul (ID: `person:Paul`); Audience Hall (ID: `realia:AudienceHall`)

## Acts 26:1–23 (id: 9542)

In his careful defense before King Agrippa, Paul argued that his message agreed fully with the Jewish faith. He began by respectfully recognizing Agrippa’s knowledge of Jewish matters (26:2–3). He then described his early life, his training in Judaism, and his membership in the Pharisees (a Jewish group known for strict obedience to the law; 26:4–5). 

Paul explained that the charges against him were only because he believed that God had fulfilled the Jewish hope of the resurrection (God raising the dead to life; 26:6–8). 

He then told how he had once strongly opposed Christians (26:9–11), but changed after seeing a vision from Jesus on the road to Damascus (26:12–18; see 9:1–18). 

Paul said that his preaching simply obeyed this message from God (26:19–20). Although some Jews opposed him violently (26:21\), God protected him. He continued to teach a message that his fellow Jews should have accepted(22–23). This defense shows an example for Christians who are put on trial for their faith (see 9:15; Luke 21:12–15).

* **Associated Passages:** Acts 26:1-23

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); LORD (ID: `deity:Lord`); Agrippa (ID: `person:Agrippa`); Gentiles (ID: `group:Gentile`); Jesus (ID: `person:Jesus.2`); Jerusalem (ID: `place:Jerusalem`); Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`); Resurrection (ID: `keyterm:Resurrection`); Hope (ID: `keyterm:Hope`); Damascus (ID: `place:Damascus`); Paul (ID: `person:Paul`); Decided (ID: `keyterm:Decided`); Rescue (ID: `keyterm:Rescue`); Pharisees (ID: `group:Pharisee`); Witness (ID: `keyterm:Witness`); Priesthood (ID: `keyterm:Priesthood`); To Sin (ID: `keyterm:Sin`); Serve (ID: `keyterm:Serve`); Heavenly (ID: `keyterm:Heavenly`); Forgive (ID: `keyterm:Forgive`); Life (ID: `keyterm:Life`); Holy (ID: `keyterm:Holy`); From Heaven (ID: `keyterm:FromHeaven`); Declare (ID: `keyterm:Declare`); Satan (ID: `deity:Satan`); Believe (ID: `keyterm:Believe`); Name (ID: `keyterm:Name`); Judea (ID: `place:Judea`); Moses (ID: `person:Moses`); Happy (ID: `keyterm:Happy`); Repent (ID: `keyterm:Repent`); Bear Witness (ID: `keyterm:BearWitness`); Nazareth (ID: `place:Nazareth`); Blaspheme (ID: `keyterm:Blaspheme`); Promise (ID: `keyterm:Promise`); Daily Life (ID: `keyterm:DailyLife`); Consecration (ID: `keyterm:Consecration.2`); Prison (ID: `realia:Prison`); Synagogue (ID: `realia:Synagogue`); Temple (ID: `realia:Temple`)

## Acts 26:12–18 (id: 9543)

See 9:1–18.

* **Associated Passages:** Acts 26:12-18

* **Associated ACAI Concepts:** Ability (ID: `keyterm:Ability`); Authority (ID: `keyterm:Authority.2`); Rescue (ID: `keyterm:Rescue`); Gentiles (ID: `group:Gentile`); LORD (ID: `deity:Lord`); Forgive (ID: `keyterm:Forgive`); Satan (ID: `deity:Satan`); Jesus (ID: `person:Jesus.2`); Believe (ID: `keyterm:Believe`); From Heaven (ID: `keyterm:FromHeaven`); Witness (ID: `keyterm:Witness`); To Sin (ID: `keyterm:Sin`); Damascus (ID: `place:Damascus`); Consecration (ID: `keyterm:Consecration.2`); Paul (ID: `person:Paul`)

## Acts 26:17–18 (id: 9544)

Throughout his defense in this trial for his life, Paul also clearly set out the conditions for receiving new life in Christ (also in 26:20, 23; see Matthew 10:19–20).

* **Associated Passages:** Acts 26:17-18

* **Associated ACAI Concepts:** Rescue (ID: `keyterm:Rescue`); Gentiles (ID: `group:Gentile`); Ability (ID: `keyterm:Ability`); LORD (ID: `deity:Lord`); Satan (ID: `deity:Satan`); Authority (ID: `keyterm:Authority.2`); Believe (ID: `keyterm:Believe`); Forgive (ID: `keyterm:Forgive`); To Sin (ID: `keyterm:Sin`); Consecration (ID: `keyterm:Consecration.2`)

## Acts 26:22–23 (id: 9545)

Paul stressed that God protected him as he carried out his witness (compare 3:18; 10:43; Luke 24:25–27, 44–47). Paul urged his listeners to believe that Jesus is the Christ who fulfills Old Testament promises (see Luke 24:27, 44).

* **Associated Passages:** Acts 26:22-23

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Declare (ID: `keyterm:Declare`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Resurrection (ID: `keyterm:Resurrection`); Moses (ID: `person:Moses`)

## Acts 26:24 (id: 9546)

*You are insane, Paul!*: Festus, a Roman, thought Paul's talk about the prophets and resurrection was crazy (compare 17:18, 32). He concluded that Paul had studied so much it drove him to madness.

* **Associated Passages:** Acts 26:24

* **Associated ACAI Concepts:** Festus (ID: `person:Festus`); Paul (ID: `person:Paul`)

## Acts 26:26 (id: 9547)

*it was not done in a corner*: The main events of the Christian faith were public historical events. Witnesses could confirm them as true. King Agrippa could not contradict Paul's statements of fact.

* **Associated Passages:** Acts 26:26

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Cornerstone (ID: `realia:Cornerstone`)

## Acts 26:27–28 (id: 9548)

Paul's question put King Agrippa in a difficult position. If Agrippa said he believed the prophets, Paul would press the Christian message. If Agrippa denied belief in the prophets, he would upset the devout Jews who were listening. 

Agrippa knew Paul was not crazy and that Paul's testimony about Jesus was historically accurate (26:26). So, Agrippa avoided answering Paul's question and refused to consider the claims of Christ. Agrippa said that Paul's statement was too short for him to make a responsible decision.

* **Associated Passages:** Acts 26:27-28

* **Associated ACAI Concepts:** Agrippa (ID: `person:Agrippa`); Christians (ID: `group:Christ`); Persuade (ID: `keyterm:Persuade`); Believe (ID: `keyterm:Believe`); Jesus (ID: `person:Jesus.2`); Paul (ID: `person:Paul`)

## Acts 26:28 (id: 9549)

*"**Can you persuade me in such a short time to become a Christian?**"*: This remark might have been ironic, expressing a lack of belief. It might have been making fun of Paul, or a dismissal of Paul's challenge. It might also have been a comment on how well Paul could persuade. Or it could have been a genuine question about Paul's intention. It seems best to understand King Agrippa's reply as intentionally trying to avoid the question. He did not want to admit he believed the prophets, because Paul had made a strong case (26:27). 

The next step would be to believe in Jesus as the promised Christ, which Agrippa did not want to do. On the other hand, he did not want to say he did not believe the prophets. This would cause his Jewish subjects to feel isolated, and Agrippa wanted their allegiance. His lack of a committed response emphasizes his discomfort with Paul's testimony.

* **Associated Passages:** Acts 26:28

* **Associated ACAI Concepts:** Christians (ID: `group:Christ`); Persuade (ID: `keyterm:Persuade`); Agrippa (ID: `person:Agrippa`); Jesus (ID: `person:Jesus.2`); Paul (ID: `person:Paul`)

## Acts 26:29 (id: 9550)

Paul’s bold answer shows that he could respond quickly and clearly. He challenges Agrippa and the whole audience to think about the value of knowing Christ and making a personal commitment to him.

* **Associated Passages:** Acts 26:29

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`); Chain (ID: `realia:Chain`)

## Acts 26:31 (id: 9551)

The rulers agreed that Paul did not deserve death or imprisonment. Roman authorities repeatedly gave this verdict when considering Paul's case (25:25; see Luke 23:4, 15, 22).

* **Associated Passages:** Acts 26:31


## Acts 26:32 (id: 9552)

Paul could have been set free because the legal verdict was evident (26:31). However, if he had not appealed to Caesar, Paul might not have survived (25:1–11). By appealing, he was fulfilling God's purposes for him (23:11).

* **Associated Passages:** Acts 26:32

* **Associated ACAI Concepts:** Agrippa (ID: `person:Agrippa`); Festus (ID: `person:Festus`); Call Upon (ID: `keyterm:CallUpon`); Claudius (ID: `person:Claudius`)

## Acts 27:1 (id: 9559)

* Julius is not known from other sources.
* *the Imperial Regiment*: This group of soldiers was stationed in Syria during this period (see study note on 10:1–8).

* **Associated Passages:** Acts 27:1
* **Associated Articles:** Acts 10:1-8 (ID: `9292@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Italy (ID: `place:Italy`); Augustus (ID: `person:Augustus`); Julius (ID: `person:Julius`); Name (ID: `keyterm:Name`); Decided (ID: `keyterm:Decided`); Paul (ID: `person:Paul`)

## Acts 27:1–28.16 (id: 9553)

* The graphic language describing sailing in the story of Paul's journey to Rome provides one of the best accounts of an ancient sea journey.
* This is the last "we" section in Acts (see also 16:10–17; 20:5–15; 21:1–18). During Paul's two years of imprisonment, Luke probably researched his gospel throughout Judea and Galilee. Here, as part of Paul's sailing group, he had a personal witness of the dangers at sea.

* **Associated Passages:** Acts 27:1-28:16

* **Associated ACAI Concepts:** Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`); Salvation (ID: `keyterm:Salvation`); LORD (ID: `deity:Lord`); Crete (ID: `place:Crete`); Life (ID: `keyterm:Life`); Fear (ID: `keyterm:Fear`); Ask for (ID: `keyterm:AskFor`); Julius (ID: `person:Julius`); Alexandria (ID: `place:Alexandria`); Publius (ID: `person:Publius`); Anchor (ID: `realia:Anchor`); Rome (ID: `place:Rome`); Italy (ID: `place:Italy`); Pray (ID: `keyterm:Pray`); Serve (ID: `keyterm:Serve`); Name (ID: `keyterm:Name`); Fair Havens (ID: `place:FairHavens`); Augustus (ID: `person:Augustus`); Phoenix (ID: `place:Phoenix`); Sidon (ID: `place:Sidon`); Malta (ID: `place:Malta`); Lasea (ID: `place:Lasea`); Cilicia (ID: `place:Cilicia`); Pamphylia (ID: `place:Pamphylia`); Cnidus (ID: `place:Cnidus`); Three Taverns (ID: `place:ThreeTaverns`); Cauda (ID: `place:Cauda`); Rhegium (ID: `place:Rhegium`); Puteoli (ID: `place:Puteoli`)

## Acts 27:2 (id: 9562)

* Aristarchus was from Thessalonica and worked with Paul in Asia (see 19:29; 20:4, 6; Philemon 1:24).
* Adramyttium was a seaport on the west coast of Asia Minor, southeast of Troas.

* **Associated Passages:** Acts 27:2

* **Associated ACAI Concepts:** Adramyttium (ID: `place:Adramyttium`); Asia (ID: `place:Asia`); Aristarchus (ID: `person:Aristarchus`); Macedonia (ID: `place:Macedonia`); Thessalonians (ID: `group:Thessalonica`); Thessalonica (ID: `place:Thessalonica`); Ship (ID: `realia:Ship`)

## Acts 27:3 (id: 9567)

Sidon was a city on the coast, about 110 kilometers (70 miles) north of Caesarea. It was the ship’s first stop. Julius (the Roman officer in charge) treated Paul kindly and allowed his Christian friends there to care for him.

* **Associated Passages:** Acts 27:3

* **Associated ACAI Concepts:** Julius (ID: `person:Julius`); Sidon (ID: `place:Sidon`); Paul (ID: `person:Paul`)

## Acts 27:4–6 (id: 9568)

* Luke gives a detailed description of the sea journey. The ship traveled north and then west along the southern coast of Asia Minor (modern\-day Turkey).
* Myra was an important stop. Ships from Egypt carrying grain (food crops like wheat) to Italy regularly stopped there.

* **Associated Passages:** Acts 27:4-6

* **Associated ACAI Concepts:** Lycia (ID: `place:Lycia`); Alexandria (ID: `place:Alexandria`); Cilicia (ID: `place:Cilicia`); Pamphylia (ID: `place:Pamphylia`); Myra (ID: `place:Myra`); Italy (ID: `place:Italy`); Cyprus (ID: `place:Cyprus`); Ship (ID: `realia:Ship`)

## Acts 27:7 (id: 9570)

* The difficulty was partly because it was late in the season (27:9).
* Cnidus was a seaport on the southwestern coast of Asia Minor, near the island of Cos.
* The cape of Salmone was at the northeastern tip of Crete, the largest island in Greece.

* **Associated Passages:** Acts 27:7

* **Associated ACAI Concepts:** Crete (ID: `place:Crete`); Cnidus (ID: `place:Cnidus`); Salmone (ID: `place:Salmone`)

## Acts 27:8 (id: 9571)

Fair Havens is a small bay on the south side of the island of Crete.

* **Associated Passages:** Acts 27:8

* **Associated ACAI Concepts:** Fair Havens (ID: `place:FairHavens`); Lasea (ID: `place:Lasea`)

## Acts 27:9 (id: 9572)

*because it was after the Fast:* It was late in the fall, which meant travel on the Mediterranean Sea was dangerous.

* **Associated Passages:** Acts 27:9

* **Associated ACAI Concepts:** Abstain from Food (ID: `keyterm:Fast`); Paul (ID: `person:Paul`)

## Acts 27:10–11 (id: 9554)

Paul understood the consequences if they continued. He warned the ship's officers. But they and the Roman officer were not likely to listen to an imprisoned Jewish rabbi with no sailing experience. Later, however, they would respect him more (27:30–36, 42–43).

* **Associated Passages:** Acts 27:10-11

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Life (ID: `keyterm:Life`); Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`)

## Acts 27:12 (id: 9555)

The winds from the southeast made Fair Havens not a safe harbor for winter. Phoenix, a town further up the coast of Crete, offered a better harbor.

* **Associated Passages:** Acts 27:12

* **Associated ACAI Concepts:** Phoenix (ID: `place:Phoenix`); Crete (ID: `place:Crete`)

## Acts 27:14–16 (id: 9556)

*the Northeaster*: This storm had the strength of a hurricane or cyclone. It threatened both the cargo and the crew. Forced to let the ship run in front of the storm, they sailed past a small island called Cauda (known today as Gaudos), south of Crete.

* **Associated Passages:** Acts 27:14-16

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`); Cauda (ID: `place:Cauda`)

## Acts 27:17 (id: 9557)

* The purpose of wrapping ropes around the ship's framework (hull) was to strengthen it against the storm's pressure. This process is called *frapping*.
* *Syrtis* refers to the shallow bays with *sandbars* off the North African coast, west of Cyrene.

* **Associated Passages:** Acts 27:17

* **Associated ACAI Concepts:** the Syrtis (ID: `place:Syrtis`); Fear (ID: `keyterm:Fear`); Ship (ID: `realia:Ship`)

## Acts 27:18–20 (id: 9558)

The violent and persistent storm forced the crew to throw the cargo over the side of the boat (compare Jonah 1:5). The crew had lost hope.

* **Associated Passages:** Acts 27:18-20

* **Associated ACAI Concepts:** Hope (ID: `keyterm:Hope`); Salvation (ID: `keyterm:Salvation`); Ship (ID: `realia:Ship`)

## Acts 27:21–26 (id: 9560)

Paul spoke to the crew. He first criticized them for not listening to him (27:10–12). Then, he encouraged them by sharing the angel's assurance that everyone would survive.

* **Associated Passages:** Acts 27:21-26

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`); Serve (ID: `keyterm:Serve`); Fear (ID: `keyterm:Fear`); Forgive (ID: `keyterm:Forgive`); Believe (ID: `keyterm:Believe`); Priesthood (ID: `keyterm:Priesthood`); Life (ID: `keyterm:Life`); Crete (ID: `place:Crete`); Claudius (ID: `person:Claudius`); An Angel (ID: `deity:Angel`); Ship (ID: `realia:Ship`)

## Acts 27:27 (id: 9561)

The Adriatic Sea is also known as the Ionian Sea. It is south of Italy and Greece, between Malta and Crete.

* **Associated Passages:** Acts 27:27

* **Associated ACAI Concepts:** Adriatic Sea (ID: `place:AdriaticSea`)

## Acts 27:30–32 (id: 9563)

This time the soldiers listened to Paul (compare 27:10–11).

* **Associated Passages:** Acts 27:30-32

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`); Anchor (ID: `realia:Anchor`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`); Rope (ID: `realia:Rope`)

## Acts 27:33–35 (id: 9564)

Paul acts like a true leader. He personally evaluates a dangerous situation, decides on a plan, and guides others to solve the problem (compare Nehemiah 1–3; contrast with Jonah 1). Paul’s positive example and strong faith in God encouraged the others to eat and remain hopeful (Acts 27:22–25).

* **Associated Passages:** Acts 27:33-35

* **Associated ACAI Concepts:** Ask for (ID: `keyterm:AskFor`); LORD (ID: `deity:Lord`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`)

## Acts 27:36–37 (id: 9565)

* The food gave new strength and hope to the scared and tired crew and prisoners.
* *276 of us*: This number matches what we know about ships carrying grain from that time period.

* **Associated Passages:** Acts 27:36-37

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`)

## Acts 27:39–41 (id: 9566)

They guided the ship onto a sandbar (a shallow area of sand in the water).

* **Associated Passages:** Acts 27:39-41

* **Associated ACAI Concepts:** Ship (ID: `realia:Ship`); Sail (ID: `realia:Sail`); Rudder (ID: `realia:Rudder`); Anchor (ID: `realia:Anchor`); Rope (ID: `realia:Rope`)

## Acts 27:42–44 (id: 9569)

Even during a crisis, the soldiers were responsible for the prisoners (see 12:19; 16:27; 27:32; see study note on 16:27). Fortunately, the commanding officer helped them. This showed God's protection and favor, as all 276 people safely reached the shore, exactly as the angel promised (see 27:24).

* **Associated Passages:** Acts 27:42-44
* **Associated Articles:** Acts 16:27 (ID: `9393@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`); Ship (ID: `realia:Ship`)

## Acts 28:1 (id: 9581)

Malta was a large island controlled by the Romans, located about 100 kilometers (60 miles) south of Sicily.

* **Associated Passages:** Acts 28:1

* **Associated ACAI Concepts:** Malta (ID: `place:Malta`); Salvation (ID: `keyterm:Salvation`)

## Acts 28:3–6 (id: 9587)

The people of Malta believed that justice was a personal power or a god who punished criminals. When nothing bad happened to Paul, they changed their minds. They saw that he was not harmed by the snake. So they thought he had power over snakes and said that he must be a god (compare 14:11–12).

But this was not true. Paul did not have special power in himself. Instead, his survival showed that God was protecting him (compare Mark 16:17–18).

* **Associated Passages:** Acts 28:3-6

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Salvation (ID: `keyterm:Salvation`); Life (ID: `keyterm:Life`); Paul (ID: `person:Paul`); Snake (ID: `fauna:Snake`); Cobra (ID: `fauna:Cobra`)

## Acts 28:8–9 (id: 9589)

Compare with Luke 4:38–40.

* **Associated Passages:** Acts 28:8-9

* **Associated ACAI Concepts:** Publius (ID: `person:Publius`); Pray (ID: `keyterm:Pray`); Serve (ID: `keyterm:Serve`); Paul (ID: `person:Paul`)

## Acts 28:10 (id: 9573)

Showing their gratefulness, the people supplied the ship’s company with what they needed.

* **Associated Passages:** Acts 28:10


## Acts 28:11 (id: 9575)

Another Egyptian ship from Alexandria took Paul and his companions on board after an interval of three months and the worst of the winter had passed.

* **Associated Passages:** Acts 28:11

* **Associated ACAI Concepts:** Alexandria (ID: `place:Alexandria`); Emblem (ID: `realia:Emblem`); Ship (ID: `realia:Ship`)

## Acts 28:11–16 (id: 9574)

Luke was on this journey and described Paul's route from Malta to Rome with great geographical detail (see study note on 27:1–28:16).

* **Associated Passages:** Acts 28:11-16
* **Associated Articles:** Acts 27:1-28.16 (ID: `9553@AquiferOpenStudyNotes`)

* **Associated ACAI Concepts:** Rome (ID: `place:Rome`); Paul (ID: `person:Paul`); Alexandria (ID: `place:Alexandria`); Syracuse (ID: `place:Syracuse`); Rhegium (ID: `place:Rhegium`); Forum of Appius (ID: `place:ForumOfAppius`); Three Taverns (ID: `place:ThreeTaverns`); Puteoli (ID: `place:Puteoli`); LORD (ID: `deity:Lord`); Ask for (ID: `keyterm:AskFor`); Emblem (ID: `realia:Emblem`); Ship (ID: `realia:Ship`); Inn (ID: `realia:Inn`)

## Acts 28:12 (id: 9576)

Syracuse was the capital of the eastern half of Sicily.

* **Associated Passages:** Acts 28:12

* **Associated ACAI Concepts:** Syracuse (ID: `place:Syracuse`)

## Acts 28:13–14 (id: 9577)

* They sailed across the Straits of Messina to Rhegium at the southern tip of Italy. This seaport was a stop for ships traveling from Italy's west coast to the eastern Mediterranean.
* Puteoli (modern Pozzuoli) was a major entry port for large ships bringing grain and supplies from the east to Rome. Paul spent a week here with local Christians before continuing to Rome.

* **Associated Passages:** Acts 28:13-14

* **Associated ACAI Concepts:** Puteoli (ID: `place:Puteoli`); Rhegium (ID: `place:Rhegium`); Ask for (ID: `keyterm:AskFor`); Rome (ID: `place:Rome`)

## Acts 28:15 (id: 9578)

*The brothers there*: The Christians in that place welcomed Paul and the group as they traveled to Rome.

* **Associated Passages:** Acts 28:15

* **Associated ACAI Concepts:** Forum of Appius (ID: `place:ForumOfAppius`); Three Taverns (ID: `place:ThreeTaverns`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`); Inn (ID: `realia:Inn`)

## Acts 28:16 (id: 9579)

They allowed Paul to have private quarters, with a soldier to guard him. Even though Paul traveled in chains, “the word of God cannot be chained!” (2 Timothy 2:9). Paul was possibly treated well because of his social status or Roman citizenship (compare Acts 16:37–38; 22:25–28).

* **Associated Passages:** Acts 28:16

* **Associated ACAI Concepts:** Rome (ID: `place:Rome`); Paul (ID: `person:Paul`)

## Acts 28:17–20 (id: 9580)

Paul knew that the gospel should be shared with the Jews first (13:46; Romans 1:16). He was also concerned that false charges against him might have already reached Rome. So he called together the local Jewish leaders and explained his life and work. He insisted that he had not committed any crime. 

But strong opposition from other Jews had forced him to appeal to the emperor. Paul said he had nothing against his own people. Instead, he wanted to explain his deep conviction that the Messiah (God’s promised king) they were waiting for had already come in Jesus of Nazareth.

* **Associated Passages:** Acts 28:17-20

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Discern (ID: `keyterm:Discern`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Hope (ID: `keyterm:Hope`); Ask for (ID: `keyterm:AskFor`); Rome (ID: `place:Rome`); Call Upon (ID: `keyterm:CallUpon`); Claudius (ID: `person:Claudius`); Chain (ID: `realia:Chain`)

## Acts 28:21–22 (id: 9582)

The Jewish leaders assured Paul that they had received no reports against him, and they wanted to hear his explanation of this movement.

* **Associated Passages:** Acts 28:21-22

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Think (ID: `keyterm:Think`)

## Acts 28:23 (id: 9583)

Paul explained how Jesus fulfilled the Old Testament hopes for the kingdom of God. This was the main theme of the preaching of Jesus (Mark 1:14–15; see Matthew 4:12–17; Luke 4:14–21, 43). Paul used the Old Testament Scriptures to show that Jesus was the promised Messiah.

* **Associated Passages:** Acts 28:23

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Kingdom (ID: `keyterm:Kingdom`); Law (ID: `keyterm:Law`); Declare (ID: `keyterm:Declare`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Moses (ID: `person:Moses`); Guest Room (ID: `realia:GuestRoom`)

## Acts 28:24 (id: 9584)

Paul spoke to them all day, but the people responded in different ways, just as people had done in other places (for example, 13:40–51; 17:11–14).

* **Associated Passages:** Acts 28:24

* **Associated ACAI Concepts:** Persuade (ID: `keyterm:Persuade`); Disbelieve (ID: `keyterm:Disbelieve`)

## Acts 28:25–28 (id: 9585)

As Paul left, he gave a warning from Scripture. These words are often used in the New Testament to explain why many Jews did not accept the good news about Jesus (compare Matthew 13:14–15; Mark 4:12; Luke 8:10; John 12:38–40; see Romans 11:1–12, 25–32).

* **Associated Passages:** Acts 28:25-28

* **Associated ACAI Concepts:** Gentiles (ID: `group:Gentile`); Holy Spirit (ID: `deity:HolySpirit`); Heart (ID: `keyterm:Heart`); LORD (ID: `deity:Lord`); Word (ID: `keyterm:Word`); Isaiah (ID: `person:Isaiah`); Salvation (ID: `keyterm:Salvation`); Paul (ID: `person:Paul`)

## Acts 28:28 (id: 9586)

Since Jews everywhere had the chance to accept the faith, it was now time to offer this salvation to the Gentiles (13:46; see Romans 1:16).

* **Associated Passages:** Acts 28:28

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Gentiles (ID: `group:Gentile`); Salvation (ID: `keyterm:Salvation`)

## Acts 28:31 (id: 9588)

* Even though Paul was under house arrest, he boldly proclaimed the kingdom message (the good news about God’s rule; 28:16).
* *freely* (Greek *akōlutōs*, meaning “without restriction”): This Greek word is the last word in the book of Acts and helps show its meaning. God’s word cannot be chained, even when his messengers are (2 Timothy 2:9; see Philippians 1:12–14).

    The book of Acts tells the story of a message of good news that extends to everyone worldwide. This includes Jews and Gentiles (non\-Jews), converts to Judaism, the rich, and the poor. The mission to spread this message is done through the power of the Holy Spirit (Acts 1:8).

    It reaches:

    + Jews (3:1–5:42),
        + Samaritans (8:1–25),
        + converts to Judaism (2:11; 13:43),
        + “God\-fearers” (8:26–40; 9:32–11:18), and
        + Gentiles (13:1–28:28).

* Luke ends his account with Paul still under house arrest in Rome (around AD 60–62\). They later released Paul and he traveled again. Tradition says Paul was imprisoned again in Rome around AD 64\. He died as a martyr (a witness for Christ) during Emperor Nero’s persecution of Christians.

* **Associated Passages:** Acts 28:31

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Kingdom (ID: `keyterm:Kingdom`); Jesus (ID: `person:Jesus.2`); Proclaim (ID: `keyterm:Proclaim`)
